...
To run the test case located in the `TestCase/fiml/naca0012/` folder, you need to compile the code using the following command:
```
./meson.py setup ../build-dbg/ -Denable-autodiff=true -Denable-pywrapper=true -Denable-mixedprec=true -Denable-directdiff=true --buildtype=debug
```
The test case is then run using the script `shape_optimization.py` with the following command:
```
python3 shape_optimization.py -n 1 -g DISCRETE_ADJOINT -f turb_NACA0012_sst.cfg -o BFGSG -r project.pkl
```
The important new keywords in the configuration file are

- `KIND_RST_MODEL= POPE`, which specifies the type of modelisation for the Reynolds Stress Tensor;
- `NBPOPECOEFFS= 1`, which specifies the number of Pope's expansion coefficients;
- `INV_DESIGN_FIML= YES`, which specifies whether the FIML method is used;
- `DV_KIND= FIML`, which specifies the type of Design Variables;
- `OBJECTIVE_FUNCTION= INVERSE_DESIGN_VELOCITY_FIML`, which specifies the type of objective function; 
- `NPOIN= 3704`, which specifies the number of cells in the mesh; this is a trick to avoid writing the design variables by hand.

The major current issue is that the sensitivity of the objective function w.r.t. the DVs is simply zero. 
