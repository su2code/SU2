...
import os, time, libSU2, numpy, sys
...
def main():
    ...
    options.timestep   = int(options.timestep)
...
def merge_solution( filename            ,
    ...
    if output_format == "TECPLOT":
        ...
        if output != "False" and merge_vol != "NO":
            npoint = 0; nelem = 0;
            for domain in range(int(partitions)):
                if int(timestep) == -1:
                    ...
                    if math_problem == "DIRECT": input_file = open("%s_%s_%s.plt" % (volume_flow_filename, domain+1, timestep))
                    elif math_problem == "ADJOINT": input_file = open("%s_%s_%s.plt" % (volume_adjoint_filename, domain+1, timestep))
            ...
            for domain in range(int(partitions)):
                ...
                percentage = int(100*float(domain)/(float(partitions)-1.0))
                sys.stdout.write("\rMerging solution files (.plt)... %d%%" % percentage)
                sys.stdout.flush()
            ...
            if int(timestep) == -1:
                ...
                if math_problem == "DIRECT": output_file = open("%s_%s.plt" % (volume_flow_filename, timestep) ,"w")
                elif math_problem == "ADJOINT": output_file = open("%s_%s.plt" % (volume_adjoint_filename, timestep) ,"w")
            ...
            for domain in range(int(partitions)):
                if int(timestep) == -1:
                    ...
                    if math_problem == "DIRECT": os.remove("%s_%s_%s.plt" % (volume_flow_filename, domain+1, timestep))
                    elif math_problem == "ADJOINT": os.remove("%s_%s_%s.plt" % (volume_adjoint_filename, domain+1, timestep))
        ...
        if math_problem == "DIRECT":
            if output != "False" and merge_vol != "NO":
                ...
                for domain in range(int(partitions)):
                    if int(timestep) == -1:
                        ...
                        input_file = open("%s_%s_%s.vtk" % (volume_flow_filename, domain+1, int(timestep)))
                    for line in input_file:
                        if "POINTS" in line:
            ...
            # Open the output file
                if int(timestep) == -1:
                    ...
                    output_file = open("%s_%s.vtk" % (volume_flow_filename, int(timestep)) ,"w")
            ...
            # Write the flow variables
                ...
                for domain in range(int(partitions)):
                    if int(timestep) == -1:
                        ...
                        os.remove("%s_%s_%s.vtk" % (volume_flow_filename, domain+1, int(timestep)))
        ...
        # Surface file
            if merge_srf != "NO":
                ...
                for domain in range(int(partitions)):
                    if int(timestep) == -1:
                        ...
                        input_file = open("%s_%s_%s.vtk" % (surface_flow_filename, domain+1, int(timestep)))         
                    for line in input_file:
                        if "POINTS" in line:
                            local_point = int(line.replace("\t"," ").split(" ")[1])
                            npoint = npoint + local_point             
            ...
            # Open the output file
                if int(timestep) == -1:
                    ...
                    output_file = open("%s_%s.vtk" % (surface_flow_filename, int(timestep)) ,"w")
            ...
            # Write the flow variables
                ...
                for domain in range(int(partitions)):
                    if int(timestep) == -1:
                        ...
                        os.remove("%s_%s_%s.vtk" % (surface_flow_filename, domain+1, int(timestep)))
