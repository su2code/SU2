...
void CSparseMatrix::SetIndexes(unsigned long val_nPoint, unsigned long val_nPointDomain, unsigned short val_nVar, unsigned long* val_row_ptr, 
...
void CSparseMatrix::ComputeLineletPreconditioner(const CSysVector & vec, CSysVector & prod) {
	...
	for (iLinelet = 0; iLinelet < nLinelet; iLinelet++) {
		...
		for (iElem = 0; iElem < nElem; iElem++) {
			iPoint = LineletPoint[iLinelet][iElem];
			for (iVar = 0; iVar < nVar; iVar++)
				rVector[iElem][iVar] = vec[iPoint*nVar+iVar];
		}
		...
		for (iElem = 0; iElem < nElem; iElem++) {
			iPoint = LineletPoint[iLinelet][iElem];
			for (iVar = 0; iVar < nVar; iVar++)
				prod[iPoint*nVar+iVar] = zVector[iElem][iVar];
		}
	}
