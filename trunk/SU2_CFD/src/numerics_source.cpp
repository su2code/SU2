...
void CSourcePieceWise_TransLM::SetResidual_TransLM(double *val_residual, double **val_Jacobian_i, double **val_Jacobian_j, CConfig *config, double &gamma_sep) {
	...
	//  cout << "Zeroing out val_residual and Jacobian -AA\n";
	val_residual[0] = 0.0;
	val_residual[1] = 0.0;
	if (implicit) {
		val_Jacobian_i[0][0] = 0.0;
	...
	if (dist_i > 0.0) {   // Only operate away from wall
		...
		//      cout << "Implicit part: -AA\n";
		/*--- Implicit part ---*/
		if (implicit) {
			var1 = 0.;
			if (TransVar_i[0] > 0.001) {
				var1 = 0.5*sqrt(f_onset/TransVar_i[0]);
			} else {
				//        var1 = f_onset/TransVar_i[0]-1;
				//        var1 = 0.5* (1- (var1)/2 + 3/8*pow(var1,2));
			}
			val_Jacobian_i[0][0]  = flen*c_a1*U_i[0]*strain*(var1-3./2*c_e1*sqrt(f_onset*TransVar_i[0]));
			val_Jacobian_i[0][0] -= c_a2*U_i[0]*Vorticity*f_turb*(2*c_e2*TransVar_i[0]-1);

			val_Jacobian_i[0][1] = 0.0;
			val_Jacobian_i[1][1] = -c_theta*U_i[0]/time_scale *  (1.-f_theta);
			val_Jacobian_i[1][0] = 0.0;
...
void CSource_JouleHeating::SetResidual(double *val_residual, double **val_Jacobian_i, CConfig *config) {
	...
	double Current = 1.9;//config->GetCurrent();
	for (unsigned short iVar = 0; iVar < nVar; iVar ++) {
		val_residual[iVar] = 0.0;
		for (unsigned short jVar = 0; jVar < nVar; jVar ++)
			val_Jacobian_i[iVar][jVar] = 0.0;
	...
	val_residual[nDim] = Current*Current*Elec_Conduct*Volume/(4.0*PI_NUMBER*PI_NUMBER*Integralsqr);
}
...
void CSource_JouleHeating::SetElec_Cond() {
	...
	Temperature = Density/(Gas_Constant*Pressure);
	...
	sigma = coeff_a[0] - coeff_d[0]*exp(-pow(Temperature/coeff_d[0],w));
	...
	coeff_a[2] =  exp(1.593153e0*x0  +  4.137850e-2*x1 + 1.430491e-2*x2  - 4.403957e-7*x3  + 0.0*x4);
	coeff_a[3] = -exp(2.627897e-1*x0 +  2.917558e-3*x1 + 3.036205e-3*x2  - 1.926651e-4*x3  - 2.917018e-5*x4);
	coeff_a[4] = -exp(1.707216e-1*x0 +  2.035164e-2*x1 + 1.809127e-3*x2  - 9.630175e-5*x3  + 1.781249e-5*x4);
	coeff_a[5] = -exp(2.480007e-1*x0 +  2.217818e-2*x1 + 9.094614e-4*x2  + 0.0*x3 		   + 0.0*x4);
	coeff_a[6] =  exp(3.636707e0*x0  -  1.436268e-1*x1 - 2.934926e-3*x2  + 0.0*x3 		   + 0.0*x4);
	...
	//cout << " Elec_Cond = " << Elec_Conduct << endl;
}
