...
void CSolution::Initialize_Jacobian_Structure(CGeometry *geometry, CConfig *config) {
	...
	unsigned short nSub_blocks, iSub_blocks;
	unsigned short *Sub_block_sizes;
	bool preconditioner = false, blockDiagonalJacobian = false;
	...
	blockDiagonalJacobian = config->GetBlockDiagonalJacobian();

	nSub_blocks = 1;
	Sub_block_sizes = new unsigned short [nSub_blocks];
	Sub_block_sizes[0] = nVar;

	if ((config->GetKind_Solver() == PLASMA_EULER) || (config->GetKind_Solver() == PLASMA_NAVIER_STOKES)) {
		nSub_blocks = nSpecies;
		Sub_block_sizes = new unsigned short [nSub_blocks];
		for ( iSub_blocks = 0; iSub_blocks < nSub_blocks; iSub_blocks++)
			if (iSub_blocks < nDiatomics)	Sub_block_sizes[iSub_blocks] = (nDim+3);
			else Sub_block_sizes[iSub_blocks] = (nDim+2);
	}
	Jacobian.SetIndexes(nPoint, nPointDomain, nVar, row_ptr, col_ind, nnz, preconditioner, blockDiagonalJacobian, nSub_blocks, Sub_block_sizes);
...
void CSolution::Initialize_StiffMatrix_Structure(CGeometry *geometry, CConfig *config) {
	...
	StiffMatrix.SetIndexes(nPoint, nPointDomain, nVar, row_ptr, col_ind, nnz, preconditioner, blockDiagonalJacobian, nSub_blocks, Sub_block_sizes);
...
void CSolution::Initialize_StiffMatrixSpace_Structure(CGeometry *geometry, CConfig *config) {
	...
	StiffMatrixSpace.SetIndexes(nPoint, nPointDomain, nVar, row_ptr, col_ind, nnz, preconditioner, blockDiagonalJacobian, nSub_blocks, Sub_block_sizes);
...
void CSolution::Initialize_StiffMatrixTime_Structure(CGeometry *geometry, CConfig *config) {
	...
	StiffMatrixTime.SetIndexes(nPoint, nPointDomain, nVar, row_ptr, col_ind, nnz, preconditioner, blockDiagonalJacobian, nSub_blocks, Sub_block_sizes);
