...
void SetSensitivity_Files(CGeometry ***geometry, CConfig **config, unsigned short val_nZone) {
  ...
  for (iZone = 0; iZone < val_nZone; iZone++) {
    ...
    for (iPoint = 0; iPoint < nPoint; iPoint++) {
      for (iDim = 0; iDim < nDim; iDim++) {
        solver->node->SetSolution(iPoint, iDim, geometry[iZone][INST_0]->node[iPoint]->GetCoord(iDim));
      }
      for (iVar = 0; iVar < nDim; iVar++) {
        solver->node->SetSolution(iPoint, iVar+nDim, geometry[iZone][INST_0]->GetSensitivity(iPoint, iVar));
      }
    }
    ...
    for (iMarker = 0; iMarker < nMarker; iMarker++) {
      ...
      if((config[iZone]->GetMarker_All_KindBC(iMarker) == HEAT_FLUX ) ||
        ...
        for (iVertex = 0; iVertex < nVertex; iVertex++) {
          ...
          solver->node->SetSolution(iPoint, 2*nDim, Sens);
