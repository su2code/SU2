...
int main(int argc, char *argv[]) {
  ...
  for (iZone = 0; iZone < nZone; iZone++) {
    ...
    geometry_container[iZone] = new CPhysicalGeometry(geometry_aux, config_container[iZone], nZone);
...
//    strcpy (out_file, "Surface_Grid"); strcat(out_file, buffer_char); geometry[ZONE_0]->SetBoundSTL(out_file, true, config[ZONE_0]);
   ...
   * Otherwise, the surface deformation has been set already in SetSurface_Deformation.  --- */
    ...
    if (config_container[iZone]->GetDesign_Variable(0) == SCALE and allmoving) {
    ...
    } else if (config_container[iZone]->GetDesign_Variable(0) == TRANSLATION and allmoving) {
    ...
    } else if (config_container[iZone]->GetDesign_Variable(0) == ROTATION and allmoving) {
