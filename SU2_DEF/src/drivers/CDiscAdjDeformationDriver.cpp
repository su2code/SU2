...
void CDiscAdjDeformationDriver::Input_Preprocessing() {
    ...
    // TODO Check that config is for Discrete Adjoint !
...
void CDiscAdjDeformationDriver::SetDiscAdjDeformation_FD(CGeometry *geometry, CConfig *config, CSurfaceMovement *surface_movement, su2double** Gradient){
  ...
  for (iDV = 0; iDV  < nDV; iDV++){
    ...
    if (nDV_Value != 1){
      SU2_MPI::Error("The DiscAdjDeformation using finite differences currently only supports a fixed direction of movement for FFD points.", CURRENT_FUNCTION);
    }
  }
...
void CDiscAdjDeformationDriver::SetDiscAdjDeformation_AD(CGeometry *geometry, CConfig *config, CSurfaceMovement *surface_movement, su2double** Gradient){
      ...
      if(config[iZone]->GetSolid_Wall(iMarker) || (config[iZone]->GetMarker_All_DV(iMarker) == YES )) {
        ...
        for (iVertex = 0; iVertex < nVertex; iVertex++) {
          ...
          for (iDim = 0; iDim < nDim; iDim++) {
            ...
            /*--- Calculate scalar product for DiscAdjDeformation onto the normal vector ---*/
          ...
          /*--- DiscAdjDeformation of the gradient onto the normal vector of the surface ---*/
