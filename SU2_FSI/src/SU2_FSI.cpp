/*!
 * \file SU2_FSI.cpp
 * \brief Main file of the Fluid Structure Interaction code
...
int main(int argc, char *argv[]) {
	...
	for (iZone = 0; iZone < nZone; iZone++) {
		...
		driver->Solver_Preprocessing(solver_container[iZone], geometry_container[iZone],
		...
		driver->Integration_Preprocessing(integration_container[iZone], geometry_container[iZone],
				config_container[iZone]);
		...
		driver->Numerics_Preprocessing(numerics_container[iZone], solver_container[iZone],
	...
	if ((nZone > 1) && (nZone < 3)) {
		...
		driver->Interface_Preprocessing(transfer_container, interpolator_container, geometry_container,
...
//			}
	...
	if (rank == MASTER_NODE)
		cout << endl <<"------------------------- Exit Success (SU2_FSI) ------------------------" << endl << endl;
