/*!
 * \file CFEMStandardPyraVolumeSol.cpp
 * \brief Functions for the class CFEMStandardPyraVolumeSol.
...
/*----------------------------------------------------------------------------------*/
/*             Public member functions of CFEMStandardPyraVolumeSol.                */
/*----------------------------------------------------------------------------------*/
...
CFEMStandardPyraVolumeSol::CFEMStandardPyraVolumeSol(const unsigned short val_nPoly,
  ...
  VandermondePyramid(nPoly, rPyraSolDOFs, sPyraSolDOFs, tPyraSolDOFs, Vtmp.GetMat());
...
passivedouble CFEMStandardPyraVolumeSol::GetFactorInviscidSpectralRadius(const unsigned short nPolyElem) const {
...
passivedouble CFEMStandardPyraVolumeSol::GetFactorViscousSpectralRadius(const unsigned short nPolyElem) const {
...
void CFEMStandardPyraVolumeSol::ModalToNodal(ColMajorMatrix<su2double> &solDOFs) {
...
void CFEMStandardPyraVolumeSol::NodalToModal(ColMajorMatrix<su2double> &solDOFs) {
...
void CFEMStandardPyraVolumeSol::GradSolIntPoints(ColMajorMatrix<su2double>          &matSolDOF,
...
void CFEMStandardPyraVolumeSol::SolIntPoints(ColMajorMatrix<su2double> &matSolDOF,
...
void CFEMStandardPyraVolumeSol::SolIntPointsDOFsPadded(ColMajorMatrix<su2double> &matSolDOF,
                                                       ColMajorMatrix<su2double> &matSolInt) {
...
void CFEMStandardPyraVolumeSol::ResidualBasisFunctions(ColMajorMatrix<su2double> &scalarDataInt,
...
void CFEMStandardPyraVolumeSol::ResidualGradientBasisFunctions(vector<ColMajorMatrix<su2double> > &vectorDataInt,
                                                               ColMajorMatrix<su2double>          &resDOFs) {
...
void CFEMStandardPyraVolumeSol::MultiplicationFactorsPSequencing(void) {
  ...
  /*--- Loop over the DOFs of this standard pyramd in the same manner
    ...
    for(unsigned short j=0; j<=nPoly; ++j) {
      ...
      for(unsigned short k=0; k<=(nPoly-muij); ++k, ++ii) {
        ...
        /*--- Determine the polynomial degree from which on
              this DOF is present. ---*/
        const unsigned short nPolyLimit = k + muij;
