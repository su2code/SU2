...
void CFEMStandardTetVolumeSol::MultiplicationFactorsPSequencing(void) {
  ...
  /*--- Loop over the DOFs of this standard tet in the same manner
    ...
    for(unsigned short j=0; j<=(nPoly-i); ++j) {
      for(unsigned short k=0; k<=(nPoly-i-j); ++k, ++ii) {
        ...
        /*--- Determine the polynomial degree from which on
              this DOF is present. ---*/
        const unsigned short nPolyLimit = i + j + k;
