/*!
 * \file TensorProductVolumeIntPoints2D_11_8.cpp
 * \brief Function, which carries out the tensor product for (nDOFs1D,nInt1D) = (11,8)
...
void TensorProductVolumeIntPoints2D_11_8(const int           N,
  ...
  su2double tmpJ[11][MP];

  /*--- Outer loop over N. ---*/
  for(int l=0; l<N; ++l) {

    /*--- Cast the index l of B and C to multi-dimensional arrays. ---*/
    const su2double (*b)[11] = (const su2double (*)[11]) &B[l*ldb];
