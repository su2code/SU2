/*!
 * \file TensorProductSurfaceIntPoints3D_6_9.cpp
...
void TensorProductSurfaceIntPoints3D_6_9(const int           N,
                                         const int           faceID,
                                         const int           ldb,
                                         const int           ldc,
  ...
  const int MP = CFEMStandardElementBase::PaddedValue(9);
  ...
  su2double bFace[6][6];
  su2double tmpI[6][MP], tmpJ[9][MP];

  /*--- Outer loop over N. ---*/
  for(int l=0; l<N; ++l) {

    /*--- Cast the index l of B to a 3D array. ---*/
    const su2double (*b)[6][6] = (const su2double (*)[6][6]) &B[l*ldb];
    ...
    for(int j=0; j<6; ++j) {
      SU2_OMP_SIMD_(safelen(6))
      for(int i=0; i<6; ++i) bFace[j][i] = 0.0;
    }
