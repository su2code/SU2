/*!
 * \file TensorProductSurfaceIntPoints3D_8_12.cpp
...
void TensorProductSurfaceIntPoints3D_8_12(const int           N,
                                          const int           faceID,
                                          const int           ldb,
                                          const int           ldc,
  ...
  su2double bFace[8][8];
  su2double tmpI[8][MP], tmpJ[12][MP];
  ...
  for(int l=0; l<N; ++l) {
    ...
    for(int j=0; j<8; ++j) {
      ...
      for(int i=0; i<8; ++i) bFace[j][i] = 0.0;
    }
