/*!
 * \file TensorProductSurfaceIntPoints3D_10_10.cpp
...
void TensorProductSurfaceIntPoints3D_10_10(const int           N,
                                           const int           faceID,
                                           const int           ldb,
                                           const int           ldc,
  ...
  su2double bFace[10][10];
  su2double tmpI[10][MP], tmpJ[10][MP];
  ...
  for(int l=0; l<N; ++l) {
    ...
    for(int j=0; j<10; ++j) {
      SU2_OMP_SIMD_(safelen(10))
      for(int i=0; i<10; ++i) bFace[j][i] = 0.0;
    }
    ...
    if((faceID == 0) || (faceID == 1)) {
      ...
      for(int k=0; k<10; ++k) {
        for(int j=0; j<10; ++j) {
          SU2_OMP_SIMD_IF_NOT_AD_(safelen(10))
          for(int i=0; i<10; ++i)
