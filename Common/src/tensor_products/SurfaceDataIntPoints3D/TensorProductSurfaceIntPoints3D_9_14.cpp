/*!
 * \file TensorProductSurfaceIntPoints3D_9_14.cpp
 * \brief Function, which carries out the tensor product for (nDOFs1D,nInt1D) = (9,14)
...
void TensorProductSurfaceIntPoints3D_9_14(const int           N,
                                          const int           faceID,
                                          const int           ldb,
                                          const int           ldc,
  ...
  const int MP = CFEMStandardElementBase::PaddedValue(14);
  ...
  su2double bFace[9][9];
  su2double tmpI[9][MP], tmpJ[14][MP];
  ...
  for(int l=0; l<N; ++l) {
    ...
    for(int j=0; j<9; ++j) {
      SU2_OMP_SIMD_(safelen(9))
      for(int i=0; i<9; ++i) bFace[j][i] = 0.0;
    }
    ...
    if((faceID == 0) || (faceID == 1)) {
      ...
      for(int k=0; k<9; ++k) {
        for(int j=0; j<9; ++j) {
          SU2_OMP_SIMD_IF_NOT_AD_(safelen(9))
          for(int i=0; i<9; ++i)
