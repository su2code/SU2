/*!
 * \file TensorProductSurfaceIntPoints2D_4_4.cpp
 * \brief Function, which carries out the tensor product for (nDOFs1D,nInt1D) = (4,4)
...
void TensorProductSurfaceIntPoints2D_4_4(const int           N,
  ...
  for(int l=0; l<N; ++l) {
    ...
    SU2_OMP_SIMD_(safelen(4))
    for(int i=0; i<4; ++i) bFace[i] = 0.0;
