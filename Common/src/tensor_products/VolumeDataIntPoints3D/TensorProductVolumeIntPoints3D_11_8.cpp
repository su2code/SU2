/*!
 * \file TensorProductVolumeIntPoints3D_11_8.cpp
...
void TensorProductVolumeIntPoints3D_11_8(const int           N,
                                         const int           ldb,
                                         const int           ldc,
  ...
  su2double tmpK[11][11][MP];
  su2double tmpJ[8][11][MP];

  /*--- Outer loop over N. ---*/
  for(int l=0; l<N; ++l) {

    /*--- Cast the index l of B and C to multi-dimensional arrays. ---*/
    const su2double (*b)[11][11] = (const su2double (*)[11][11]) &B[l*ldb];
