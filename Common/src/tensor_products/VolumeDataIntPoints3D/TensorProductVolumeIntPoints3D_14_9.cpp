/*!
 * \file TensorProductVolumeIntPoints3D_14_9.cpp
 * \brief Function, which carries out the tensor product for (nDOFs1D,nInt1D) = (14,9)
...
void TensorProductVolumeIntPoints3D_14_9(const int           N,
                                         const int           ldb,
                                         const int           ldc,
  ...
  su2double tmpK[14][14][MP];
  su2double tmpJ[9][14][MP];

  /*--- Outer loop over N. ---*/
  for(int l=0; l<N; ++l) {

    /*--- Cast the index l of B and C to multi-dimensional arrays. ---*/
    const su2double (*b)[14][14] = (const su2double (*)[14][14]) &B[l*ldb];
