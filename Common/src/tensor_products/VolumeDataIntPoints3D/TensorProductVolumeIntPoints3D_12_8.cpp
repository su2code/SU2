/*!
 * \file TensorProductVolumeIntPoints3D_12_8.cpp
 * \brief Function, which carries out the tensor product for (nDOFs1D,nInt1D) = (12,8)
...
void TensorProductVolumeIntPoints3D_12_8(const int           N,
                                         const int           ldb,
                                         const int           ldc,
  ...
  su2double tmpK[12][12][MP];
  su2double tmpJ[8][12][MP];

  /*--- Outer loop over N. ---*/
  for(int l=0; l<N; ++l) {

    /*--- Cast the index l of B and C to multi-dimensional arrays. ---*/
    const su2double (*b)[12][12] = (const su2double (*)[12][12]) &B[l*ldb];
