/*!
 * \file TensorProductSurfaceResVolumeDOFs3D_13_8.cpp
 * \brief Function, which carries out the tensor product for (nDOFs1D,nInt1D) = (13,8)
...
void TensorProductSurfaceResVolumeDOFs3D_13_8(const int           N,
  ...
  const int KP = CFEMStandardElementBase::PaddedValue(13);
  ...
  su2double bFace[13][KP];
  su2double tmpI[8][KP], tmpJ[8][8];
  ...
  for(int l=0; l<N; ++l) {
    ...
    if((faceID == 0) || (faceID == 1)) {
      ...
      for(int k=0; k<13; ++k) {
        for(int j=0; j<13; ++j) {
          SU2_OMP_SIMD_IF_NOT_AD_(safelen(13))
          for(int i=0; i<13; ++i)
