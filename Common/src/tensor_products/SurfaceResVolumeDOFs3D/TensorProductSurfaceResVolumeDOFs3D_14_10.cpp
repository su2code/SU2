/*!
 * \file TensorProductSurfaceResVolumeDOFs3D_14_10.cpp
 * \brief Function, which carries out the tensor product for (nDOFs1D,nInt1D) = (14,10)
...
void TensorProductSurfaceResVolumeDOFs3D_14_10(const int           N,
  ...
  const int KP = CFEMStandardElementBase::PaddedValue(14);
  ...
  su2double bFace[14][KP];
  su2double tmpI[10][KP], tmpJ[10][10];

  /*--- Outer loop over N. ---*/
  for(int l=0; l<N; ++l) {
    const su2double (*b)[10] = (const su2double (*)[10]) &B[l*ldb];
    su2double (*c)[14][14] = (su2double (*)[14][14]) &C[l*ldc];
    ...
    if((faceID == 0) || (faceID == 1)) {
      ...
      for(int k=0; k<14; ++k) {
        for(int j=0; j<14; ++j) {
          SU2_OMP_SIMD_IF_NOT_AD_(safelen(14))
          for(int i=0; i<14; ++i)
