/*!
 * \file TensorProductSurfaceResVolumeDOFs3D_9_14.cpp
...
void TensorProductSurfaceResVolumeDOFs3D_9_14(const int           N,
  ...
  const int KP = CFEMStandardElementBase::PaddedValue(9);
  ...
  su2double bFace[9][KP];
  su2double tmpI[14][KP], tmpJ[14][14];

  /*--- Outer loop over N. ---*/
  for(int l=0; l<N; ++l) {
    const su2double (*b)[14] = (const su2double (*)[14]) &B[l*ldb];
