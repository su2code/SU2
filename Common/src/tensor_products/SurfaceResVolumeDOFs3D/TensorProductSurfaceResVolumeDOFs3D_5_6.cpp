/*!
 * \file TensorProductSurfaceResVolumeDOFs3D_5_6.cpp
...
void TensorProductSurfaceResVolumeDOFs3D_5_6(const int           N,
  ...
  const int KP = CFEMStandardElementBase::PaddedValue(5);
  ...
  su2double bFace[5][KP];
  su2double tmpI[6][KP], tmpJ[6][6];

  /*--- Outer loop over N. ---*/
  for(int l=0; l<N; ++l) {
    const su2double (*b)[6] = (const su2double (*)[6]) &B[l*ldb];
    su2double (*c)[5][5] = (su2double (*)[5][5]) &C[l*ldc];
    ...
    if((faceID == 0) || (faceID == 1)) {
      ...
      for(int k=0; k<5; ++k) {
        for(int j=0; j<5; ++j) {
          SU2_OMP_SIMD_IF_NOT_AD_(safelen(5))
          for(int i=0; i<5; ++i)
