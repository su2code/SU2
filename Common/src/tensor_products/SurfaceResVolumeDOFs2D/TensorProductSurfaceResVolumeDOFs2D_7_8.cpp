/*!
 * \file TensorProductSurfaceResVolumeDOFs2D_7_8.cpp
 * \brief Function, which carries out the tensor product for (nDOFs1D,nInt1D) = (7,8)
...
void TensorProductSurfaceResVolumeDOFs2D_7_8(const int           N,
  ...
  const int KP = CFEMStandardElementBase::PaddedValue(7);
  ...
  for(int l=0; l<N; ++l) {
    ...
    if((faceID == 0) || (faceID == 2)) {
      ...
      for(int j=0; j<7; ++j) {
        SU2_OMP_SIMD_IF_NOT_AD_(safelen(7))
        for(int i=0; i<7; ++i)
          c[j][i] += an[j]*bFace[i];
      }
    }
