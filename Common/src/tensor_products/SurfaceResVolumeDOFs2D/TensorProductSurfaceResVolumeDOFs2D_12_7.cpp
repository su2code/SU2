/*!
 * \file TensorProductSurfaceResVolumeDOFs2D_12_7.cpp
 * \brief Function, which carries out the tensor product for (nDOFs1D,nInt1D) = (12,7)
...
void TensorProductSurfaceResVolumeDOFs2D_12_7(const int           N,
  ...
  const int KP = CFEMStandardElementBase::PaddedValue(12);
  ...
  for(int l=0; l<N; ++l) {
    ...
    if((faceID == 0) || (faceID == 2)) {
      ...
      for(int j=0; j<12; ++j) {
        SU2_OMP_SIMD_IF_NOT_AD_(safelen(12))
        for(int i=0; i<12; ++i)
          c[j][i] += an[j]*bFace[i];
      }
    }
