...
void CConfig::SetPostprocessing(unsigned short val_software, unsigned short val_izone, unsigned short val_nDim) {
  ...
  if (!ideal_gas) {
		if (Kind_Upwind_Flow != ROE && Kind_Upwind_Flow !=  AUSMPlus && Kind_Upwind_Flow !=  JST && Kind_Upwind_Flow != HLLC) {
			cout << "Only ROE, AUSM+, HLLC and JST Upwind scheme can be used for Not Ideal Compressible Fluids" << endl;
			exit(EXIT_FAILURE);
		}
	}
