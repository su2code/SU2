...
CMMSNSTwoHalfCirclesSolution::CMMSNSTwoHalfCirclesSolution(unsigned short val_nDim,
  ...
  if(Kind_Solver != MAIN_SOLVER::EULER && Kind_Solver != MAIN_SOLVER::NAVIER_STOKES && Kind_Solver != MAIN_SOLVER::RANS &&
     ...
     Kind_Solver != MAIN_SOLVER::FEM_LES)    SU2_MPI::Error("Compressible flow equations must be selected for the MMS NS Two Half Circles case",
                   CURRENT_FUNCTION);
...
bool CMMSNSTwoHalfCirclesSolution::IsManufacturedSolution(void) const {
  return true;
}
