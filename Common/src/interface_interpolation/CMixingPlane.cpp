...
void CMixingPlane::WriteInterpolationDetails(const std::string& filename, const CConfig* const* config) {
  ...
  outFile << "Mixing-Plane Interpolator Details. Donor Zone = " << donorZone << " Target Zone = " << targetZone
  ...
  for (auto iMarkerInt = 0; iMarkerInt < nMarkerInt + 1; iMarkerInt++) {
    ...
    for (size_t iSpanTarget = 0; iSpanTarget < targetSpans[iMarkerInt].size(); iSpanTarget++) {
      ...
      outFile << iSpanTarget << ", " << targetSpan.donorSpan << ", " << targetSpan.coefficient << "\n";
    }
    outFile << "\n";
  }
    ...
    for (unsigned long iDonor = 0; iDonor <= maxDonorSpan; iDonor++) {
      ...
      for (size_t iSpanTarget = 0; iSpanTarget < targetSpans[iMarkerInt].size(); iSpanTarget++) {
        if (targetSpans[iMarkerInt][iSpanTarget].donorSpan == iDonor) {
          ...
          targets << "Target " << iSpanTarget << " (coeff=" << targetSpans[iMarkerInt][iSpanTarget].coefficient << ")";
          hasTargets = true;
        }
      }
