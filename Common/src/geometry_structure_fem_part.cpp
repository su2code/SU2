  ...
  while (getline (mesh_file, text_line)) {
    ...
    if (position != string::npos) {
      ...
      for(unsigned long i = 0; i < (unsigned long)size; i++) {
        nPoint_Lin[i] = Global_nElem/size;
        total_elem_accounted = total_elem_accounted + nPoint_Lin[i];
      }
...
void CPhysicalGeometry::SetColorFEMGrid_Parallel(CConfig *config) {
  ...
  ComputeFEMGraphWeights(config, localFaces, adjacency,
  ...
  if(size > SINGLE_NODE)
  {
    ...
    xadjPar[0] = 0;
    for(unsigned long i=0; i<nElem; ++i)
      xadjPar[i+1] = xadjPar[i] + (idx_t)adjacency[i].size();
    ...
    for(unsigned long i=0; i<nElem; ++i) {
      for(unsigned long j=0; j<adjwgt[i].size(); ++j, ++ii)
        adjwgtPar[ii] = (idx_t) ceil(adjwgt[i][j]);
    }
