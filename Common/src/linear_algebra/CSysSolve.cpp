...
unsigned long CSysSolve<ScalarType>::CG_LinSolver(const CSysVector<ScalarType> & b, CSysVector<ScalarType> & x,
  ...
  if (config->GetComm_Level() == COMM_FULL) {
    ...
    if ((norm_r < tol*norm0) || (norm_r < eps)) {
      if (master && !config->GetBoolOneShot()) cout << "CSysSolve::ConjugateGradient(): system solved by initial guess." << endl;
      return 0;
    }
...
unsigned long CSysSolve<ScalarType>::FGMRES_LinSolver(const CSysVector<ScalarType> & b, CSysVector<ScalarType> & x,
  ...
  if ((beta < tol*norm0) || (beta < eps)) {
    ...
    if (master && !config->GetBoolOneShot()) cout << "CSysSolve::FGMRES(): system solved by initial guess." << endl;
    residual = beta;
    return 0;
  }
...
unsigned long CSysSolve<ScalarType>::BCGSTAB_LinSolver(const CSysVector<ScalarType> & b, CSysVector<ScalarType> & x,
  ...
  if (config->GetComm_Level() == COMM_FULL) {
    ...
    norm0  = b.norm();
    if ((norm_r < tol*norm0) || (norm_r < eps)) {
      if (master && !config->GetBoolOneShot()) cout << "CSysSolve::BCGSTAB(): system solved by initial guess." << endl;
      return 0;
    }
...
unsigned long CSysSolve<ScalarType>::Smoother_LinSolver(const CSysVector<ScalarType> & b, CSysVector<ScalarType> & x,
  ...
  if (config->GetComm_Level() == COMM_FULL) {
    ...
    norm0  = b.norm();
    if ( (norm_r < tol*norm0) || (norm_r < eps) ) {
      if (master && !config->GetBoolOneShot()) cout << "CSysSolve::Smoother_LinSolver(): system solved by initial guess." << endl;
      return 0;
    }
