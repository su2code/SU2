...
unsigned long CSysSolve<ScalarType>::Solve(CSysMatrix<ScalarType>& Jacobian, const CSysVector<su2double>& LinSysRes,
   ...
   *    alone), and to the coarse levels: the finest grid always rebuilds, because its preconditioner
   *    drives the outer nonlinear convergence and freezing it measured worth far less than freezing
   *    the coarse levels. The first solve on this instance always builds (count 0), which matters
   *    because the factorization is otherwise uninitialized.
   *
