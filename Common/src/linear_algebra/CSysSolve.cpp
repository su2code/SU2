...
class CHostFGMRESOps {
 public:
  static constexpr bool UseSolverModGramSchmidt = true;

  bool NestedParallel() const { return !omp_in_parallel() && omp_get_max_threads() > 1; }

  void PrepareInput(const CSysVector<ScalarType>&, CSysVector<ScalarType>&, bool) const {}

  void FinalizeOutput(CSysVector<ScalarType>&) const {}

  ScalarType Norm(const CSysVector<ScalarType>& v) const { return v.norm(); }
  ...
  void UpdateSolution(unsigned long n, const std::vector<CSysVector<ScalarType>>& basis, const su2vector<ScalarType>& y,
                      CSysVector<ScalarType>& x, bool nestedParallel) const {
    LinearCombination(nestedParallel, n, basis, y, x, true);
  }
};
