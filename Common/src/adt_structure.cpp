...
su2_adtComparePointClass::su2_adtComparePointClass(const su2double *coor,
		const unsigned short splitDir, const unsigned short nDimADT) :
...
void su2_adtBaseClass::BuildADT(unsigned short nDim, unsigned long nPoints,
	...
	for (;;) {
		...
		for (unsigned long i = 0; i < nPointIDs[nLeavesToDivide]; ++i)
			pointIDs[i] = pointIDsNew[i];
	}
}

su2_adtPointsOnlyClass::su2_adtPointsOnlyClass(unsigned short nDim,
		unsigned long nPoints, const su2double *coor,
...
void su2_adtPointsOnlyClass::DetermineNearestNode(const su2double *coor,
		su2double &dist, unsigned long &pointID, int &rankID) {
