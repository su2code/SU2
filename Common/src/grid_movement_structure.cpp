...
void CFreeFormDefBox::SetCGNS(CGeometry *geometry, unsigned short iFFDBox, bool original) {
  ...
  if (new_file) {
    cg_open(FFDBox_filename, CG_MODE_WRITE, &FFDBox_cgfile);
    cg_descriptor_write("Title", "Visualization of the FFD boxes generated by SU2_DEF." );
  }
  else cg_open(FFDBox_filename, CG_MODE_MODIFY, &FFDBox_cgfile);
  ...
  if (iFFDBox == 0)
     cg_base_write(FFDBox_cgfile, basename, cell_dim, phys_dim, &cgbase);

  cg_family_write(FFDBox_cgfile, cgbase, Tag.c_str(), &cgfam);
  ...
  cg_zone_write(FFDBox_cgfile, cgbase, zonename, dims, CGNS_ENUMV(Structured), &cgzone);
  cg_goto(FFDBox_cgfile, cgbase, zonename, 0, NULL);
  cg_famname_write(Tag.c_str());
  ...
  for (iDim=0; iDim<nDim; iDim++)
  {
    ...
    cg_coord_write(FFDBox_cgfile, cgbase, cgzone, CGNS_ENUMV(RealDouble), names[iDim], buffer, &dummy);

  }
  if (nDim==2){
    std::fill_n(buffer, pointlen, 0.0);
    cg_coord_write(FFDBox_cgfile, cgbase, cgzone, CGNS_ENUMV(RealDouble), names[2], buffer, &dummy);
  }

  delete [] buffer;
  cg_close(FFDBox_cgfile);
#endif
}
