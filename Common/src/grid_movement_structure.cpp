...
void CVolumetricMovement::SetVolume_Deformation(CGeometry *geometry, CConfig *config, bool UpdateGeo, bool Derivative) {
  ...
  for (iNonlinear_Iter = 0; iNonlinear_Iter < Nonlinear_Iter; iNonlinear_Iter++) {
    ...
    Transform_DoFs(geometry, config, LinSysSol, true);
...
su2double CVolumetricMovement::SetFEAMethodContributions_Elem(CGeometry *geometry, CConfig *config) {
  ...
  if ((config->GetDeform_Stiffness_Type() == SOLID_WALL_DISTANCE) ||
    ...
    if ((config->GetDeform_Stiffness_Type() == SOLID_WALL_DISTANCE) ||
            (config->GetDeform_Stiffness_Type() == DEF_WALL_DISTANCE)) {
      ElemDistance = 0.0;
      for (iNodes = 0; iNodes < nNodes; iNodes++)
    ...
    /*--- Set the tangential boundary condition --- */

    SetTangential_BC(geometry, config, StiffMatrix_Elem, PointCorners, CoordCorners, nNodes);
...
void CVolumetricMovement::SetTangential_BC(CGeometry *geometry, CConfig *config, su2double **StiffMatrix_Elem, unsigned long PointCorners[8], su2double CoordCorners[8][3], unsigned short nNodes){
...
// PABLOs note: commented, this is already taken care of below
...
//  }
    ...
    for (iMarker = 0; iMarker < config->GetnMarker_All(); iMarker++) {
        if (((config->GetMarker_All_Moving(iMarker) == YES) && (Kind_SU2 == SU2_CFD)) ||
            ((config->GetMarker_All_DV(iMarker) == YES) && (Kind_SU2 == SU2_DEF)) ||
            ((config->GetDirectDiff() == D_DESIGN) && (Kind_SU2 == SU2_CFD) &&
        ...
        if ((config->GetMarker_All_DeformTangential(iMarker) == YES) ||
            ...
            for (iVertex = 0; iVertex < geometry->nVertex[iMarker]; iVertex++) {
                ...
                if (geometry->node[iPoint]->GetDomain()) {
                    ...
                    if (nDim == 2) {
                        LinSysRes[iPoint * nDim + 1] = 0;
                        LinSysSol[iPoint * nDim + 1] = 0;
                        StiffMatrix.DeleteValsRowi(iPoint * nDim + 1);
                    } else {
                        LinSysRes[iPoint * nDim + 2] = 0;
                        LinSysSol[iPoint * nDim + 2] = 0;

//                        LinSysRes[iPoint * nDim + 0] = 0;
//                        LinSysSol[iPoint * nDim + 0] = 0;
...
void CVolumetricMovement::Transform_DoFs(CGeometry *geometry, CConfig *config, CSysVector& vector, bool transpose){
...
void CVolumetricMovement::SetBoundaryDerivatives(CGeometry *geometry, CConfig *config) {
  ...
  if ((config->GetDirectDiff() == D_DESIGN) && (Kind_SU2 == SU2_CFD)) {
    ...
    /*--- Transform DoFs in vectors to normal and tangential coordinate system ---*/
...
void CVolumetricMovement::SetDomainDisplacements(CGeometry *geometry, CConfig *config) {
  ...
  su2double *Coord, *MinCoordValues, *MaxCoordValues, *Hold_GridFixed_Coord;
    ...
    //    PABLOs comment: this below is not implemented in turbo_ffd
