...
void CConfig::addConvectOption(const string name, unsigned short& space_field, unsigned short& centered_field,
                               unsigned short& upwind_field) {
  assert(option_map.find(name) == option_map.end());
  all_options.insert(pair<string, bool>(name, true));
...
void CConfig::SetPointersNull(void) {
  ...
  Marker_Load_Sine = nullptr;
  Marker_Clamped = nullptr;
  Marker_FlowLoad = nullptr;
  ...
  Load_Sine_Dir = nullptr;
  Load_Sine_Amplitude = nullptr;
  Load_Sine_Frequency = nullptr;
...
void CConfig::SetConfig_Options() {
  ...
  /*!\brief CP_VALUE  \n DESCRIPTION: Specific heat at constant volume, Cp (717.645 J/kg*K (air), constant density
   * incompressible fluids only) \ingroup Config*/
  ...
  /*!\brief Smluchowski/Maxwell wall boundary marker(s)  \n DESCRIPTION: Slip velocity and temperature jump wall
   boundary marker(s)
   Format: ( Heat flux marker,  wall temperature (static), momentum accomodation coefficient, thermal accomodation
   coefficient ... ) \ingroup Config*/
  ...
  ad_coeff_heat[0] = 0.5;
  ad_coeff_heat[1] = 0.02;
  /*!\brief JST_SENSOR_COEFF_HEAT \n DESCRIPTION: 2nd and 4th order artificial dissipation coefficients for the JST
   * method \ingroup Config*/
   ...
   * element_properties.dat \ingroup Config */
    ...
    if (TokenizeString(text_line, option_name, option_value)) {
      ...
      if (option_map.find(option_name) == option_map.end()) {
        ...
        if (!option_name.compare("RELAXATION_FACTOR_ADJFLOW"))
          newString.append(
              "Option RELAXATION_FACTOR_ADJFLOW is now RELAXATION_FACTOR_ADJOINT, "
          ...
          newString.append(
              "WRT_BINARY_RESTART is deprecated. The type of restart is determined from the OUTPUT_FILES list.\n\n");
          ...
          newString.append(
              "WRT_CON_FREQ is deprecated. Use SCREEN_WRT_FREQ_INNER or SCREEN_WRT_FREQ_OUTER for multizone cases "
              "instead.\n\n");
          ...
          newString.append(
              "CONV_CRITERIA is deprecated. SU2 will choose the criteria automatically based on the CONV_FIELD.\n"
          ...
          newString.append(
              "THERMAL_DIFFUSIVITY_SOLID is deprecated. Set THERMAL_CONDUCTIVITY_CONSTANT, MATERIAL_DENSITY and "
              "SPECIFIC_HEAT_CP instead.\n\n");
  ...
  if ((iZone == 0) && (rank == MASTER_NODE)) {
    ...
    switch (val_software) {
      case SU2_COMPONENT::SU2_CFD:
        cout << "|   |___/\\___//___|   Suite (Computational Fluid Dynamics Code)         |" << endl;
        break;
      case SU2_COMPONENT::SU2_DEF:
        cout << "|   |___/\\___//___|   Suite (Mesh Deformation Code)                     |" << endl;
        break;
      case SU2_COMPONENT::SU2_DOT:
        cout << "|   |___/\\___//___|   Suite (Gradient Projection Code)                  |" << endl;
        break;
      case SU2_COMPONENT::SU2_GEO:
        cout << "|   |___/\\___//___|   Suite (Geometry Definition Code)                  |" << endl;
        break;
      case SU2_COMPONENT::SU2_SOL:
        cout << "|   |___/\\___//___|   Suite (Solution Exporting Code)                   |" << endl;
        break;
    }
...
void CConfig::SetPostprocessing(SU2_COMPONENT val_software, unsigned short val_izone, unsigned short val_nDim) {
  ...
  bool ideal_gas = ((Kind_FluidModel == STANDARD_AIR) || (Kind_FluidModel == IDEAL_GAS) ||
                    (Kind_FluidModel == INC_IDEAL_GAS) || (Kind_FluidModel == FLUID_MIXTURE) ||
  ...
  if ((!ideal_gas) && (!noneq_gas)) {
    if (nMarker_Inlet != 0) {
      SU2_MPI::Error(
          "Riemann Boundary conditions or Giles must be used for inlet and outlet with Not Ideal Compressible Fluids ",
          CURRENT_FUNCTION);
    }
    if (nMarker_Outlet != 0) {
  ...
  if (GetGrid_Movement() && (GetSurface_Movement(AEROELASTIC) || GetSurface_Movement(AEROELASTIC_RIGID_MOTION)))
  ...
  if ((nMarker_HeatFlux > 0) || (nMarker_Isothermal > 0) || (nMarker_HeatTransfer) || (nMarker_CHTInterface > 0)) {
    ...
    if (nRough_Wall == 0) {
      ...
      SU2_MPI::Error(
          "Mismatch in number of rough walls and solid walls. Number of rough walls cannot be more than solid walls.",
          CURRENT_FUNCTION);
      ...
      for (iMarker = 0; iMarker < nRough_Wall; iMarker++) temp_rough.push_back(Roughness_Height[iMarker]);
      ...
      for (iMarker = 0; iMarker < nRough_Wall; iMarker++) {
        ...
        for (jMarker = 0; jMarker < nMarker_CHTInterface; jMarker++)
          if (Marker_CHTInterface[jMarker].compare(Marker_RoughWall[iMarker]) == 0) {
            Roughness_Height[nMarker_HeatFlux + nMarker_Isothermal + nMarker_HeatTransfer + jMarker] =
                temp_rough[iMarker];
            chkRough++;
          }
      }
      ...
      for (iMarker = 0; iMarker < nWall; iMarker++)
        if (Roughness_Height[iMarker] != 0.0) Kind_Wall[iMarker] = WALL_TYPE::ROUGH;

      /*--- Check if a non solid wall marker was specified as rough. ---*/
      if (chkRough != nRough_Wall) SU2_MPI::Error("Only solid walls can be rough.", CURRENT_FUNCTION);
    }
  }
  ...
  if (DiscreteAdjoint) {
    ...
    if (Kind_SU2 == SU2_COMPONENT::SU2_CFD) {
      SU2_MPI::Error(
          string("SU2_CFD: Config option MATH_PROBLEM= DISCRETE_ADJOINT requires AD support!\n") +
  ...
  if (Kind_Species_Model != SPECIES_MODEL::NONE) {
    ...
    if (Axisymmetric)
      SU2_MPI::Error("Species transport currently not possible with axissymmetric flow.", CURRENT_FUNCTION);
...
void CConfig::SetMarkers(SU2_COMPONENT val_software) {
  unsigned short iMarker_All, iMarker_CfgFile, iMarker_Euler, iMarker_Custom, iMarker_FarField, iMarker_SymWall,
      ...
      iMarker_Load, iMarker_FlowLoad, iMarker_Internal, iMarker_Monitoring, iMarker_Designing, iMarker_GeoEval,
      iMarker_Plotting, iMarker_Analyze, iMarker_DV, iMarker_Moving, iMarker_SobolevBC, iMarker_PyCustom,
      iMarker_Supersonic_Inlet, iMarker_Supersonic_Outlet, iMarker_Clamped, iMarker_ZoneInterface, iMarker_CHTInterface,
      iMarker_Load_Dir, iMarker_Disp_Dir, iMarker_Load_Sine, iMarker_Fluid_Load, iMarker_Deform_Mesh,
      iMarker_Deform_Mesh_Sym_Plane, iMarker_ActDiskInlet, iMarker_ActDiskOutlet, iMarker_Turbomachinery,
  ...
  nMarker_CfgFile = nMarker_Euler + nMarker_FarField + nMarker_SymWall + nMarker_PerBound + nMarker_NearFieldBound +
                    ...
                    nMarker_Load + nMarker_FlowLoad + nMarker_Custom + nMarker_Damper + nMarker_Fluid_Load +
                    nMarker_Clamped + nMarker_Load_Sine + nMarker_Load_Dir + nMarker_Disp_Dir + nMarker_ActDiskInlet +
                    nMarker_ActDiskOutlet + nMarker_ZoneInterface;
...
void CConfig::SetOutput(SU2_COMPONENT val_software, unsigned short val_izone) {
  unsigned short iMarker_Euler, iMarker_Custom, iMarker_FarField, iMarker_SymWall, iMarker_PerBound,
      ...
      iMarker_EngineExhaust, iMarker_Displacement, iMarker_Damper, iMarker_Load, iMarker_FlowLoad, iMarker_Internal,
      iMarker_Monitoring, iMarker_Designing, iMarker_GeoEval, iMarker_Plotting, iMarker_Analyze, iMarker_DV, iDV_Value,
      iMarker_ZoneInterface, iMarker_PyCustom, iMarker_Load_Dir, iMarker_Disp_Dir, iMarker_Load_Sine, iMarker_Clamped,
      iMarker_Moving, iMarker_Supersonic_Inlet, iMarker_Supersonic_Outlet, iMarker_ActDiskInlet, iMarker_Emissivity,
      ...
      case MAIN_SOLVER::NEMO_EULER:
        ...
        if (Kind_FluidModel == SU2_NONEQ) {
          if ((GasModel != "N2") && (GasModel != "AIR-5") && (GasModel != "AIR-7") && (GasModel != "ARGON"))
            SU2_MPI::Error(
                "The GAS_MODEL given as input is not valid. Choose one of the options: N2, AIR-5, AIR-7, ARGON.",
                CURRENT_FUNCTION);
        }
        break;
      case MAIN_SOLVER::NEMO_NAVIER_STOKES:
        ...
        if (Kind_FluidModel == SU2_NONEQ) {
          if ((GasModel != "N2") && (GasModel != "AIR-5") && (GasModel != "ARGON"))
            SU2_MPI::Error("The GAS_MODEL given as input is not valid. Choose one of the options: N2, AIR-5, ARGON.",
                           CURRENT_FUNCTION);
        }
        break;
    ...
    for (unsigned short iDV = 0; iDV < nDV; iDV++) {
      if ((Design_Variable[iDV] != NO_DEFORMATION) && (Design_Variable[iDV] != FFD_SETTING) &&
        ...
        if ((Design_Variable[iDV] == TRANSLATION) || (Design_Variable[iDV] == NACA_4DIGITS) ||
            ...
            (Design_Variable[iDV] == FFD_CAMBER) || (Design_Variable[iDV] == FFD_TWIST_2D) ||
        ...
        for (unsigned short iParamDV = 0; iParamDV < nParamDV; iParamDV++) {
          ...
          if ((iParamDV == 0) &&
              ((Design_Variable[iDV] == NO_DEFORMATION) || (Design_Variable[iDV] == FFD_SETTING) ||
               ...
               (Design_Variable[iDV] == FFD_TWIST_2D) || (Design_Variable[iDV] == FFD_CONTROL_POINT) ||
               (Design_Variable[iDV] == FFD_NACELLE) || (Design_Variable[iDV] == FFD_GULL) ||
               (Design_Variable[iDV] == FFD_TWIST) || (Design_Variable[iDV] == FFD_ROTATION) ||
               (Design_Variable[iDV] == FFD_CONTROL_SURFACE) || (Design_Variable[iDV] == FFD_CAMBER) ||
               (Design_Variable[iDV] == FFD_THICKNESS)))
            cout << FFDTag[iDV];
          else
...
void CConfig::SetKind_ConvNumScheme(unsigned short val_kind_convnumscheme, unsigned short val_kind_centered,
                                    unsigned short val_kind_upwind, LIMITER val_kind_slopelimit, bool val_muscl,
