...
CPrimalGrid::~CPrimalGrid() {
	...
	//	for (iNode = 0; iNode < nNodesFace[iFace]; iNode++)
	//		if (Coord_FaceElems_CG[iFace] != NULL) delete [] Coord_FaceElems_CG[iFace];
...
void CVertexMPI :: Change_Orientation(void) { cout << "Cambio de orientacion no definido" << endl;}	
...
CTriangle::CTriangle(unsigned long val_iPoint, unsigned long val_jPoint, 
	...
	for (unsigned short iNeighbor_Elements = 0; iNeighbor_Elements<nNeighbor_Elements; iNeighbor_Elements++) {
...
CPyramid::CPyramid(unsigned long val_iPoint, unsigned long val_jPoint, 
	...
	for (unsigned short iDim=0; iDim<nDim; iDim++)
		Coord_CG[iDim]=0.0;
	Coord_FaceElems_CG = new double* [nFaces];
	for (unsigned short iFace=0; iFace<nFaces; iFace++) {
		Coord_FaceElems_CG[iFace] = new double [nDim];
		for (unsigned short iDim=0; iDim<nDim; iDim++)
			Coord_FaceElems_CG[iFace][iDim]=0.0;
	}
...
void CPyramid :: Change_Orientation(void) { cout << "Cambio de orientacion no definido" << endl; }
