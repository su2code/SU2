...
// Ways to flatten a matrix into an array, contiguous rows or contiguous columns respectively.
enum class StorageType {RowMajor=0, ColumnMajor=1};

// Special value to indicate a dynamic size
enum SizeType : size_t {DynamicSize=0};


// "Private" namespace for helper classes and functions used by the container class.
namespace container_helpers
{
// Base accessor class and version of template for both sizes known at compile time.
...
// Specialization for compile-time number of rows.
    ...
    public container_helpers::AccessorImpl<Index_t,Scalar_t,Store,AlignSize,StaticRows,StaticCols>
{
  ...
  private:
    size_t m_resize(Index_t rows, Index_t cols) noexcept
    {
        ...
        // compare with current dimensions to determine if deallocation is needed
        // also makes the container safe against self assignment
  ...
  public:
    ...
    // For matrices size1 is rows and size2 columns,
    // for vectors size1 is lenght and size2 is ignored.
    ...
    // For static specializations these will work as copy construct/assign,
    // for dynamic see notes on CUSTOM_CTOR_AND_DTOR macro.
    ...
    // Providing an interface that allowed resizing with value could cause
