/*!
 * \file CFEMStandardTriAdjacentPyraSol.hpp
 * \brief Class for the FEM standard surface triangle element
 *        adjacent to a pyramid for the solution.
 *        The functions are in the <i>CFEMStandardTriAdjacentPyraSol.cpp</i> file.
...
/*!
 * \class CFEMStandardTriAdjacentPyraSol.
 ...
 * \version 7.1.1 "Blackbird"
 */
class CFEMStandardTriAdjacentPyraSol final: public CFEMStandardPyraBase {
...
public:
   ...
   */
  CFEMStandardTriAdjacentPyraSol() = delete;
   ...
   */
  CFEMStandardTriAdjacentPyraSol(const unsigned short val_nPoly,
   ...
   */
  ~CFEMStandardTriAdjacentPyraSol() = default;
  ...
  inline unsigned short GetNSolDOFsPad(void) const override {
   const unsigned short nn   = nPoly+1;
   const unsigned short nSol = nn*(nn+1)*(2*nn+1)/6;
