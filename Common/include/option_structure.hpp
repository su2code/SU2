...
enum ENUM_SOLVER {
  ...
  POTENTIAL_FLOW = 1,   		/*!< \brief Definition of the potential flow solver. */
  ...
  ADJ_POTENTIAL_FLOW = 6,		/*!< \brief Definition of the continuous adjoint potential flow solver. */
  ...
  ADJ_ELECTRIC_POTENTIAL = 10,	        /*!< \brief Definition of the continuous adjoint electric potential solver. */
  LIN_POTENTIAL_FLOW = 11,		/*!< \brief Definition of the linear potential flow solver. */
  ...
  LIN_ELECTRIC_POTENTIAL = 14,  	/*!< \brief Definition of the linear electric potential solver. */
  ONE_SHOT_ADJ_EULER = 15,		/*!< \brief Definition of the one shot continuous adjoint Euler solver. */
  ONE_SHOT_ADJ_NAVIER_STOKES = 16,	/*!< \brief Definition of the one shot continuous adjoint for Navier-Stokes solver. */
  MULTI_SPECIES_NAVIER_STOKES = 17,	/*!< \brief Definition of the plasma solver. */
  TWO_PHASE_FLOW = 18,			/*!< \brief Definition of the Level set solver. */
  COMBUSTION = 19,			/*!< \brief Definition of the plasma solver. */
  TEMPLATE_SOLVER = 29                  /*!< \brief Definition of template solver. */
};
static const map<string, ENUM_SOLVER> Solver_Map = CCreateMap<string, ENUM_SOLVER>
    ...
    ("ADJ_POTENTIAL_FLOW", ADJ_POTENTIAL_FLOW)
    ("ADJ_EULER", ADJ_EULER)
    ("ADJ_NAVIER_STOKES", ADJ_NAVIER_STOKES)
    ("ADJ_RANS", ADJ_RANS )
    ("ADJ_ELECTRIC_POTENTIAL", ADJ_ELECTRIC_POTENTIAL)
    ("LIN_POTENTIAL_FLOW", LIN_POTENTIAL_FLOW)
    ("LIN_EULER", LIN_EULER)
    ("LIN_NAVIER_STOKES", LIN_NAVIER_STOKES)
    ("LIN_ELECTRIC_POTENTIAL", LIN_ELECTRIC_POTENTIAL)
    ("ONE_SHOT_ADJ_EULER", ONE_SHOT_ADJ_EULER)
    ("ONE_SHOT_ADJ_NAVIER_STOKES", ONE_SHOT_ADJ_NAVIER_STOKES)
    ("MULTI_SPECIES_NAVIER_STOKES", MULTI_SPECIES_NAVIER_STOKES)
...
enum RUNTIME_TYPE {
  ...
  RUNTIME_ADJELEC_SYS = 8,		/*!< \brief One-physics case, the code is solving the adjoint electronic potential equation. */
  ...
  RUNTIME_LINELEC_SYS = 11,		/*!< \brief One-physics case, the code is solving the linear electric potential equation. */
  ...
  RUNTIME_COMBUSTION_SYS = 17		/*!< \brief One-physics case, the code is solving the flow equations(Euler and Navier-Stokes). */
};
...
const int ADJELEC_SOL = 1;	/*!< \brief Position of the continuous adjoint electronic potential solution in the solution container array. */
const int LINFLOW_SOL = 1;	/*!< \brief Position of the linear flow solution in the solution container array. */
const int LINELEC_SOL = 1;	/*!< \brief Position of the linear electronic potential solution in the solution container array. */
const int EIKONAL_SOL = 4;	/*!< \brief Position of the Eikonal (distance computation) solution in the solution container array. */
const int PLASMA_SOL = 1;	/*!< \brief Position of the plasma solution in the solution container array. */
const int LEVELSET_SOL = 1;	/*!< \brief Position of the level set solution in the solution container array. */
const int COMBUSTION_SOL = 1;	/*!< \brief Position of the level set solution in the solution container array. */
...
enum BC_TYPE {
  ...
  ROT_PERIODIC_BOUNDARY = 11,	/*!< \brief Rotational periodic boundary definition. */
...
enum ENUM_ADAPT {
  ...
  TWOPHASE = 16,			/*!< \brief Do a grid refinement on the two phase interphase. */
...
enum ENUM_DEFORM_SOLVER {
  ...
  CONJUGATE_GRADIENT = 5,	/*!< \brief Conjugate gradient method for grid deformation. */
  PREC_CONJUGATE_GRADIENT = 6,	/*!< \brief Preconditionated conjugate gradient method for grid deformation. */
  FLEXIBLE_GMRES = 7    	/*!< \brief Flexible Generalized Minimal RESidual method. */
};
static const map<string, ENUM_DEFORM_SOLVER> Deform_Solver_Map = CCreateMap<string, ENUM_DEFORM_SOLVER>
    ("STEEPEST_DESCENT", STEEPEST_DESCENT)
    ("NEWTON", NEWTON)
    ("QUASI_NEWTON", QUASI_NEWTON)
    ("SYM_GAUSS_SEIDEL", SYM_GAUSS_SEIDEL)
    ("CONJUGATE_GRADIENT", CONJUGATE_GRADIENT)
    ("PREC_CONJUGATE_GRADIENT", PREC_CONJUGATE_GRADIENT)
...
enum ENUM_UNSTEADY {
  ...
  DUAL_TIME_STEPPING = 2,	/*!< \brief Use a dual time stepping strategy for unsteady computations. */
  ROTATIONAL_FRAME = 3    	/*!< \brief Use a rotational source term. */
};
static const map<string, ENUM_UNSTEADY> Unsteady_Map = CCreateMap<string, ENUM_UNSTEADY>
    ("NO", STEADY)
    ("TIME_STEPPING", TIME_STEPPING)
    ("DUAL_TIME_STEPPING", DUAL_TIME_STEPPING)
...
class CMathProblemRef : public CAnyOptionRef {
 ...
 public:
  ...
  void SetValue(const vector<string> & value) {
    ...
    if (value[0] == "ONE_SHOT_ADJOINT") { 
      *OneShot_ = true; *Restart_Flow_ = false;
    }
