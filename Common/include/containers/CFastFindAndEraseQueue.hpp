 ...
 */
template<typename ItemType_ = unsigned long,
         ItemType_ ErasedValue_ = std::numeric_limits<ItemType_>::max(),
...
public:
  ...
  ItemType front() const {
    for (auto value : items) if (value != ErasedValue) return value;
