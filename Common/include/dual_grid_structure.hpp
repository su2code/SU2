...
private:
	...
	vector<unsigned long> Point;	/*!< \brief Points surronding the central node of the control volume. */
	...
	Volume_n,	/*!< \brief Volume of the problem at time n. */
	Volume_n1;	/*!< \brief Volume of the problem at time n-1. */
	...
	Boundary_Physical;	/*!< \brief To see if a point belong to the boundary that is going to be received (MPI). */
	...
	*Coord_sum;		/*!< \brief Sumatory coordinates vector for geometry smoothing. */
	...
	bool Agglomerate_Indirect;					/*!< \brief This flag indicates if the indirtect points can be agglomerated. */
...
public:
	 ...
	 */
	CPoint(unsigned short val_nDim, CConfig *config);
	...
	/*! 
	 * \brief Set if a point belong to the physical boundary (do not include send-receive boundaries).
	 * \param[in] val_boundary_physical - <code>TRUE</code> if the point belong to the boundary; otherwise <code>FALSE</code>.
	 */		
	void SetBoundary_Physical(bool val_boundary_physical);
	
	/*! 
	 * \brief Provides information about if a point belong to the physical boundary.
	 * \return <code>TRUE</code> if the point belong to the physical boundary; otherwise <code>FALSE</code>.
	 */		
	bool GetBoundary_Physical(void);
...
private:
	...
	double ParCoord[3];		/*!< \brief Vertex parametrics coordinates for ffd. */
...
public:
	...
	/*! 
	 * \brief Set the value of an auxiliar variable for gradient computation.
	...
	/*! 
	 * \brief Set the value of the parametric coordinate for the vertex.
	 * \param[in] val_coord - Value of the parametric coordinate.
	 */
	void SetParCoord(double *val_coord);
	
	/*! 
	 * \brief Get the value of the parametric coordinate for the vertex.
	 * \return Value of the parametric coordinate of the vertex.
	 */
	double *GetParCoord(void);
	
	/*! 
	 * \brief Set the chunk in which the vertex is.
	 * \param[in] val_chunk - Value of the chunk where the vertex is.
	 */
	void SetChunk(short val_chunk);
	
	/*! 
	 * \brief Get the value of the chunk where the vertex is.
	 * \return Value of the chunk where the vertex is.
	 */
	short GetChunk(void);
	 ...
	 */
	void SetPeriodicPoint(long val_periodicpoint);
