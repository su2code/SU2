 ...
 * \author R. Sanchez, T. Albring, F. Palacios, T. Economon.
 * \version 5.0.0 "Raven"
 */
...
public:
  ...
  /*!
   * \brief Get information on the clamped marker for the mesh deformation.
   * \param[in] config - Definition of the particular problem.
   * \param[in] val_marker - The marker identifier.
   * \return Return true if the the marker should be fixed.
   */
  bool GetClamped_Boundary(CConfig* config, unsigned short val_marker);

  /*!
   * \brief Get information on the moving marker for the mesh deformation.
   * \param[in] config - Definition of the particular problem.
   * \param[in] val_marker - The marker identifier.
   * \return Return true if the the marker is moving.
   */
  bool GetMoving_Boundary(CConfig* config, unsigned short val_marker);

  su2double Get_Tangent2D(su2double* Normal, su2double* t1);

  su2double Get_Tangent3D(su2double* Normal, su2double* t1, su2double* t2);

  void SetTangential_BC(unsigned long* indexNode, CElement *elem, CGeometry* geometry, CConfig *config);

  void Transform_DoFs(CGeometry *geometry, CConfig *config, CSysVector& vector, unsigned short val_marker, bool transpose);

  bool GetContraintMoving_Boundary(CConfig* config, unsigned short val_marker);

  void SetConstraintMoving_Boundary(CGeometry *geometry, CConfig *config, unsigned short val_marker);
  /*!
