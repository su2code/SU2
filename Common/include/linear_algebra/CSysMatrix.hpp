...
#undef GET_BLOCK_VIEW_IMPL
  ...
  /*!
   * \brief SIMD version, sets the four blocks of an edge for multiple edges at once.
   * \note Nothing is updated if the mask is 0. As with the scalar overload, the diagonal
   *       blocks are accumulated and the off-diagonal blocks are set.
   */
  template <class MatTypeSIMD, size_t N, class I, class F = ScalarType>
  FORCEINLINE void SetBlocks(simd::Array<I, N> iEdge, simd::Array<I, N> iPoint, simd::Array<I, N> jPoint,
                             const MatTypeSIMD& jac_ii, const MatTypeSIMD& jac_ij, const MatTypeSIMD& jac_ji,
                             const MatTypeSIMD& jac_jj, simd::Array<F, N> mask = 1) {
    static_assert(MatTypeSIMD::StaticSize, "This method requires static size blocks.");
    static_assert(MatTypeSIMD::IsRowMajor, "Block storage is not compatible with matrix.");
    /*--- Read through (iVar, jVar) rather than a flat MatTypeSIMD::StaticSize/StaticNRows walk:
     * EdgeResidual's Jacobian blocks are floored to a minimum static size of 2 (see its own
     * comment), so a static nVar 1 model still hands in a 2x2 block here and blkSz must come
     * from the matrix's own (runtime) nVar/nEqn, not from the possibly padded block type. ---*/
    const auto blkSz = nVar * nEqn;
    assert(blkSz <= MatTypeSIMD::StaticSize);

    /*--- "Transpose" the four blocks, scale, and possibly convert types,
     * giving the compiler the chance to vectorize all of these. ---*/
    ScalarType blk_ii[N][MAXNVAR * MAXNVAR], blk_ij[N][MAXNVAR * MAXNVAR];
    ScalarType blk_ji[N][MAXNVAR * MAXNVAR], blk_jj[N][MAXNVAR * MAXNVAR];

    unsigned long offset = 0;
    for (auto iVar = 0ul; iVar < nVar; ++iVar) {
      for (auto jVar = 0ul; jVar < nEqn; ++jVar, ++offset) {
        SU2_OMP_SIMD_IF_NOT_AD
        for (size_t k = 0; k < N; ++k) {
          blk_ii[k][offset] = PassiveAssign(mask[k] * jac_ii(iVar, jVar)[k]);
          blk_ij[k][offset] = PassiveAssign(mask[k] * jac_ij(iVar, jVar)[k]);
          blk_ji[k][offset] = PassiveAssign(mask[k] * jac_ji(iVar, jVar)[k]);
          blk_jj[k][offset] = PassiveAssign(mask[k] * jac_jj(iVar, jVar)[k]);
        }
      }
    }
    ...
    for (size_t k = 0; k < N; ++k) {
      ...
      if (quantized_mode) {
        ...
        EncodeQuantBlock([&, k](unsigned long r, unsigned long c) { return blk_ij[k][r * nVar + c]; },
                         &q_scale.u[iEdge[k] * nVar], &q_blocks.u[iEdge[k] * blkSz], nVar);
        const auto k_l = edge_ptr_l[iEdge[k]];
        EncodeQuantBlock([&, k](unsigned long r, unsigned long c) { return blk_ji[k][r * nVar + c]; },
                         &q_scale.l[k_l * nVar], &q_blocks.l[k_l * blkSz], nVar);
      } else {
        auto bij = &mat.u[iEdge[k] * blkSz];
        auto bji = &mat.l[edge_ptr_l[iEdge[k]] * blkSz];
  ...
  /*!
   * \brief SIMD version, sets the off-diagonal blocks of an edge for multiple edges at once.
   */
  template <class MatTypeSIMD, size_t N, class I, class F = ScalarType>
  FORCEINLINE void SetOffDiagBlocks(simd::Array<I, N> iEdge, const MatTypeSIMD& jac_ij, const MatTypeSIMD& jac_ji,
                                    simd::Array<F, N> mask = 1) {
    static_assert(MatTypeSIMD::StaticSize, "This method requires static size blocks.");
    static_assert(MatTypeSIMD::IsRowMajor, "Block storage is not compatible with matrix.");
    /*--- See the note in the four-block SetBlocks above about reading via (iVar, jVar). ---*/
    const auto blkSz = nVar * nEqn;
    assert(blkSz <= MatTypeSIMD::StaticSize);

    ScalarType blk_ij[N][MAXNVAR * MAXNVAR], blk_ji[N][MAXNVAR * MAXNVAR];
