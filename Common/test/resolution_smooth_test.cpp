...
void WriteQuadMeshFile () {
  /*--- Local variables ---*/
    ...
    /*--- Open .su2 grid file ---*/
    Mesh_File.open("smoothtestgrid.su2", ios::out);
...
int main() {
  ...
  CConfig* config = new CConfig("smoothtest.cfg", SU2_CFD, 0, 1, 2, VERB_NONE);
  ...
  geometry->SmoothResolutionTensor();
  ...
  for (iPoint = 0; iPoint < geometry->GetnPointDomain(); iPoint++) {
    ...
    if (Mij[0][0] != 0.9 ) entries_correct = false;
    if (Mij[0][1] != 0.0 ) entries_correct = false;
    if (Mij[1][0] != 0.0 ) entries_correct = false;
    if (Mij[1][1] != 0.9 ) entries_correct = false;
