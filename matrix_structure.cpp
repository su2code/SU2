...
#include "../include/Diag_Upper.hpp"
...
void CSysMatrix::MatrixVectorProduct(const CSysVector & vec, CSysVector & prod, CGeometry *geometry, CConfig *config) {
  ...
  switch (nVar) {
  case 1: 
       MVP1(nPointDomain,row_ptr,col_ind, prod, matrix, vec);
       break;
  case 2: 
       MVP2(nPointDomain,row_ptr,col_ind, prod, matrix, vec);
       break;
  case 3: 
       MVP3(nPointDomain,row_ptr,col_ind, prod, matrix, vec);
       break;
  case 4: 
       MVP4(nPointDomain,row_ptr,col_ind, prod, matrix, vec);
       break;
  case 5: 
       MVP5(nPointDomain,row_ptr,col_ind, prod, matrix, vec);
       break;
...
void CSysMatrix::ComputeLU_SGSPreconditioner(const CSysVector & vec, CSysVector & prod, CGeometry *geometry, CConfig *config) {
  ...
  for (iPoint = nPointDomain-1; (int)iPoint >= 0; iPoint--) {
    // Compute aux_vector = D.x*-U.x_(n+1)
    switch(nVar) 
    {
     case 1:
     	Diag_Upper1((int)nVar,iPoint,vp,matrix,aux_vector,row_ptr,col_ind);
 	break;
     case 2:
     	Diag_Upper2((int)nVar,iPoint,vp,matrix,aux_vector,row_ptr,col_ind);
 	break;
     case 3:
     	Diag_Upper3((int)nVar,iPoint,vp,matrix,aux_vector,row_ptr,col_ind);
 	break;
     case 4:
     	Diag_Upper4((int)nVar,iPoint,vp,matrix,aux_vector,row_ptr,col_ind);
 	break;
     case 5:
	Diag_Upper5((int)nVar,iPoint,vp,matrix,aux_vector,row_ptr,col_ind);
 	break;
    default:
