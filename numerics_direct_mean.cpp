...
#include "../include/GetPMatrix.hpp"
...
CUpwRoe_Flow::CUpwRoe_Flow(unsigned short val_nDim, unsigned short val_nVar, CConfig *config) : CNumerics(val_nDim, val_nVar, config) {
  ...
  Lambda = new su2double [nVar];
  Epsilon = new su2double [nVar];
  P_Tensor = new su2double [nVar*nVar];
  invP_Tensor = new su2double [nVar*nVar];
  Proj_ModJac_Tensor_ij = new su2double [nVar*nVar];
}
...
void CUpwRoe_Flow::ComputeResidual(su2double *val_residual, su2double **val_Jacobian_i, su2double **val_Jacobian_j, CConfig *config) {
  ...
  RoeSoundSpeed2 = Gamma_Minus_One*(RoeEnthalpy-0.5*sq_vel);
  ...
  GetInviscidProjFlux_R(&Density_i, Velocity_i, &Pressure_i, &Enthalpy_i, Normal, ProjFlux_i,nDim);
  
  /*--- Compute ProjFlux_j ---*/
  
  GetInviscidProjFlux_R(&Density_j, Velocity_j, &Pressure_j, &Enthalpy_j, Normal, ProjFlux_j,nDim);
  ...
  GetPMatrix_R(&RoeDensity, RoeVelocity, &RoeSoundSpeed, UnitNormal, P_Tensor, 
 	sq_vel,Gamma_Minus_One,nDim,nVar);
  
  /*--- Compute inverse P ---*/
  
  GetPMatrix_inv_R(&RoeDensity, RoeVelocity, &RoeSoundSpeed, UnitNormal, 
	invP_Tensor,sq_vel,Gamma_Minus_One,nDim,nVar);
  ...
  for (iVar = 0; iVar < nVar; iVar++) {
    for (jVar = 0; jVar < nVar; jVar++) {
      ...
      /*--- Compute |Proj_ModJac_Tensor| = P x |Lambda| x inverse P ---*/
      for (kVar = 0; kVar < nVar; kVar++)
        tmp += P_Tensor[iVar*nVar+kVar]*Lambda[kVar]*invP_Tensor[kVar*nVar+jVar];
      
    Proj_ModJac_Tensor_ij[iVar*nVar+jVar] = tmp;
    }
  ...
  if (implicit) {
    GetInviscidProjJac_R(Velocity_i, &Energy_i, Normal, kappa, val_Jacobian_i,Gamma,(int)nDim);
    GetInviscidProjJac_R(Velocity_j, &Energy_j, Normal, kappa, val_Jacobian_j,Gamma,(int)nDim);
  }
  ...
  for (iVar = 0; iVar < nVar; iVar++) {
    ...
    tmp = kappa*(ProjFlux_i[iVar]+ProjFlux_j[iVar]);
    for (jVar = 0; jVar < nVar; jVar++) {
      tmp -= kappaAr*Proj_ModJac_Tensor_ij[iVar*nVar+jVar]*Diff_U[jVar];
      if (implicit) {
        val_Jacobian_i[iVar][jVar] += kappaAr*Proj_ModJac_Tensor_ij[iVar*nVar+jVar];
        val_Jacobian_j[iVar][jVar] -= kappaAr*Proj_ModJac_Tensor_ij[iVar*nVar+jVar];
      }
    }
    val_residual[iVar] = tmp;
  }
