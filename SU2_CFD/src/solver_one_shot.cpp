...
void COneShotSolver::CalculateAlphaBetaGamma(CConfig *config, su2double val_bcheck_norm){
  ...
  su2double beta  = 2.;
  su2double gamma = 2./(beta*val_bcheck_norm+config->GetBCheckEpsilon());
  // su2double alpha = sqrt(2.*theta/((1.-rho)*(1.-rho)) * config->GetOneShotAlpha());
  // su2double beta  = sqrt(2./theta * config->GetOneShotBeta());
  // su2double gamma = sqrt(2./(beta*val_bcheck_norm+config->GetBCheckEpsilon()) * config->GetOneShotGamma());
