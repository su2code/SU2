...
void COneShotSolver::CalculateRhoTheta(CConfig *config){
  ...
  rho   = min(max(sqrt(normDeltaNew)/sqrt(normDelta), 0.1*rho_old), 1.0-1.0E-3); // Upper bound 1-(1E-6)
  theta = max(max(sqrt(fabs(helper)/normDelta), 0.1*theta_old), 1.0E-3); // Lower bound 1E-6
  // rho   = min(sqrt(normDeltaNew)/sqrt(normDelta), 1.0-1.0E-3); // Upper bound 1-(1E-6)
  // theta = max(sqrt(fabs(helper)/normDelta), 1.0E-3); // Lower bound 1E-6
...
void COneShotSolver::CalculateAlphaBeta(CConfig *config){
  ...
  alpha = max(min(alpha, 1.0E5), 1.0);
  beta = max(min(beta, 1.0E5), 1.0);
...
void COneShotSolver::CalculateGamma(CConfig *config, su2double val_bcheck_norm, su2double* val_constr_func, su2double* val_lambda){
  ...
  for(iConstr = 0; iConstr < nConstr; iConstr++) {
    su2double gamma = 2.0/val_bcheck_norm;
