...
void COneShotSolver::CalculateRhoTheta(CConfig *config){
  ...
  rho   = min(max(sqrt(normDeltaNew)/sqrt(normDelta), 0.9*rho_old), 1.0-1.0E-8); // Upper bound 1-(1E-8)
  // theta = min(max(sqrt(fabs(helper)/normDelta*theta_old), 0.9*theta_old), theta_old/0.9);
  theta = max(max(sqrt(fabs(helper)/normDelta*theta_old), 0.9*theta_old), 1.0E-8); // Lower bound 1E-8
...
void COneShotSolver::CalculateGamma(CConfig *config, su2double val_bcheck_norm, su2double* val_constr_func){
  ...
  for(iConstr = 0; iConstr < nConstr; iConstr++) {
    ...
    if((config->GetKind_ConstrFuncType(iConstr) != EQ_CONSTR) && (val_constr_func[iConstr] <= 0.0)) {
      gamma = max(min(max(gamma, config->GetOneShotGammaRate()*config->GetOneShotGamma(iConstr)), config->GetOneShotGammaMax()), 1.0E-8);
    }
    // gamma = max(min(max(gamma, config->GetOneShotGammaRate()*config->GetOneShotGamma(iConstr)), config->GetOneShotGammaMax()), 1.0E-8);
