...
void CAvgGradArtComp_Flow::SetResidual(double *val_residual, double **val_Jacobian_i, double **val_Jacobian_j, CConfig *config) {
	...
	/*--- Mean Viscosities ---*/
	Mean_Density = 0.5*(DensityInc_i + DensityInc_j);
	Mean_Laminar_Viscosity = 0.5*(Laminar_Viscosity_i + Laminar_Viscosity_j);
	Mean_Eddy_Viscosity = 0.5*(Eddy_Viscosity_i + Eddy_Viscosity_j);
	...
	/*--- Get projected flux tensor ---*/
	GetViscousArtCompProjFlux(Mean_PrimVar, Mean_GradPrimVar, Normal, Mean_Laminar_Viscosity/Mean_Density, Mean_Eddy_Viscosity/Mean_Density);
	...
	if (implicit) {
		...
		for (iDim = 0; iDim < nDim; iDim++)
			dist_ij += (Coord_j[iDim]-Coord_i[iDim])*(Coord_j[iDim]-Coord_i[iDim]);
		dist_ij = sqrt(dist_ij);
		GetViscousArtCompProjJacs(Mean_Density, Mean_PrimVar[0], Mean_Laminar_Viscosity, Mean_Eddy_Viscosity, 
											 dist_ij, UnitaryNormal, Area, Mean_PrimVar, Proj_Flux_Tensor, val_Jacobian_i, val_Jacobian_j);
	}
}

CAvgGrad_Combustion::CAvgGrad_Combustion(unsigned short val_nDim, unsigned short val_nVar, CConfig *config) : CNumerics(val_nDim, val_nVar, config) {

}

CAvgGrad_Combustion::~CAvgGrad_Combustion(void) {

}

void CAvgGrad_Combustion::SetResidual(double *val_residual, double **Jacobian_i, double **Jacobian_j, CConfig *config) {
	/*---How to compute the viscous residual ---*/
}
...
void CAvgGradCorrected_TurbSST::SetResidual(double *val_residual, double **Jacobian_i, double **Jacobian_j, CConfig *config) {
	...
	double sigma_kine, sigma_omega;
	double visc_i_kine, visc_i_omega, visc_j_kine, visc_j_omega, visc_avg_kine, visc_avg_omega;

	/*--- Compute the blended constant for the viscous terms ---*/
	sigma_kine  = F1*sigma_kine_1+(1-F1)*sigma_kine_2;
	sigma_omega = F1*sigma_omega_1+(1-F1)*sigma_omega_2;

	/*--- Compute mean effective viscosity ---*/
	visc_i_kine  = Laminar_Viscosity_i+sigma_kine*Eddy_Viscosity_i;
	visc_i_omega = Laminar_Viscosity_i+sigma_omega*Eddy_Viscosity_i;
	visc_j_kine  = Laminar_Viscosity_j+sigma_kine*Eddy_Viscosity_j;
	visc_j_omega = Laminar_Viscosity_j+sigma_omega*Eddy_Viscosity_j;

	visc_avg_kine  = 0.5*(visc_i_kine+visc_j_kine);
	visc_avg_omega = 0.5*(visc_i_omega+visc_j_omega);
	...
	val_residual[0] = visc_avg_kine*Proj_Mean_GradTurbVar_Corrected[0];
	val_residual[1] = visc_avg_omega*Proj_Mean_GradTurbVar_Corrected[1];

	/*--- For Jacobians -> Use of TSL approx. to compute derivatives of the gradients ---*/
	if (implicit) {
		Jacobian_i[0][0] = -visc_avg_kine*proj_vector_ij;		Jacobian_i[0][1] = 0.0;
		Jacobian_i[1][0] = 0.0;									Jacobian_i[1][1] = -visc_avg_omega*proj_vector_ij;

		Jacobian_j[0][0] = visc_avg_kine*proj_vector_ij;		Jacobian_j[0][1] = 0.0;
		Jacobian_j[1][0] = 0.0;									Jacobian_j[1][1] = visc_avg_omega*proj_vector_ij;
	}
...
CAvgGrad_Plasma::CAvgGrad_Plasma(unsigned short val_nDim, unsigned short val_nVar, CConfig *config) : CNumerics(val_nDim, val_nVar, config) {
	Gamma = config->GetGamma();
	Gamma_Minus_One = Gamma - 1.0;
}
