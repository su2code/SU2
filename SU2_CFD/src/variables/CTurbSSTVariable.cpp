...
CTurbSSTVariable::CTurbSSTVariable(su2double kine, su2double omega, su2double mut, unsigned long npoint, unsigned long ndim, unsigned long nvar, const su2double* constants, CConfig *config)
  ...
  if (sstParsedOptions.rc) Vort2StrainRatio.resize(nPoint) = su2double(0.0);
  if (sstParsedOptions.comp){
    Mt2.resize(nPoint) = su2double(0.0);
    F_Mt.resize(nPoint) = su2double(0.0);
    CompCorrection_k.resize(nPoint) = su2double(1.0);
    CompCorrection_w.resize(nPoint) = su2double(1.0);
  }
