...
void COutput::SetConvHistory_Body(ofstream *ConvHist_file,
  ...
  if (rank == MASTER_NODE) {
    ...
    adj_turb_resid[1000],
    begin_fem[1000], fem_coeff[1000], heat_resid[1000], combo_obj[1000],poisson_resid[1000],
	...
	bool pressure_based = config[val_iZone]->GetKind_Incomp_System() == PRESSURE_BASED;			// Pressure based solver
    ...
    switch (config[val_iZone]->GetKind_Solver()) {
      ...
      case HEAT_EQUATION_FVM:
        ...
        for (iVar = 0; iVar < nVar_Poisson; iVar++) {
          residual_poisson[iVar] = solver_container[val_iZone][val_iInst][FinestMesh][POISSON_SOL]->GetRes_RMS(iVar);
        }
...
void COutput::LoadLocalData_IncFlow(CConfig *config, CGeometry *geometry, CSolver **solver, unsigned short val_iZone) {
  ...
  nVar_First = solver[FirstIndex]->GetnVar();
  if (((!energy) && (!weakly_coupled_heat))&&(!pressure_based)) nVar_First--;
