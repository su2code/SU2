...
void COutput::MergeSolution(CConfig *config, CGeometry *geometry, CSolver **solver, unsigned short val_iZone) {
  ...
  switch (Kind_Solver) {
    case EULER : case NAVIER_STOKES: FirstIndex = FLOW_SOL; if(config->GetHeat_Inc()) SecondIndex = HEAT_SOL; else SecondIndex = NONE; ThirdIndex = NONE; break;
    case RANS : FirstIndex = FLOW_SOL; SecondIndex = TURB_SOL; if (transition) ThirdIndex=TRANS_SOL; else ThirdIndex = NONE;  if(config->GetHeat_Inc()) ThirdIndex = HEAT_SOL; else ThirdIndex = NONE; break;
...
void COutput::SetConvHistory_Header(ofstream *ConvHist_file, CConfig *config, unsigned short val_iZone) {
  ...
  char heat_coeff[]= ",\"HeatFlux_Total\",\"HeatFlux_Maximum\",\"AvgTemperature_Total\"";
...
void COutput::SetConvHistory_Body(ofstream *ConvHist_file,
  ...
  if (!disc_adj && !DualTime_Iteration) {
  ...
  if (rank == MASTER_NODE) {
    ...
    Total_CT = 0.0, Total_CQ = 0.0, Total_CWave = 0.0, Total_CHeat = 0.0, Total_HeatFluxDiff = 0.0,
    Total_Heat = 0.0, Total_MaxHeat = 0.0, Total_AvgTemperature, Total_Heat_CHT = 0.0, Total_CFEM = 0.0, Total_Custom_ObjFunc = 0.0,
    ...
    switch (config[val_iZone]->GetKind_Solver()) {
      ...
      case DISC_ADJ_EULER:          case DISC_ADJ_NAVIER_STOKES:          case DISC_ADJ_RANS:
        ...
        if (thermal && heat) {
          ...
          if (direct_diff != NO_DERIVATIVE) {
            D_Total_Custom_ObjFunc  = SU2_TYPE::GetDerivative(Total_Heat);
          }
          Total_MaxHeat           = solver_container[val_iZone][FinestMesh][FLOW_SOL]->GetTotal_MaxHeatFlux();
          Total_Heat_CHT          = Total_Heat;
          Total_AvgTemperature    = solver_container[val_iZone][FinestMesh][HEAT_SOL]->GetTotal_AvgTemperature();
        }
      ...
      if (!DualTime_Iteration) {
        ...
        switch (config[val_iZone]->GetKind_Solver()) {
          ...
          case DISC_ADJ_NAVIER_STOKES: case DISC_ADJ_RANS:
            ...
            if (thermal) SPRINTF (heat_coeff, ", %14.8e, %14.8e, %14.8e",  Total_Heat, Total_MaxHeat, Total_AvgTemperature);
            ...
            if (nDim == 2) {
              ...
              if (heat) SPRINTF (flow_resid, ", %14.8e, %14.8e, %14.8e, %14.8e, %14.8e", log10 (residual_flow[0]), log10 (residual_heat[0]), log10 (residual_flow[2]), dummy, dummy);
            }
            else {
...
void COutput::SetCFL_Number(CSolver ****solver_container, CConfig **config, unsigned short val_iZone) {
  ...
  bool heat = config[val_iZone]->GetHeat_Inc();

  switch( config[val_iZone]->GetKind_Solver()) {
    case EULER : case NAVIER_STOKES : case RANS:
      RhoRes_New = solver_container[val_iZone][FinestMesh][FLOW_SOL]->GetRes_RMS(0);
      if (heat) RhoRes_New = solver_container[val_iZone][FinestMesh][HEAT_SOL]->GetRes_RMS(0);
      break;
    case ADJ_EULER : case ADJ_NAVIER_STOKES: case ADJ_RANS:
  ...
  switch( config[val_iZone]->GetKind_Solver()) {
  case EULER : case NAVIER_STOKES : case RANS:
    RhoRes_Old[val_iZone] = solver_container[val_iZone][FinestMesh][FLOW_SOL]->GetRes_RMS(0);
    if (heat) RhoRes_Old[val_iZone] = solver_container[val_iZone][FinestMesh][HEAT_SOL]->GetRes_RMS(0);
    break;
  case ADJ_EULER : case ADJ_NAVIER_STOKES: case ADJ_RANS:
...
void COutput::LoadLocalData_Flow(CConfig *config, CGeometry *geometry, CSolver **solver, unsigned short val_iZone) {
  ...
  if (incompressible && config->GetHeat_Inc()) nVar_Par += 1;
  ...
  if (incompressible && config->GetHeat_Inc()) Variable_Names.push_back("Temperature");
  ...
  for (iPoint = 0; iPoint < geometry->GetnPoint(); iPoint++) {
    ...
    if (!Local_Halo[iPoint] || Wrt_Halo) {
      ...
      if(incompressible && config->GetHeat_Inc()) {
