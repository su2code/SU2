...
void COutput::SetSurfaceCSV_Flow(CConfig *config, CGeometry *geometry,
  ...
  switch (solver) {
    case EULER : case TWO_PHASE_EULER: SurfFlow_file <<  "\"Mach_Number\"" << "\n"; break;
    case NAVIER_STOKES: case RANS: case TWO_PHASE_NAVIER_STOKES: case TWO_PHASE_RANS:
    ...
    if (config->GetMarker_All_Plotting(iMarker) == YES)
      for (iVertex = 0; iVertex < geometry->GetnVertex(iMarker); iVertex++) {
        ...
        if (geometry->node[iPoint]->GetDomain()) {
          ...
          if (solver == EULER || solver == TWO_PHASE_EULER)
            Buffer_Send_Mach[nVertex_Surface] = sqrt(FlowSolver->node[iPoint]->GetVelocity2()) / FlowSolver->node[iPoint]->GetSoundSpeed();
          if ((solver == NAVIER_STOKES) || (solver == RANS) || (solver == TWO_PHASE_NAVIER_STOKES)|| (solver == TWO_PHASE_RANS)) {
  ...
  if (solver == EULER || solver == TWO_PHASE_EULER) SU2_MPI::Gather(Buffer_Send_Mach, MaxLocalVertex_Surface, MPI_DOUBLE, Buffer_Recv_Mach, MaxLocalVertex_Surface, MPI_DOUBLE, MASTER_NODE, MPI_COMM_WORLD);
  if ((solver == NAVIER_STOKES) || (solver == RANS) || (solver == TWO_PHASE_NAVIER_STOKES)|| (solver == TWO_PHASE_RANS)) {
...
void COutput::MergeSolution(CConfig *config, CGeometry *geometry, CSolver **solver, unsigned short val_iZone) {
  ...
  switch (Kind_Solver) {
    ...
    case TWO_PHASE_EULER : case TWO_PHASE_NAVIER_STOKES : FirstIndex = FLOW_SOL; SecondIndex = TWO_PHASE_SOL; ThirdIndex = NONE; break;
  ...
  if (!config->GetLow_MemoryOutput()) {
    ...
    if ((Kind_Solver == EULER) || (Kind_Solver == NAVIER_STOKES) || (Kind_Solver == RANS) ||
    	(Kind_Solver == TWO_PHASE_EULER) || (Kind_Solver == TWO_PHASE_NAVIER_STOKES)
...
void COutput::SetConvHistory_Header(ofstream *ConvHist_file, CConfig *config, unsigned short val_iZone) {
  char cstr[200], buffer[50], turb_resid[1000], two_phase_resid[1000];
  ...
  bool thermal = (config->GetKind_Solver() == RANS or config->GetKind_Solver()  == NAVIER_STOKES
                  or config->GetKind_Solver()  == TWO_PHASE_NAVIER_STOKES 
                  or config->GetKind_Solver()  == TWO_PHASE_RANS);
  ...
  switch (config->GetKind_2phase_Model()) {
    case HILL_RUS:     SPRINTF (two_phase_resid, ",\"Res_2phase[0]\",\"Res_2phase[1]\",\"Res_2phase[2]\", \"Res_2phase[3]\""); break;
  }
  char adj_turb_resid[]= ",\"Res_AdjTurb[0]\"";
  char adj_2phase_resid[]= ",\"Res_Adj2Phase[0]\",\"Res_Adj2Phase[3]\"";
...
void COutput::SetConvHistory_Body(ofstream *ConvHist_file,
  ...
  if (rank == MASTER_NODE) {
    ...
    char begin[1000], direct_coeff[1000], equivalent_area_coeff[1000], engine_coeff[1000], rotating_frame_coeff[1000], Cp_inverse_design[1000], Heat_inverse_design[1000], surface_coeff[1000], aeroelastic_coeff[1000], monitoring_coeff[10000],
    adjoint_coeff[1000], flow_resid[1000], adj_flow_resid[1000], turb_resid[1000], trans_resid[1000],
    adj_turb_resid[1000], adj_2phase_resid[1000], wave_coeff[1000], two_phase_resid[1000],
    ...
    bool two_phase = ((config[val_iZone]->GetKind_Solver() == TWO_PHASE_RANS) || (config[val_iZone]->GetKind_Solver() == TWO_PHASE_EULER) ||
                      (config[val_iZone]->GetKind_Solver() == TWO_PHASE_NAVIER_STOKES) ||
                      (config[val_iZone]->GetKind_Solver() == DISC_ADJ_TWO_PHASE_EULER) ||
                      (config[val_iZone]->GetKind_Solver() == DISC_ADJ_TWO_PHASE_NAVIER_STOKES) ||
	...
	(config[val_iZone]->GetKind_Solver() == TWO_PHASE_EULER) || (config[val_iZone]->GetKind_Solver() == TWO_PHASE_NAVIER_STOKES) ||
    ...
    *Surface_CMz        = NULL;
    ...
    unsigned short nVar_Flow = 0, nVar_Turb = 0, nVar_2phase = 0,
    nVar_Trans = 0, nVar_Wave = 0, nVar_Heat = 0,
    nVar_AdjFlow = 0, nVar_AdjTurb = 0, nVar_Adj2phase = 0,
    nVar_FEM = 0;
    ...
    if (two_phase) {
      switch (config[val_iZone]->GetKind_2phase_Model()) {
        case HILL_RUS:     nVar_2phase = 4; break;      }
    }
    ...
    residual_adjtwophase  = new su2double[nVar_Adj2phase];
    ...
    if (  (!fem && ((In_NoDualTime || In_DualTime_0 || In_DualTime_1) && (In_NoDualTime || In_DualTime_2 || In_DualTime_3))) ||
      ...
      if (val_iZone == 0){
        ...
        if (  (!fem && ((write_heads) && !(!DualTime_Iteration && Unsteady))) ||
          ...
          switch (config[val_iZone]->GetKind_Solver()) {
          case EULER :case NAVIER_STOKES: case TWO_PHASE_EULER :case TWO_PHASE_NAVIER_STOKES:
          ...
          case RANS :case TWO_PHASE_RANS :
            ...
            case DISC_ADJ_TWO_PHASE_EULER:  case DISC_ADJ_TWO_PHASE_NAVIER_STOKES:
            ...
            case ADJ_RANS : case DISC_ADJ_RANS: case DISC_ADJ_TWO_PHASE_RANS:
...
void COutput::SpecialOutput_ForcesBreakdown(CSolver ****solver, CGeometry ***geometry, CConfig **config, unsigned short val_iZone, bool output) {
  ...
  bool turbulent          = config[val_iZone]->GetKind_Solver() == RANS || config[val_iZone]->GetKind_Solver() == TWO_PHASE_RANS;
  ...
  bool two_phase          = (config[val_iZone]->GetKind_Solver() == TWO_PHASE_EULER)
					   || (config[val_iZone]->GetKind_Solver() == TWO_PHASE_NAVIER_STOKES) ||
		                       (config[val_iZone]->GetKind_Solver() == TWO_PHASE_RANS);
  ...
  unsigned short nDim = geometry[val_iZone][FinestMesh]->GetnDim();
  bool flow = ((config[val_iZone]->GetKind_Solver() == EULER) || (config[val_iZone]->GetKind_Solver() == NAVIER_STOKES) ||
               (config[val_iZone]->GetKind_Solver() == RANS) ||(config[val_iZone]->GetKind_Solver() == TWO_PHASE_EULER)
			   || (config[val_iZone]->GetKind_Solver() == TWO_PHASE_NAVIER_STOKES) ||
                       (config[val_iZone]->GetKind_Solver() == TWO_PHASE_RANS));
...
void COutput::SpecialOutput_SpanLoad(CSolver *solver, CGeometry *geometry, CConfig *config, bool output) {
  ...
  if (geometry->GetnDim() == 3) {
    ...
    for (iPoint = 0; iPoint < geometry->GetnPoint(); iPoint++) {
      CPressure[iPoint] = (solver->node[iPoint]->GetPressure() - RefPressure)*factor*RefArea;
    }
  ...
  for (iZone = 0; iZone < val_nZone; iZone++) {
    ...
    switch (config[iZone]->GetKind_Solver()) {
      case EULER : case NAVIER_STOKES: case RANS :
        ...
        break;
      case TWO_PHASE_EULER : case TWO_PHASE_NAVIER_STOKES : case TWO_PHASE_RANS :
    	LoadLocalData_2phase(config[iZone], geometry[iZone][MESH_0], solver_container[iZone][MESH_0], iZone);
    	break;
      case ADJ_EULER : case ADJ_NAVIER_STOKES : case ADJ_RANS :
      case DISC_ADJ_EULER: case DISC_ADJ_NAVIER_STOKES: case DISC_ADJ_RANS:
      case DISC_ADJ_TWO_PHASE_EULER: case DISC_ADJ_TWO_PHASE_NAVIER_STOKES: case DISC_ADJ_TWO_PHASE_RANS:
...
void COutput::LoadLocalData_2phase(CConfig *config, CGeometry *geometry, CSolver **solver, unsigned short val_iZone) {
  ...
  if (!config->GetLow_MemoryOutput()) {
    ...
    if ((Kind_Solver == TWO_PHASE_NAVIER_STOKES) || (Kind_Solver == TWO_PHASE_RANS)) {
			if (config->GetOutput_FileFormat() == PARAVIEW){
				nVar_Par += 1; Variable_Names.push_back("Laminar_Viscosity");
				nVar_Par += 2;
				Variable_Names.push_back("Skin_Friction_Coefficient_X");
...
void COutput::LoadLocalData_AdjFlow(CConfig *config, CGeometry *geometry, CSolver **solver, unsigned short val_iZone) {
  ...
  if ((Kind_Solver == DISC_ADJ_EULER)                   ||
	  ...
	  Variable_Names.push_back("Sensit.x");
	  Variable_Names.push_back("Sensit.y");
	  nVar_Par += 2;
      if (geometry->GetnDim()== 3) {
        Variable_Names.push_back("Sensit.z");
        nVar_Par +=1;
    }
  }
...
void COutput::SpecialOutput_AnalyzeSurface(CSolver *solver, CGeometry *geometry, CConfig *config, bool output) {
  ...
  unsigned short Kind_Average = config->GetKind_AverageProcess();
