...
void COutput::WriteActDisk_FanFace(CConfig *config, CGeometry *geometry, CSolver *FlowSolver) {
  ...
  if (rank == MASTER_NODE) cout << endl << "Writing Actuator Disk fan face file.";
    ...
    if (config->GetMarker_All_KindBC(iMarker) == ACTDISK_INLET) {
      ...
      for (iVertex = 0; iVertex < geometry->GetnVertex(iMarker); iVertex++) {
        ...
        if (geometry->node[iPoint]->GetDomain()) {
          ...
          dMach_dVel_z = FlowSolver->node[iPoint]->GetVelocity(2) / (Mach * sqrt(Velocity2));
  ...
  if (rank == MASTER_NODE) {
    ...
    strcpy (cstr, "ActDisk_FanFace.dat");
    SurfFlow_file.precision(15);
    SurfFlow_file.open(cstr, ios::out);
    
    SurfFlow_file <<"TITLE = \"Fan face Total Pressure\"" <<endl;
    ...
    SurfFlow_file <<"ZONE T= \"x = 0 in.\" NODES=" << nAngle*nStation << " , ELEMENTS= " << nAngle*(nStation-1) <<", DATAPACKING=POINT, ZONETYPE=FEQUADRILATERAL" << endl;
    ...
    SurfFlow_file << ", MFR " << FlowSolver->GetTotal_MFR() << ",\\" << "\\n";
    SurfFlow_file << "IDC " << FlowSolver->GetTotal_IDC()*100 << "%, IDCM " << FlowSolver->GetTotal_IDC_Mach()*100 << "%, IDR " << FlowSolver->GetTotal_IDR()*100 << "%,\\" << "\\n";
    SurfFlow_file << "DC60 " << FlowSolver->GetTotal_DC60() << ", Rev. Flow " << FlowSolver->GetTotal_ReverseFlow()*100 << "%.\"" << endl;
