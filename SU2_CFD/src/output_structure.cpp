...
void COutput::Solution_Interpolation(CSolver **solver, CGeometry *geometry,
    ...
    for (unsigned long iPoint =0; iPoint < geometry_interp->GetnPoint(); iPoint++) {
        ...
        for (unsigned short iDim=0; iDim < nDim; iDim++){
            if (abs(probe_loc[iDim])/(abs(NN[iDim]))>1e5 || abs(probe_loc[iDim])/(abs(NN[iDim]))< 1e-5)
                FoundNearestNode = 0;
        }
        
        if (!FoundNearestNode && fabs(probe_loc[0])<1e-15 && fabs(probe_loc[1])<1e-15 ){
...
//                if(abs(probe_loc[1]) < 1e-4 && abs(probe_loc[0]) > 1e-4)    probe_loc_tmp[1] = probe_loc_tmp[1] *mult_factor;
                if(abs(probe_loc[0]) < 1e-20 && abs(probe_loc[1]) < 1e-20)
                {
                    //probe_loc_tmp[0] = probe_loc_tmp[0] * mult_factor;
                    //probe_loc_tmp[1] = probe_loc_tmp[1] * mult_factor;
                    probe_loc_tmp[0] = 1e-10; probe_loc_tmp[1] = 1e-9;
                }
    ...
    for (int iProc=0; iProc < nProcs_recv; iProc++) {
        ...
        for (unsigned long iNode_proc = 0; iNode_proc < nPointIDs; iNode_proc++ ) {
            ...
            if ( done_a == 0){
                ...
                su2double *probe_check;
                probe_check = new su2double[3];
                ...
                if(Inside_check)
                    ...
                    cout << "\\\\\\\\######## probe check point is NOT inside the 2.5x2.5x2.5 cube at origin" << endl;
                delete [] probe_check;
                cout << "Inside Elem check ---- " << geometry->elem[probelem_check]->GetGlobalIndex() << endl;
                done_a =1;
            }*/
            ...
            if (config->GetExtIter()-config->GetUnst_RestartIter() == 0)
            {
                ...
                if (nDim == 3) {
                    if ( fabs(probe_loc[0] - 0.0810624) < 1e-4 || fabs(probe_loc[0] - 0.0810801) < 1e-4)
                        cout << "Searching For prob_loc[0] = " << probe_loc[0] << ", probe_loc[1] = " << probe_loc[1] << ", probe_loc[2] = " << probe_loc[2] << endl;
            ...
            //cout << "For prob_loc[0] = " << probe_loc[0] << ", probe_loc[1] = " << probe_loc[1] << ", probe_loc[2] = " << probe_loc[2] << ", located in element " << probe_elem << endl;
            ...
            if (nDim == 3){
                ...
                if (!isIn){
                    ...
                    if (fabs(probe_loc[0]-0.098579) < 1e-4 ){
                     probe_loc_tmp[0] = 0.0985798897315491029; probe_loc_tmp[1] = 0.000323067898276204954;
                    }
                    
                    if (fabs(probe_loc[0]-0.0996937) < 1e-4 ){
                        probe_loc_tmp[0] = 0.0996933362198272749; probe_loc_tmp[1] = -0.000168783709528503808;
                    }
                    
                    probe_loc_tmp[2] = probe_loc[2];
                    ...
                    if (isIn){
                        cout << "Finally located " << "Probe_loc[0] = " << probe_loc[0] << "Probe_loc[1] = " << probe_loc[1] << "Probe_loc[2] = " << probe_loc[2] << ",    parCoor[0] = " << parCoor[0] << ", parCoor[1] = " << parCoor[1] << ", parCoor[2] = " << parCoor[2] << endl;
                        for (unsigned short iNode=0; iNode < nNodes_elem; iNode++) {
                            unsigned long iPoint_loc = geometry->elem[probe_elem]->GetNode(iNode);
                            cout << "Rank " << rank << " - The probe with xcoord = " << probe_loc[0] << " and ycoord = " << probe_loc[1] <<  "is located in elem " << probe_elem << " with node numbers " << geometry->node[iPoint_loc]->GetGlobalIndex() << endl;
...
unsigned long COutput::FindProbeLocElement_fromNearestNodeElem3D(CGeometry *geometry, unsigned long pointID, su2double *probe_loc){
    /*--- Compute the total number of nodes on no-slip boundaries ---*/
    unsigned long jElem, probe_elem, elemID=0;
    ...
    unsigned short nElem_node = geometry->node[pointID]->GetnElem();
    unsigned short iFace;
    unsigned short count_intersect_min = 3;
    
    su2double pointID_Coord[3];
    pointID_Coord[0] = geometry->node[pointID]->GetCoord(0);
    pointID_Coord[1] = geometry->node[pointID]->GetCoord(1);
    pointID_Coord[2] = geometry->node[pointID]->GetCoord(2);
    
    su2double *parCoor;
    parCoor = new su2double[3];
    
    unsigned short nNodes_face = 4, nDim = 3;
    //su2double Coord_face[4][3];
    ...
    while (iter < 150)
    {
        ...
        iter += 1;
        if (iter>148){
            cout << " Did 148 iters to locate the poitn probe_loc[0] = " << probe_loc[0] << ", probe_loc[1] = " << probe_loc[1] << endl;
            cout << " NN[0] = " << geometry->node[pointID]->GetCoord(0) << ", NN[1] = " << geometry->node[pointID]->GetCoord(1) << endl;
            Inside = true;
            return newElem;
        }
...
bool COutput::IsPointInsideHex(CGeometry *geometry, su2double *probe_loc,unsigned long jElem){
    ...
    su2double min_coord[3], max_coord[3];
    ...
    for (unsigned short iDim =0; iDim < nDim; iDim++)
    {
        min_coord[iDim] = Coord_elem[0][iDim];
        max_coord[iDim] = Coord_elem[0][iDim];
    }
    ...
    for (unsigned short iFace = 0; iFace < nFaces_elem; iFace++) {
        ...
        if (Intersect1 || Intersect2)
        {
            ...
            /* Increase the count only if the probe is not located on the face */
            if (!IsPointOnFace)
                count += 1;
        }
    ...
    if (count == 0){
        for(unsigned short iDim =0; iDim < nDim; iDim++){
            for (iNode = 0; iNode < nNodes_elem; iNode++){
                if(min_coord[iDim] > Coord_elem[iNode][iDim])
                    min_coord[iDim] = Coord_elem[iNode][iDim];
                if(max_coord[iDim] < Coord_elem[iNode][iDim])
                    max_coord[iDim] = Coord_elem[iNode][iDim];
            }
            
            if ((min_coord[iDim] > probe_loc[iDim]) || (max_coord[iDim] < probe_loc[iDim]) )
                count += 1;
        }
        /*for(unsigned short iDim = 0; iDim < nDim; iDim++){
            cout << "min_coord[" << iDim << "] = " << min_coord[iDim] << endl;
            cout << "max_coord[" << iDim << "] = " << max_coord[iDim] << endl;
        }*/
    }
...
//    if(Inside)
//        cout << "probe_lco is inside probe_elem" << endl;
//    else
//        cout << "Not located inside the element" << endl;
...
bool COutput::SegmentIntersectsFace(CGeometry *geometry, su2double segpoint1[3], su2double segpoint2[3], su2double **Coord_face){
    ...
    //cout << " SegIntersectFace for iFace is " << Intersect << endl;
    return Intersect;
...
void COutput::Probe_sol(CSolver *solver, CGeometry *geometry, CConfig *config,
    ...
    /*--- Compute the total number of nodes on no-slip boundaries ---*/
    unsigned short nDim = 2;
    unsigned long jElem, iPoint, probe_elem;
    /* Total number of points in given processor */
    unsigned long nPoint_proc = geometry->GetnPoint();
    ...
    if( NodesADT.IsEmpty() ) {
        ...
        NodesADT.DetermineNearestNode(probe_loc, dist_probe,
                                      pointID, rankID);
    }
    if (rank == rankID)
    {
