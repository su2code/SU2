...
void COutput::SetSurfaceCSV_Flow(CConfig *config, CGeometry *geometry,
  ...
  su2double PressCoeff = 0.0, SkinFrictionCoeff[3], HeatFlux, T, Tve;
  ...
  su2double *Buffer_Send_T = new su2double [MaxLocalVertex_Surface];
  su2double *Buffer_Recv_T = NULL;

  su2double *Buffer_Send_Tve = new su2double [MaxLocalVertex_Surface];
  su2double *Buffer_Recv_Tve = NULL;
  ...
  if (solver == TNE2_NAVIER_STOKES){
    ...
    SU2_MPI::Gather(Buffer_Send_Tve, MaxLocalVertex_Surface, MPI_DOUBLE, Buffer_Recv_Tve, MaxLocalVertex_Surface, MPI_DOUBLE, MASTER_NODE, MPI_COMM_WORLD);
      ...
      for (iVertex = 0; iVertex < Buffer_Recv_nVertex[iProcessor]; iVertex++) {
        ...
        switch (solver) {
          ...
          case TNE2_NAVIER_STOKES:
            ...
            if (nDim == 3) SkinFrictionCoeff[2] = Buffer_Recv_SkinFriction_z[Total_Index];
            HeatFlux = Buffer_Recv_HeatTransfer[Total_Index];
            T        = Buffer_Recv_T[Total_Index];
            Tve      = Buffer_Recv_Tve[Total_Index];
            if (nDim == 2) SurfFlow_file << scientific << SkinFrictionCoeff[0] << ", " << SkinFrictionCoeff[1] << ", " << HeatFlux << ", " << T << ", " << Tve << "\n";
            if (nDim == 3) SurfFlow_file << scientific << SkinFrictionCoeff[0] << ", " << SkinFrictionCoeff[1] << ", " << SkinFrictionCoeff[2] << ", " << HeatFlux << ", " << T << ", " << Tve << "\n";
            break;
        }
...
void COutput::MergeSolution(CConfig *config, CGeometry *geometry, CSolver **solver, unsigned short val_iZone) {
  ...
  unsigned short iVar_GridVel = 0, iVar_PressCp = 0, iVar_Lam = 0, iVar_MachMean = 0, iVar_Tempv = 0,
  ...
  su2double *Aux_Frict_x = NULL, *Aux_Frict_y = NULL, *Aux_Frict_z = NULL, *Aux_Heat = NULL, *Aux_yPlus = NULL, *Aux_Sens = NULL, *Aux_Lim = NULL;
  ...
  switch (Kind_Solver) {
    ...
    case DISC_ADJ_TNE2_EULER : case DISC_ADJ_TNE2_NAVIER_STOKES : FirstIndex = ADJTNE2_SOL; SecondIndex = NONE; ThirdIndex = NONE; break;
  ...
  if (!config->GetLow_MemoryOutput()) {
    ...
    if (Kind_Solver == EULER     || Kind_Solver == NAVIER_STOKES     || Kind_Solver == RANS ||
        ...
        Kind_Solver == FEM_LES   || Kind_Solver == TNE2_EULER        || Kind_Solver == TNE2_NAVIER_STOKES) {
    ...
    if (Kind_Solver == NAVIER_STOKES     || Kind_Solver == RANS     ||
        ...
        Kind_Solver == FEM_LES           || Kind_Solver == TNE2_NAVIER_STOKES) {
      iVar_Lam = nVar_Total;
      nVar_Total += 1;
    ...
    /*--- laminar. Diffusivity, HEat Flux, &yplus to the restart file ---*/
  ...
  if (Kind_Solver == TNE2_NAVIER_STOKES) {
    Aux_Lim = new su2double[geometry->GetnPoint()];
  }
    ...
    if ((Kind_Solver == TNE2_EULER) || (Kind_Solver == TNE2_NAVIER_STOKES)) {
      ...
      for (iPoint = 0; iPoint < geometry->GetnPoint(); iPoint++) {
        ...
        if (!Local_Halo[iPoint] || Wrt_Halo) {
          ...
          Buffer_Send_Res[jPoint] = solver[TNE2_SOL]->node[iPoint]->GetTemperature();
          Buffer_Send_Vol[jPoint] = (solver[FLOW_SOL]->node[iPoint]->GetPressure() - RefPressure)*factor*RefArea;
    ...
    if ((Kind_Solver == TNE2_EULER) || (Kind_Solver == TNE2_NAVIER_STOKES)) {
      ...
      for (iPoint = 0; iPoint < geometry->GetnPoint(); iPoint++) {
        ...
        if (!Local_Halo[iPoint] || Wrt_Halo) {
          ...
          Buffer_Send_Res[jPoint] = solver[TNE2_SOL]->node[iPoint]->GetTemperature_ve();
          jPoint++;
        }
      }
    ...
    if ((Kind_Solver == TNE2_EULER) || (Kind_Solver == TNE2_NAVIER_STOKES))  {
      ...
      for (iPoint = 0; iPoint < geometry->GetnPoint(); iPoint++) {
        ...
        if (!Local_Halo[iPoint] || Wrt_Halo) {
          ...
          if (incompressible) {
            Buffer_Send_Var[jPoint] = sqrt(solver[TNE2_SOL]->node[iPoint]->GetVelocity2())*config->GetVelocity_Ref()/
            sqrt(config->GetBulk_Modulus()/(solver[TNE2_SOL]->node[iPoint]->GetDensity()*config->GetDensity_Ref()));
          }
          jPoint++;
        }
      }
    ...
    if (Kind_Solver == TNE2_NAVIER_STOKES) {
      ...
      for (iPoint = 0; iPoint < geometry->GetnPoint(); iPoint++) {
        ...
        if (!Local_Halo[iPoint] || Wrt_Halo) {
          ...
          Buffer_Send_Res[jPoint] = solver[TNE2_SOL]->node[iPoint]->GetLaminarViscosity();
      ...
      for (iMarker = 0; iMarker < config->GetnMarker_All(); iMarker++)
        if (config->GetMarker_All_Plotting(iMarker) == YES) {
          for (iVertex = 0; iVertex < geometry->nVertex[iMarker]; iVertex++) {
            iPoint = geometry->vertex[iMarker][iVertex]->GetNode();
            Aux_Frict_x[iPoint] = solver[TNE2_SOL]->GetCSkinFriction(iMarker, iVertex, 0);
            Aux_Frict_y[iPoint] = solver[TNE2_SOL]->GetCSkinFriction(iMarker, iVertex, 1);
            if (geometry->GetnDim() == 3) Aux_Frict_z[iPoint] = solver[TNE2_SOL]->GetCSkinFriction(iMarker, iVertex, 2);
          }
        }
    ...
    *Surface_CMy_Mnt = NULL, *Surface_CMz_Mnt = NULL;
    ...
    switch (Kind_Solver) {
      case EULER:
        ...
        break;
      case TNE2_EULER:
        if (Kind_Regime == COMPRESSIBLE) Breakdown_file << "Compressible Two-Temperature NonEquil. Euler equations." << "\n";
        break;
      case NAVIER_STOKES:
...
void COutput::LoadLocalData_TNE2(CConfig *config, CGeometry *geometry, CSolver **solver, unsigned short val_iZone) {
  ...
  if (!config->GetLow_MemoryOutput()) {
    ...
    if (config->GetWrt_Limiters()) {
      nVar_Par += nVar_Consv_Par;
      if (config->GetKind_GasModel() == N2){
          Variable_Names.push_back("Limiter_Density[N2]");
          Variable_Names.push_back("Limiter_Density[N]");
      }
      Variable_Names.push_back("Limiter_Momentum_x");
      Variable_Names.push_back("Limiter_Momentum_y");
      if (geometry->GetnDim() == 3) Variable_Names.push_back("Limiter_Momentum_z");
      Variable_Names.push_back("Limiter_Energy");
      Variable_Names.push_back("Limiter_Elec_Energy");
    ...
    if (config->GetWrt_Residuals()) {
      ...
      if (config->GetKind_GasModel() == N2){
          Variable_Names.push_back("Residual_Density[N2]");
          Variable_Names.push_back("Residual_Density[N]");
      }
      Variable_Names.push_back("Residual_Momentum_x");
      Variable_Names.push_back("Residual_Momentum_y");
      if (geometry->GetnDim() == 3) Variable_Names.push_back("Residual_Momentum_z");
      Variable_Names.push_back("Residual_Energy");
      Variable_Names.push_back("Residual_Elec_Energy");
    ...
    for (iMarker = 0; iMarker < config->GetnMarker_All(); iMarker++) {
      if (config->GetMarker_All_Plotting(iMarker) == YES) {
        for (iVertex = 0; iVertex < geometry->nVertex[iMarker]; iVertex++) {
          ...
          Aux_Heat[iPoint] = solver[TNE2_SOL]->GetHeatFlux(iMarker, iVertex);
          Aux_yPlus[iPoint] = solver[TNE2_SOL]->GetYPlus(iMarker, iVertex);
          if (config->GetBuffet_Monitoring() || config->GetKind_ObjFunc() == BUFFET_SENSOR) Aux_Buffet[iPoint] = solver[TNE2_SOL]->GetBuffetSensor(iMarker, iVertex);
        }
      }
    }
  }
    ...
    if (!Local_Halo[iPoint] || Wrt_Halo) {
      ...
      if (!config->GetLow_MemoryOutput()) {
        ...
        if ((Kind_Solver == TNE2_NAVIER_STOKES) || (Kind_Solver == RANS)) {
          ...
          Local_Data[jPoint][iVar] = solver[TNE2_SOL]->node[iPoint]->GetLaminarViscosity(); iVar++;
        ...
        if (config->GetKind_HybridRANSLES()!=NO_HYBRIDRANSLES){
          Local_Data[jPoint][iVar] = solver[TNE2_SOL]->node[iPoint]->GetDES_LengthScale(); iVar++;
          Local_Data[jPoint][iVar] = geometry->node[iPoint]->GetWall_Distance(); iVar++;
        }
