...
void CAdjEulerSolution::Preprocessing(CGeometry *geometry, CSolution **solution_container, CNumerics **solver, CConfig *config, unsigned short iRKStep) {
...
void CAdjEulerSolution::SetDissipation_Switch(CGeometry *geometry, CSolution **solution_container, CConfig *config) {
...
void CAdjEulerSolution::ExplicitRK_Iteration(CGeometry *geometry, CSolution **solution_container, 
	...
	for (iPoint = 0; iPoint < geometry->GetnPointDomain(); iPoint++) {
			...
			Delta = solution_container[FLOW_SOL]->node[iPoint]->GetDelta_Time() / ( Vol + EPS );
...
void CAdjEulerSolution::Inviscid_Sensitivity(CGeometry *geometry, CSolution **solution_container, CNumerics *solver, CConfig *config) {
	...
	Area, **PrimVar_Grad = NULL, *ConsPsi_Grad = NULL, ConsPsi, d_press, grad_v, Beta2,
	v_gradconspsi, UnitaryNormal[3], *RotVel = NULL, *GridVel = NULL;
...
// Not implemented for Dicrete
  ...
  // Farfield Sensitivity:

  // Contribution from flow perturbation:
...
// Mach
					...
					USens[2] = rv/Mach_Inf;
					if (nDim == 2) {
						USens[3] = Gamma*Mach_Inf*p;
					} else {
						USens[3] = rw/Mach_Inf;
						USens[4] = Gamma*Mach_Inf*p;
					}
		...
		if (geometry->node[iPoint]->GetDomain()) {
			...
			if (incompressible) {
				...
				if (config->GetKind_Adjoint() != DISCRETE) {
					...
					for (iVar = 0; iVar < nVar; iVar++)
						for (jDim = 0; jDim < nDim; jDim++)
							Jacobian_i[iVar][jDim] = dPressure[iVar]*UnitaryNormal[jDim]*Area;
					for (iVar = 0; iVar < nVar; iVar ++)
					...
					ObjFuncSource[0] = 1;
					for (iDim = 0; iDim < nDim; iDim++)
						ObjFuncSource[iDim+1] = -Gamma_Minus_One*U[iDim+1]/DensityInc;
...
void CAdjEulerSolution::BC_Inlet(CGeometry *geometry, CSolution **solution_container, CNumerics *conv_solver, CNumerics *visc_solver, CConfig *config, unsigned short val_marker) {
  ...
  double alpha, aa, bb, cc, dd, bcn, phin;
  ...
  string Marker_Tag = config->GetMarker_All_Tag(val_marker);
	...
	for (iVertex = 0; iVertex < geometry->nVertex[val_marker]; iVertex++) {
      ...
      /*--- Adjoint flow solution at the boundary ---*/
        ...
        /*--- Adjoint solution at the inlet (should this be hard-coded to length [3]? ---*/
...
//            /*--- Compute an ObjFunc sensitivity wrt density ---*/
//            Density = U_domain[0];
//            Velocity2 = 0.0;
//            for (iDim = 0; iDim < nDim; iDim++) {
//              Velocity[iDim] = U_domain[iDim+1]/Density;
//              Velocity2 += Velocity[iDim]*Velocity[iDim];
//              phiv    += Psi_domain[iDim+1]*Velocity[iDim];
//            }
//            Energy      = U_domain[nVar-1]/Density;
//            Pressure    = Gamma_Minus_One*Density*(Energy-0.5*Velocity2);
//            Sens_Rho += vn*(Psi_domain[0] + phiv + 0.5*Psi_domain[nVar-1]*Velocity2) + (Pressure/Density)*(phin + bcn);
...
void CAdjEulerSolution::BC_Outlet(CGeometry *geometry, CSolution **solution_container, CNumerics *conv_solver, CNumerics *visc_solver, CConfig *config, unsigned short val_marker) {
  ...
  unsigned long iVertex, iPoint, Point_Normal;
	...
	double Density, Energy, Riemann;
	double Vn, SoundSpeed, Mach_Exit, Vn_Exit, Ubn, a1;
...
void CAdjEulerSolution::MPI_Send_Receive(CGeometry ***geometry, CSolution ****solution_container,
	...
	for (iMarker = 0; iMarker < config[iZone]->GetnMarker_All(); iMarker++)
		if (config[iZone]->GetMarker_All_Boundary(iMarker) == SEND_RECEIVE) {
			...
			bool viscous = false;
			if (config[iZone]->GetKind_Solver() == ADJ_NAVIER_STOKES) viscous = true;
			if (config[iZone]->GetKind_Solver() == ADJ_RANS) viscous = true;
			...
			if (SendRecv > 0) {
				...
				if (config[iZone]->GetKind_ConvNumScheme() == SPACE_UPWIND) {
					for (iVertex = 0; iVertex < nVertex; iVertex++) {
						...
						Adjoint_Var = node[iPoint]->GetSolution();
						if (iMGLevel == MESH_0) Adjoint_Grad = node[iPoint]->GetGradient();
						for (iVar = 0; iVar < nVar; iVar++) {
				...
				if (config[iZone]->GetKind_ConvNumScheme() == SPACE_CENTERED) {
					...
					for (iVertex = 0; iVertex < nVertex; iVertex++) {
						...
						Adjoint_Var = node[iPoint]->GetSolution();
						if (iMGLevel == MESH_0) Adjoint_Grad = node[iPoint]->GetGradient();
						if (iMGLevel == MESH_0) Adjoint_Undivided_Laplacian = node[iPoint]->GetUnd_Lapl();
						for (iVar = 0; iVar < nVar; iVar++) {
							Buffer_Send_Psi[iVar*nVertex+iVertex] = Adjoint_Var[iVar];
							if (iMGLevel == MESH_0) {
					...
					if (iMGLevel == MESH_0) {
						...
						MPI::COMM_WORLD.Bsend(Buffer_Send_Psix, nBuffer_Vector, MPI::DOUBLE,send_to, 3);
						MPI::COMM_WORLD.Bsend(Buffer_Send_Psiy, nBuffer_Vector, MPI::DOUBLE,send_to, 4);
						if (nDim == 3) MPI::COMM_WORLD.Bsend(Buffer_Send_Psiz, nBuffer_Vector, MPI::DOUBLE,send_to, 5);												
					}
          ...
          unsigned long Point_0, Point_1, Point_2;
            ...
            if (donorZone != iZone) {
              ...
              for (iVar = 0; iVar < nVar; iVar++) {
                Buffer_Receive_Psi[iVar*nVertex+iVertex] = N_0*solution_container[donorZone][iMGLevel][ADJFLOW_SOL]->node[Point_0]->GetSolution(iVar)
                + N_1*solution_container[donorZone][iMGLevel][ADJFLOW_SOL]->node[Point_1]->GetSolution(iVar)
                + N_2*solution_container[donorZone][iMGLevel][ADJFLOW_SOL]->node[Point_2]->GetSolution(iVar);
                if (iMGLevel == MESH_0)
                  Buffer_Receive_Undivided_Laplacian[iVar*nVertex+iVertex] = N_0*solution_container[donorZone][iMGLevel][ADJFLOW_SOL]->node[Point_0]->GetUnd_Lapl(iVar)
                  + N_1*solution_container[donorZone][iMGLevel][ADJFLOW_SOL]->node[Point_1]->GetUnd_Lapl(iVar)
                  + N_2*solution_container[donorZone][iMGLevel][ADJFLOW_SOL]->node[Point_2]->GetUnd_Lapl(iVar);
              }
              if (iMGLevel == MESH_0)
                Buffer_Receive_Sensor[iVertex] = N_0*solution_container[donorZone][iMGLevel][ADJFLOW_SOL]->node[Point_0]->GetSensor()
                + N_1*solution_container[donorZone][iMGLevel][ADJFLOW_SOL]->node[Point_1]->GetSensor()
                + N_2*solution_container[donorZone][iMGLevel][ADJFLOW_SOL]->node[Point_2]->GetSensor();
              
              Adjoint_Grad = solution_container[donorZone][iMGLevel][ADJFLOW_SOL]->node[Point_0]->GetGradient();
              for (iVar = 0; iVar < nVar; iVar++) {
                Buffer_Receive_Psix[iVar*nVertex+iVertex] = N_0*Adjoint_Grad[iVar][0];
                Buffer_Receive_Psiy[iVar*nVertex+iVertex] = N_0*Adjoint_Grad[iVar][1];
                if (nDim == 3) Buffer_Receive_Psiz[iVar*nVertex+iVertex] = N_0*Adjoint_Grad[iVar][2];
              }
              Adjoint_Grad = solution_container[donorZone][iMGLevel][ADJFLOW_SOL]->node[Point_1]->GetGradient();
              for (iVar = 0; iVar < nVar; iVar++) {
                Buffer_Receive_Psix[iVar*nVertex+iVertex] += N_1*Adjoint_Grad[iVar][0];
                Buffer_Receive_Psiy[iVar*nVertex+iVertex] += N_1*Adjoint_Grad[iVar][1];
                if (nDim == 3) Buffer_Receive_Psiz[iVar*nVertex+iVertex] += N_1*Adjoint_Grad[iVar][2];
              }
              Adjoint_Grad = solution_container[donorZone][iMGLevel][ADJFLOW_SOL]->node[Point_2]->GetGradient();
              for (iVar = 0; iVar < nVar; iVar++) {
                Buffer_Receive_Psix[iVar*nVertex+iVertex] += N_2*Adjoint_Grad[iVar][0];
                Buffer_Receive_Psiy[iVar*nVertex+iVertex] += N_2*Adjoint_Grad[iVar][1];
                if (nDim == 3) Buffer_Receive_Psiz[iVar*nVertex+iVertex] += N_2*Adjoint_Grad[iVar][2];
              }
              
            } else {
              
              /*--- Get the solution from the donor (same zone) and store it in the buffer.
               This is essentially a dummy send/receive. ---*/
              for (unsigned short iMark = 0; iMark < config[iZone]->GetnMarker_All(); iMark++){
                if (config[iZone]->GetMarker_All_SendRecv(iMark)-1 == receive_from) donorMarker = iMark;
              }
              
              /*--- Get the specific donor point for this vertex ---*/
              donorPoint = geometry[iZone][iMGLevel]->vertex[donorMarker][iVertex]->GetNode();
              
              Adjoint_Var = node[donorPoint]->GetSolution();
              if (iMGLevel == MESH_0) Adjoint_Undivided_Laplacian = node[donorPoint]->GetUnd_Lapl();
              for (iVar = 0; iVar < nVar; iVar++) {
                Buffer_Receive_Psi[iVar*nVertex+iVertex] = Adjoint_Var[iVar];
                if (iMGLevel == MESH_0) Buffer_Receive_Undivided_Laplacian[iVar*nVertex+iVertex] = Adjoint_Undivided_Laplacian[iVar];
              }
              if (iMGLevel == MESH_0) Buffer_Receive_Sensor[iVertex] = node[donorPoint]->GetSensor();
            }
          }
#else
					MPI::COMM_WORLD.Recv(Buffer_Receive_Psi,nBuffer_Vector,MPI::DOUBLE,receive_from, 0);
					if (iMGLevel == MESH_0) {
						...
						MPI::COMM_WORLD.Recv(Buffer_Receive_Psix,nBuffer_Vector,MPI::DOUBLE,receive_from, 3);
						MPI::COMM_WORLD.Recv(Buffer_Receive_Psiy,nBuffer_Vector,MPI::DOUBLE,receive_from, 4);
						if (nDim == 3) MPI::COMM_WORLD.Recv(Buffer_Receive_Psiz,nBuffer_Vector,MPI::DOUBLE,receive_from, 5);
					}
#endif
...
void CAdjNSSolution::Preprocessing(CGeometry *geometry, CSolution **solution_container, CNumerics **solver, CConfig *config, unsigned short iRKStep) {
	unsigned long iPoint;
	bool implicit = (config->GetKind_TimeIntScheme_AdjFlow() == EULER_IMPLICIT);
...
void CAdjNSSolution::Viscous_Sensitivity(CGeometry *geometry, CSolution **solution_container, CNumerics *solver, CConfig *config) {
	unsigned long iVertex, iPoint;// Neigh;
	unsigned short iPos, iVar, jVar; // jPos, kPos, iNeigh
	...
	double cp, Laminar_Viscosity, heat_flux_factor;

	//double *d = NULL, *U = NULL, *Psi = NULL;
	//double Mach_Inf;
	...
	Total_Sens_Geo = 0.0;
	// Total_Sens_Mach, Total_Sens_AoA, Total_Sens_Press and Total_Sens_Temp part set already (don't zero!)
  ...
  //	double alpha, beta;
	...
	double ***grad_Jacobian_j;

	grad_Jacobian_j = new double**[nVar];
	for (iVar=0; iVar<nVar; iVar++)
		grad_Jacobian_j[iVar] = new double*[nVar];
	for (iVar=0; iVar<nVar; iVar++)
		for (jVar=0; jVar<nVar; jVar++)
			grad_Jacobian_j[iVar][jVar] = new double[nDim];
	...
	// Farfield Sensitivity: ONLY NEED EULER
...
//							}
//
//							// atm the doesn't have eddy viscosity in it!
//
//							if (nDim == 2) {
//								grad_Jacobian_j[0][0][0] = 0.0;
//
//							} else {
//								Jacobian_j[0][0][0] = 0.0;
//
//							}
//
//							if (!config->GetFrozen_Visc()) { // if viscosity not frozen
//
//								if (nDim == 2) {
//									grad_Jacobian_j[0][0][0] = 0.0;
//
...
//							}
	...
	// Need to incorporate turbulence equations Jacobian here...
//
//
////						} else if (config->GetKind_Adjoint() == DISCRETE) {
////// To be written...
...
////						}
////
////	// Solution gradient sensitivity (wrt U):
////
////
////	// Solution sensitivity wrt:
////	// Mach
////						USens[0] = 0.0;
////						USens[1] = ru/Mach_Inf;
////						USens[2] = rv/Mach_Inf;
...
////						for (iPos = 0; iPos < nVar; iPos++)
////							for (jPos = 0; jPos < nVar; jPos++)
////								for (kPos = 0; kPos < nVar; kPos++)
////									for (iDim = 0; iDim < nDim; iDim++) {
////										Sens_Temp[iMarker] += Psi[iPos]*grad_Jacobian_j[jPos][iPos][iDim]
////												*gradU_USens[jPos][iDim][kPos]*USens[kPos];
////									}
////
////					}
//				}
