...
void CSourcePieceWise_TurbSST::ComputeResidual(su2double *val_residual, su2double **val_Jacobian_i, su2double **val_Jacobian_j, CConfig *config) {
  ...
  if (dist_i > 1e-10) {
    ...
    /*--- Compute the divergence of the velocity, which appears in
    ...
    /*--- Production. If using UQ methodolgy, calculate production
          using perturbed Reynolds stress matrix. ---*/
    ...
    /*--- Limit the production terms and multiply it by the intermittency
          to account for transition. Note that only the production term
          of the kinetic energy equation is multiplied by the intermittency
