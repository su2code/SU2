...
void CSourcePieceWise_TurbSA_SALSA::ComputeResidual(su2double *val_residual, su2double **val_Jacobian_i, su2double **val_Jacobian_j, CConfig *config) {
  ...
  if (dist_i > 1e-10) {
    ...
    fv1 = Ji_3/(Ji_3+cv1_3);
    fv2 = 1.0 - Ji/(1.0+Ji*fv1);
    nu_t = TurbVar_i[0] * fv1;
    turb_ke = nu_t * Omega/sqrt(0.09);
    if (using_uq){
...
void CSourcePieceWise_TurbSA_SALSA::GetMeanRateOfStrainMatrix(su2double **S_ij)
{
...
void CSourcePieceWise_TurbSA_SALSA::SetReynoldsStressMatrix(su2double turb_ke){
...
void CSourcePieceWise_TurbSA_SALSA::SetPerturbedRSM(su2double turb_ke, CConfig *config){
...
void CSourcePieceWise_TurbSA_SALSA::SetPerturbedStrainMag(su2double turb_ke){
  unsigned short iDim, jDim;
...
CSourcePieceWise_TurbSA_RC::CSourcePieceWise_TurbSA_RC(unsigned short val_nDim, unsigned short val_nVar,
                                                 CConfig *config) : CNumerics(val_nDim, val_nVar, config) {
...
CSourcePieceWise_TurbSA_RC::~CSourcePieceWise_TurbSA_RC(void) { }

void CSourcePieceWise_TurbSA_RC::ComputeResidual(su2double *val_residual, su2double **val_Jacobian_i, su2double **val_Jacobian_j, CConfig *config) {
  ...
  if (dist_i > 1e-10) {
    ...
    /*--- Rotation Term ---*/
    /*--- TODO: The rotation rate term Omega_Prime is only used if the reference frame itself is rotating
     and all derivatives should be defined with respect to the reference frame. By now, I will define Omega_prime = 0.0 ---*/
    unsigned short jDim;
    su2double Omega_Prime = 0.0;
    su2double S_2 = 0.0, S_ij;
    su2double W_2 = 0.0, W_ij;
    for(iDim=0;iDim<nDim;++iDim){
      for(jDim=0;jDim<nDim;++jDim){
        S_ij = 0.5*(PrimVar_Grad_i[1+iDim][jDim] + PrimVar_Grad_i[1+jDim][iDim]);
        W_ij = 0.5*(PrimVar_Grad_i[1+iDim][jDim] - PrimVar_Grad_i[1+jDim][iDim] + Omega_Prime);
        S_2 += 2.0 * S_ij * S_ij;
        W_2 += 2.0 * W_ij * W_ij;
      }
    }
    su2double D_4 = pow(0.5 * (S_2 + W_2), 2.0);
