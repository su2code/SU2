...
void CNEMOCompOutput::SetVolumeOutputFields(CConfig *config){
  ...
  if (config->GetViscous()) {
    ...
    AddVolumeOutput("THERMAL_COND_TR", "Thermal_Cond_tr", "PRIMITIVE", "thermal tr");
    AddVolumeOutput("THERMAL__COND_VE", "Thermal_Cond_ve", "PRIMITIVE", "thermal vib");
...
void CNEMOCompOutput::LoadVolumeData(CConfig *config, CGeometry *geometry, CSolver **solver, unsigned long iPoint){
  ...
  if (config->GetViscous()){
    ...
    SetVolumeOutputValue("THERMAL_COND_TR", iPoint, Node_Flow->GetThermalConductivity(iPoint));
    SetVolumeOutputValue("THERMAL_COND_VE", iPoint, Node_Flow->GetThermalConductivity_ve(iPoint));
  }
