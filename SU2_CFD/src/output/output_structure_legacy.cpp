...
void COutputLegacy::SetConvHistory_Body(ofstream *ConvHist_file,
  ...
  if (!disc_adj && !cont_adj && !DualTime_Iteration) {
    ...
    if (((iExtIter % (config[val_iZone]->GetVolumeOutputFrequencies()[0] )) == 0) ||
          ...
          if (!fem) {
            if (!Unsteady && (config[val_iZone]->GetTime_Marching() != TIME_MARCHING::TIME_STEPPING)) {
              ...
              case MAIN_SOLVER::DISC_ADJ_INC_EULER: case MAIN_SOLVER::DISC_ADJ_INC_NAVIER_STOKES: case MAIN_SOLVER::DISC_ADJ_INC_RANS:
                ...
                cout << "Total Geometry Sensitivity (updated every "  << config[val_iZone]->GetVolumeOutputFrequencies()[0] << " iterations): ";
                cout.precision(4);
