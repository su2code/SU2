      ...
      /*--- New variables can be loaded to the Local_Data structure here,
       ...
       /*--- Recovered p/T for streamwise periodic BC ---*/
        if (config->GetKind_Streamwise_Periodic()) {
          ...
          Local_Data[jPoint][iVar] = solver[FLOW_SOL]->GetNodes()->GetStreamwise_Periodic_RecoveredPressure(iPoint); iVar++;
          if(energy) {
             Local_Data[jPoint][iVar] = solver[FLOW_SOL]->GetNodes()->GetStreamwise_Periodic_RecoveredTemperature(iPoint); iVar++; 
          }
      ...
      for (iVertex = 0; iVertex < geometry->nVertex[iMarker]; iVertex++) {
        ...
        if (geometry->node[iPoint]->GetDomain()) {
          ...
          if(config->GetKind_Streamwise_Periodic() != NONE)
            Pressure = solver->GetNodes()->GetStreamwise_Periodic_RecoveredPressure(iPoint);
          SoundSpeed = solver->GetNodes()->GetSoundSpeed(iPoint);
