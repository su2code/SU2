...
void CFlowIncOutput::SetVolumeOutputFields(CConfig *config){
  ...
  if (config->GetKind_Solver() == INC_NAVIER_STOKES){
  ...
  AddVolumeOutput("VOLUME_DUAL",   "Volume_Dual",   "MESH_QUALITY", "CV Sub-Volume");
...
void CFlowIncOutput::LoadVolumeData(CConfig *config, CGeometry *geometry, CSolver **solver, unsigned long iPoint){
  ...
  if (config->GetWrt_MeshQuality()) {
    ...
    SetVolumeOutputValue("VOLUME_DUAL",   iPoint, geometry->nodes->GetVolume(iPoint) + geometry->nodes->GetPeriodicVolume(iPoint));
  }
