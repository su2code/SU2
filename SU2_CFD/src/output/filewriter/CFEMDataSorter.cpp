...
void CFEMDataSorter::SortVolumetricConnectivity(CConfig *config, CGeometry *geometry, unsigned short Elem_Type) {
  ...
  for(unsigned long i=0; i<nVolElemOwned; ++i) {
    ...
    const unsigned short VTK_Type1 = volElem[i].standardElemFlow->GetVTK_SubType1();
    const unsigned short VTK_Type2 = volElem[i].standardElemFlow->GetVTK_SubType2();

    /* Only store the linear sub elements if they are of
       the current type that we are storing. */
    if(Elem_Type == VTK_Type1) nSubElem_Local += volElem[i].standardElemFlow->GetNSubElemsType1();
    if(Elem_Type == VTK_Type2) nSubElem_Local += volElem[i].standardElemFlow->GetNSubElemsType2();
  }
  ...
  for(unsigned long i=0; i<nVolElemOwned; ++i) {
    ...
    if(Elem_Type == VTK_Type1) {
      ...
      /* Get the number of sub-elements and the local connectivity of
         the sub-elements. */
      const unsigned short nSubElems     = volElem[i].standardElemFlow->GetNSubElemsType1();
      const unsigned short *connSubElems = volElem[i].standardElemFlow->GetSubConnType1();
    ...
    if(Elem_Type == VTK_Type2) {
      ...
      /* Get the number of sub-elements and the local connectivity of
         the sub-elements. */
      const unsigned short nSubElems     = volElem[i].standardElemFlow->GetNSubElemsType2();
      const unsigned short *connSubElems = volElem[i].standardElemFlow->GetSubConnType2();
