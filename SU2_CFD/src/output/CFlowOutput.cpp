    ...
    if (output.type == OperationType::L2_NORM || output.type == OperationType::L1_NORM) {
      ...
      SU2_OMP_PARALLEL {
        std::array<su2double, 2> local_integral = {0.0, 0.0};
          ...
          for (auto iPoint = 0ul; iPoint < geometry->GetnPointDomain(); ++iPoint) {
            if (!geometry->nodes->GetDomain(iPoint)) continue;
            if (output.type == OperationType::L2_NORM) local_integral[0] += pow(output.Eval(MakeFunctor(output, iPoint)),2);
            else local_integral[0] += abs(output.Eval(MakeFunctor(output, iPoint)));
//            local_integral[0] += pow(output.Eval(MakeFunctor(output, iPoint)),2) * geometry->nodes->GetVolume(iPoint);
          }
          END_SU2_OMP_FOR
