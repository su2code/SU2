...
void CFlowOutput::SetAnalyzeSurface(const CSolver* const*solver, const CGeometry *geometry, CConfig *config, bool output){
  ...
  // by pass GetNodes() for FEM solver since nodes are not declared in FEM solvers.
  const auto flow_nodes = (config->GetKind_Solver() == MAIN_SOLVER::FEM_EULER               ||
                           ...
                           config->GetKind_Solver() == MAIN_SOLVER::FEM_LES                 ||
                           config->GetKind_Solver() == MAIN_SOLVER::FEM_INC_EULER           ||
                           config->GetKind_Solver() == MAIN_SOLVER::FEM_INC_NAVIER_STOKES   ||
                           config->GetKind_Solver() == MAIN_SOLVER::FEM_INC_RANS            ||
                           config->GetKind_Solver() == MAIN_SOLVER::FEM_INC_LES             ) ? nullptr : solver[FLOW_SOL]->GetNodes();
