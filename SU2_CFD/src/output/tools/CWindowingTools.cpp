...
su2double CWindowingTools::GetWndWeight(WINDOW_FUNCTION windowId, unsigned long CurTimeIdx, unsigned long endTimeIdx){
    switch (windowId) {
...
su2double CWindowingTools::BumpWindow(unsigned long i, unsigned long endTimeIdx){
  if(i==0) return 0;
...
su2double CWindowedAverage::Get(){
  return val;
}

unsigned long CWindowedAverage::Count(){
  return count;
}
