...
void CAdjFlowIncOutput::LoadVolumeData(CConfig *config, CGeometry *geometry, CSolver **solver, unsigned long iPoint){
  ...
  if (weakly_coupled_heat){
    SetVolumeOutputValue("ADJ_HEAT", iPoint, Node_AdjHeat->GetSolution(iPoint, 0));
  }
  if (heat){
    if (nDim == 3) SetVolumeOutputValue("ADJ_HEAT", iPoint, Node_AdjFlow->GetSolution(iPoint, 4));
    else           SetVolumeOutputValue("ADJ_HEAT", iPoint, Node_AdjFlow->GetSolution(iPoint, 3));
  }
  // Turbulent 
  if (!config->GetFrozen_Visc_Disc()){         
    switch(turb_model){
