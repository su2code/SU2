...
bool COutput::SetResult_Files(CGeometry *geometry, CConfig *config, CSolver** solver_container,
  ...
  if (writeFiles){
    ...
    volumeDataSorter->SortConnectivity(config, geometry);
    catalyst_writer->Write_Data((unsigned long)GetHistoryFieldValue("TIME_ITER"), GetHistoryFieldValue("CUR_TIME"));
