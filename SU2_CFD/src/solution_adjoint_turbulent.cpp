...
CAdjTurbSolution::CAdjTurbSolution(CGeometry *geometry, CConfig *config) : CSolution() {
	...
	if (!restart || geometry->GetFinestMGLevel() == false) {	
		...
		for (iPoint = 0; iPoint < nPoint; iPoint++) {
			...
			node[iPoint] = new CAdjTurbVariable(PsiNu_Inf, nDim, nVar, nNeigh, config);
		}
	}
	else {
		unsigned long index;
    ...
    while (getline (restart_file,text_line)) {
      ...
      if (iPoint_Local >= 0) {
        ...
        node[iPoint_Local] = new CAdjTurbVariable(Solution[0], nDim, nVar, nNeigh, config);
      }
      iPoint_Global++;
    }
    ...
    for(iPoint = geometry->GetnPointDomain(); iPoint < geometry->GetnPoint(); iPoint++) {
    	nNeigh = geometry->node[iPoint_Local]->GetnNeighbor();
        node[iPoint] = new CAdjTurbVariable(Solution[0], nDim, nVar, nNeigh, config);
    }
