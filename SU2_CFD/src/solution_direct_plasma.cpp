...
//        Lambda = Mean_LaminarVisc*pow(2.0,nDim)*Area*Area/Mean_Density;
	...
	for (iMarker = 0; iMarker < geometry->GetnMarker(); iMarker++) {
      ...
      /*--- Compute closest normal neighbor ---*/
			jPoint = geometry->vertex[iMarker][iVertex]->GetClosest_Neighbor();
...
void CPlasmaSolution::Viscous_Residual(CGeometry *geometry, CSolution **solution_container, CNumerics *solver,
  ...
  unsigned short iSpecies, iDim, iVar, nVar_species, loc;
    ...
    for(iEdge = 0; iEdge < geometry->GetnEdge(); iEdge++) {
      ...
      for (iSpecies = 0; iSpecies < nSpecies; iSpecies++){
        ...
        for (iVar = 0; iVar < nVar_species; iVar++) {
          for (iDim = 0; iDim < nDim; iDim++) {
            node[iPoint]->SetGradient_Primitive(iSpecies, iVar, iDim,
                                                node[iPoint]->GetGradient_Primitive(iSpecies, iVar, iDim)*Limiter_i[loc+iVar]);
            node[jPoint]->SetGradient_Primitive(iSpecies, iVar, iDim,
                                                node[jPoint]->GetGradient_Primitive(iSpecies, iVar, iDim)*Limiter_j[loc+iVar]);
          }
        }
      }
    }*/
      ...
      IntegratedHeat = 0.0;
        ...
        CSkinFriction[iMarker][iVertex] = 0.0;
          ...
          for (iDim = 0; iDim < nDim; iDim++) UnitaryNormal[iDim] = Normal[iDim]/Area;
            ...
            CHeatTransfer[iMarker][iVertex] += (ThermalConductivity*GradTemperature + ThermalConductivity_vib*GradTemperature_vib)/(0.5*RefDensity*RefVel2);
          }
          IntegratedHeat += CHeatTransfer[iMarker][iVertex];
...
void CPlasmaSolution::SetPrimVar_Gradient_GG(CGeometry *geometry, CConfig *config) {
	...
	double *PrimVar_Vertex, PrimVar_Average,
  ...
  unsigned short maxPt;
    ...
    //maxGradT = 0.0;
      ...
      /*tmp = 0.0;
      for (iDim = 0; iDim < nDim; iDim++) {
        tmp += node[iPoint]->GetGradient_Primitive(iSpecies,0,iDim)*node[iPoint]->GetGradient_Primitive(iSpecies,0,iDim);
      }
      tmp = sqrt(tmp);
      if (tmp > maxGradT) {
        maxGradT = tmp;
 ...
 /* cout << "Max Temp Gradient: " << maxGradT << endl;
  cout << "Point: " << maxPt << endl;
  cout << "Coord (" << geometry->node[maxPt]->GetCoord()[0] << ", " << geometry->node[maxPt]->GetCoord()[1] << ", " << geometry->node[maxPt]->GetCoord()[2] << endl;
...
void CPlasmaSolution::SetPrimVar_Limiter(CGeometry *geometry, CConfig *config) {
	...
	unsigned long iEdge, iPoint, jPoint;
	unsigned short iVar, iDim, iSpecies, nVar_species, loc;
    ...
    for (iSpecies = 0; iSpecies < nSpecies; iSpecies++) {
      ...
      for (iVar = 0; iVar < nVar_species; iVar++) {
        PrimVar_i[iVar] = node[iPoint]->GetPrimVar(iSpecies,iVar);
        node[iPoint]->SetSolution(loc+iVar, PrimVar_i[iVar]);
      }
    }
    
		/*--- Compute the maximum, and minimum values for nodes i & j ---*/
		for (iVar = 0; iVar < nVar; iVar++) {
      du = (node[jPoint]->GetSolution(iVar) - node[iPoint]->GetSolution(iVar));
    ...
    case MINMOD:
      ...
      for (iEdge = 0; iEdge < geometry->GetnEdge(); iEdge++) {
        ...
        for (iVar = 0; iVar < nVar; iVar++) {
          ...
          if (iVar < (nDim+3)*nDiatomics) iSpecies = iVar / (nDim+3);
          else iSpecies = (iVar - (nDim+3)*nDiatomics) / (nDim+2)+nDiatomics;
