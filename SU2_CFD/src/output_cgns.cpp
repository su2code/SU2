...
//		if (cgns_err) cg_error_print();
	...
	if (unsteady) {
		...
		element_dims = geometry->GetnDim();		// Currently (release 2.0.9) only all-2D or all-3D zones permitted
		physical_dims = element_dims;

    /*--- write CGNS base data (one base assumed as of version 2.0.9) ---*/
...
void COutput::SetCGNS_Connectivity(CConfig *config, CGeometry *geometry, unsigned short iZone) {
  ...
  if (nProcessor > 1) base_file.erase (base_file.end()-2, base_file.end());
    ...
    /*--- Write base file ---*/
		...
		cgns_err = cg_descriptor_write("Solver Information","SU^2 version 2.0.9, Stanford University Aerospace Design Lab");
		if (cgns_err) cg_error_print();
