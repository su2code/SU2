...
CPoissonSolver::CPoissonSolver(CGeometry *geometry, CConfig *config, unsigned short iMesh)
  ...
  PseudoTimeCorr.resize(geometry->GetnEdge(),nDim) = su2double(0.0); // idk tbh
...
void CPoissonSolver::Preprocessing(CGeometry *geometry, CSolver **solver_container, CConfig *config, unsigned short iMesh,
  ...
  /*--- Set the momentum coefficients using the solution from the momentum equations ---*/
  bool periodic = (config->GetnMarker_Periodic() > 0);
  SetMomCoeff(geometry, solver_container, config, periodic, iMesh);
...
void CPoissonSolver::Postprocessing(CGeometry *geometry,
  ...
  for (iPoint = 0; iPoint < nPointDomain; iPoint++) {
    ...
    for (iDim = 0; iDim < nDim; iDim++) {
      vel_corr[iPoint][iDim] = nodes->GetMomCoeff(iPoint,iDim)*(nodes->GetGradient(iPoint,0,iDim));
  ...
  if (!periodic)  {
    ...
    if (implicit) {
      ...
      for (iPoint = 0; iPoint < nPointDomain; iPoint++) {
        ...
        /*--- Self contribution. ---*/
        for (iVar = 0; iVar < nDim; iVar++)
          Mom_Coeff[iVar] = flow_solution->Jacobian.GetBlock(iPoint,iPoint,iVar,iVar);
      

        //TODO: add neighbour contributions for simplec
    ...
    for (auto k = 0ul; k < color.size; ++k) {
      ...
      * --- GradP_f = (p_F^n - p_P^n)/ds , GradP_in = 0.5*(GradP_P^n + GradP_F^n)---*/
      ...
      for (iDim = 0; iDim < nDim; iDim++) {
        ...
        Coeff_Mom = 0.5*(nodes->GetMomCoeff(iPoint,iDim) + nodes->GetMomCoeff(jPoint,iDim));
      ...
      if (unsteady) {
        ...
        for (iDim = 0; iDim < nDim; iDim++) {
            den_i = geometry->nodes->GetVolume(iPoint)/nodes->GetMomCoeff(iPoint,iDim);
            den_j = geometry->nodes->GetVolume(jPoint)/nodes->GetMomCoeff(jPoint,iDim);
            Coeff_Mom = 0.5*(nodes->GetMomCoeff(iPoint,iDim) + nodes->GetMomCoeff(jPoint,iDim));
        }
        PsCorrFace = 0.0;
