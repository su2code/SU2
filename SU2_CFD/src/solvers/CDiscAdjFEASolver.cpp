...
void CDiscAdjFEASolver::RegisterSolution(CGeometry *geometry, CConfig *config){
  ...
  if (dynamic) {
    direct_solver->GetNodes()->Register_femSolution(input, push_index);
...
void CDiscAdjFEASolver::ExtractAdjoint_Solution(CGeometry *geometry, CConfig *config, bool CrossTerm){
  ...
  for (iPoint = 0; iPoint < nPoint; iPoint++){
    ...
    if(multizone) {
      ...
      direct_solver->GetNodes()->GetfemAdjointSolution_LocalIndex(iPoint,Solution);
    }
    else {
  ...
  if (dynamic){
    ...
    for (iPoint = 0; iPoint < nPoint; iPoint++){
      ...
      if(multizone) {
        ...
        direct_solver->GetNodes()->GetfemAdjointSolution_time_n_LocalIndex(iPoint,Solution);
      }
      else {
...
void CDiscAdjFEASolver::SetAdjoint_Output(CGeometry *geometry, CConfig *config){
  ...
  for (auto iPoint = 0ul; iPoint < nPoint; iPoint++){
    ...
    if (config->GetMultizone_Problem()) {
      direct_solver->GetNodes()->SetfemAdjointSolution_LocalIndex(iPoint,Solution);
    }
    else {
