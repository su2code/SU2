...
void CSpeciesFlameletSolver::Preprocessing(CGeometry* geometry, CSolver** solver_container, CConfig* config,
  ...
  for (auto i_point = 0u; i_point < nPoint; i_point++) {
    ...
    nodes->SetDataExtrapolation(i_point, misses);
    ...
    if (ignition) {
      ...
      if (dist < config->GetSparkRadius())
        nodes->SetScalarSource(i_point, I_PROGVAR,
                               nodes->GetScalarSources(i_point, I_PROGVAR) + config->GetSparkReactionRate());
    }
...
void CSpeciesFlameletSolver::SetInitialCondition(CGeometry** geometry, CSolver*** solver_container, CConfig* config,
  ...
  if ((!Restart) && ExtIter == 0) {
    ...
    unsigned long n_not_iterated_local = 0, n_not_in_domain_local = 0, n_points_unburnt_local = 0,
                  n_points_burnt_local = 0, n_points_flame_local = 0;
    unsigned long n_not_iterated_global, n_not_in_domain_global;
...
void CSpeciesFlameletSolver::Viscous_Residual(unsigned long iEdge, CGeometry* geometry, CSolver** solver_container,
  ...
  su2double scalar_i[nVar], scalar_j[nVar];

  su2activematrix scalar_grad_i(nVar, nDim), scalar_grad_j(nVar, nDim);

  su2double diff_coeff_beta_i[nVar], diff_coeff_beta_j[nVar];
...
void CSpeciesFlameletSolver::BC_Inlet(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
  ...
  su2double enth_inlet, mfrac_inlet = 0.0;
