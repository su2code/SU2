...
CDiscAdjResidualSolver::CDiscAdjResidualSolver(CGeometry *geometry, CConfig *config, CSolver *direct_solver,
  ...
  // Derivatives w.r.t. mesh coordinates
...
void CDiscAdjResidualSolver::RegisterVariables(CGeometry* geometry, CConfig* config, bool reset) {
  SU2_OMP_MASTER {
    ...
    if ((config->GetKind_Regime() == ENUM_REGIME::INCOMPRESSIBLE) &&
      ...
      /*--- Access the nondimensional free-stream temperature. ---*/
    ...
    if (mesh_solver) {
      ...
      for (auto iMarker = 0ul; iMarker < nMarker; iMarker++) {
        ...
        for (auto iVertex = 0ul; iVertex < geometry->GetnVertex(iMarker); iVertex++) {
          ...
          for (auto iDim = 0u; iDim < nDim; iDim++) {
            (*MarkerSensitivity)[iMarker](iVertex, iDim) =
                limiter * mesh_solver->GetNodes()->GetBoundDisp_Sens(iPoint, iDim);
          }
        }
        END_SU2_OMP_FOR
      }
    ...
    for (auto iPoint = 0ul; iPoint < nPoint; iPoint++) {
      for (auto iDim = 0u; iDim < nDim; iDim++) {
        su2double Sensitivity =
            Partial_Sens_dObjective_dCoordinates(iPoint, iDim) + Partial_Prod_dResiduals_dCoordinates(iPoint, iDim);
...
void CDiscAdjResidualSolver::SetAdjoint_SourceTerm(unsigned long iPoint, unsigned short iVar, su2double val) {
  Partial_Sens_dObjective_dStates[iPoint][iVar] = val;
}

void CDiscAdjResidualSolver::Preprocessing(CGeometry* geometry, CSolver** solver_container, CConfig* config,
...
void CDiscAdjResidualSolver::LoadRestart(CGeometry** geometry, CSolver*** solver, CConfig* config, int val_iter,
                                         bool val_update_geo) {
  ...
  if (KindDirect_Solver == RUNTIME_SPECIES_SYS) {
    // Skip the number of flow variables and turbulent variables to get to the adjoint species variables.
    skipVars += nDim + 2;
