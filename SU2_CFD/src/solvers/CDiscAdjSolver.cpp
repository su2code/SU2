...
void CDiscAdjSolver::ExtractAdjoint_Solution(CGeometry *geometry, CConfig *config){
  ...
  for (auto iPoint = 0u; iPoint < nPoint; iPoint++) {
    ...
    for (auto iVar = 0u; iVar < nVar; iVar++) {
      if (fabs(nodes->GetSolution(iPoint,iVar)) > 1.0e10 || (SolverName == "ADJ.FLOW" && geometry->node[iPoint]->GetGlobalIndex() == 25165)) {
        if (SolverName == "ADJ.FLOW" && geometry->node[iPoint]->GetGlobalIndex() == 25165) {
          cout << "EULER::Sol[iPoint=" << geometry->node[iPoint]->GetGlobalIndex() << "][iVar=" << iVar << "]= " << direct_solver->GetNodes()->GetSolution(iPoint,iVar) << ", Res= " << direct_solver->LinSysRes(iPoint,iVar) << ",AdjRes= " << nodes->GetSolution(iPoint,iVar) - nodes->GetSolution_Old(iPoint,iVar) << endl;
        }
        // else {
