...
void CEulerSolver::SetExtrapolationJacobian(CSolver             **solver,
  ...
  for (auto iVar = 0; iVar < nPrimVarTot; iVar++) {
    reconWeight_l[iVar] = bad_i ? 0.0 : 0.5*kappa*l_i[iVar]; 
    reconWeight_r[iVar] = bad_i ? 0.0 : 0.5*kappa*l_j[iVar];
  }
  ...
  for (auto iNeigh = 0; iNeigh < node_i->GetnPoint(); iNeigh++) {      
    ...
    for (auto iVar = 0; iVar < nPrimVarTot; iVar++)
      reconWeight_l[iVar] = bad_i ? 0.0 : factor*gradWeightDotDist*l_i[iVar];
