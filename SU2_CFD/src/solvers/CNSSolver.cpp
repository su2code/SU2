...
/*--- CFVMFlowSolverBase.inl is needed for explicit template instantiation below. ---*/
#include "../../include/solvers/CFVMFlowSolverBase.inl"  // IWYU pragma: keep
...
void CNSSolver::BC_Isothermal_Wall_Generic(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
  ...
  for (auto iVertex = 0u; iVertex < geometry->nVertex[val_marker]; iVertex++) {
    ...
    /*--- Safeguard for CONSTANT_PRANDTL_TURB on coarse meshes.
    ...
    if (config->GetKind_ConductivityModel_Turb() == CONDUCTIVITYMODEL_TURB::CONSTANT_PRANDTL) {
      su2double mut_interior = nodes->GetEddyViscosity(Point_Normal);
      su2double mul_interior = nodes->GetLaminarViscosity(Point_Normal);
      if (mut_interior / (mul_interior + EPS) > 0.1) {
        static bool warned = false;
        if (!warned && (rank == MASTER_NODE)) {
          cout
              << "Warning: CONSTANT_PRANDTL_TURB used on a coarse mesh (y+ > 1) without thermal wall functions.\n"
              << "         The linear wall temperature gradient assumption may lead to inaccurate heat "
                 "flux/temperature.\n"
              << "         Consider refining the near-wall mesh (y+ ~ 1) or using a thermal wall model if available.\n";
          warned = true;
        }
      }
    }
      ...
      while (fabs(diff) > tol) {
        ...
        if (config->GetMarker_All_KindBC(iMarker) != ISOTHERMAL) {
          ...
          if (denum > EPS) {
            ...
            SU2_OMP_CRITICAL { cout << "Warning: T_Wall < 0 \n"; }
      ...
      if (rank == MASTER_NODE && write_iter) {
        ...
        if (smallYPlusCounter)
          cout << "y+ < " << config->GetwallModel_MinYPlus() << " in " << smallYPlusCounter
               << " points. No problem, but you can increase your near-wall mesh size.\n";
      }
    }
