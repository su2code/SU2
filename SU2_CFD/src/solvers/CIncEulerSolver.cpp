...
void CIncEulerSolver::Preprocessing(CGeometry *geometry, CSolver **solver_container, CConfig *config, unsigned short iMesh,
  ...
  if (config->GetWrt_Gradient() || config->GetWrt_Hessian()){
    ...
    for (auto iPoint = 0; iPoint < nPoint; iPoint++){
      ...
      su2double StaticEnergy = nodes->GetEnergy(iPoint) - 0.5*Velocity2;

      GetFluidModel()->SetTDState_rhoe(Density, StaticEnergy);
      su2double Pressure = GetFluidModel()->GetPressure();
      su2double Temperature = GetFluidModel()->GetTemperature();
      su2double Mach = sqrt(Velocity2) / GetFluidModel()->GetSoundSpeed();
