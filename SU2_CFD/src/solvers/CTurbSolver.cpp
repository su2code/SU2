...
void CTurbSolver::Upwind_Residual(CGeometry *geometry, CSolver **solver,
  ...
  for(auto k = 0ul; k < color.size; ++k) {
    ...
    if (nodes->GetUnderRelaxation(iPoint) < 1e-6 && residual[0] >  nodes->GetSolution(iPoint,0)) cout << "TKEj= " << TKEj << ",TKEi= " << TKEi << ", Upw[" << geometry->node[iPoint]->GetGlobalIndex() << "]= " << -residual[0] << endl;
    if (nodes->GetUnderRelaxation(jPoint) < 1e-6 && residual[0] < -nodes->GetSolution(jPoint,0)) cout << "TKEi= " << TKEi << ",TKEj= " << TKEj << ", Upw[" << geometry->node[jPoint]->GetGlobalIndex() << "]= " <<  residual[0] << endl;
