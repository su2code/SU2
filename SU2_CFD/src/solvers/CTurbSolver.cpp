...
void CTurbSolver::Upwind_Residual(CGeometry *geometry, CSolver **solver,
  ...
  for(auto k = 0ul; k < color.size; ++k) {
    ...
    if (musclFlow) {
      ...
      if (!good_i) {cout << "Turb[" << iPoint << "]: k_i= " << turbPrimVar_i[0] << ", omega_i= " << turbPrimVar_i[1] << ", r_i= " << flowPrimVar_i[nDim+2] << ", p_i= " << flowPrimVar_i[nDim+1] << endl;}
      if (!good_j) {cout << "Turb[" << jPoint << "]: k_j= " << turbPrimVar_j[0] << ", omega_j= " << turbPrimVar_j[1] << ", r_j= " << flowPrimVar_j[nDim+2] << ", p_j= " << flowPrimVar_j[nDim+1] << endl;}
