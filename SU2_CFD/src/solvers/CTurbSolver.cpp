...
void CTurbSolver::Viscous_Residual(unsigned long iEdge, CGeometry *geometry, CSolver **solver,
  ...
  if (nodes->GetUnderRelaxation(iPoint) < 1e-6) cout << "VisRes[" << iPoint << "][0]= " <<  residual[0] << endl;
  if (nodes->GetUnderRelaxation(jPoint) < 1e-6) cout << "VisRes[" << jPoint << "][0]= " << -residual[0] << endl;
...
void CTurbSolver::ComputeUnderRelaxationFactor(CSolver **solver, CConfig *config) {
  ...
  // const su2double allowableIncrease = 1.0e12;
