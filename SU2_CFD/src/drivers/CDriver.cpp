    ...
    for (iPoint = 0; iPoint < geometry_container[ZONE_0][iInst][iMGlevel]->GetnPoint(); iPoint++) {
      ...
      for (jInst = 0; jInst < nInstHB; jInst++) {
        ...
        for (iVar = 0; iVar < nVar; iVar++) {
          ...
          if (!adjoint) {
            U[iVar] = solver_container[ZONE_0][jInst][iMGlevel][FLOW_SOL]->node->GetSolution(iPoint, iVar);
            Source[iVar] += U[iVar]*D[iInst][jInst];

            if (implicit) {
              U_old[iVar] = solver_container[ZONE_0][jInst][iMGlevel][FLOW_SOL]->node->GetSolution_Old(iPoint, iVar);
            ...
            Psi[iVar] = solver_container[ZONE_0][jInst][iMGlevel][ADJFLOW_SOL]->node->GetSolution(iPoint, iVar);
            Source[iVar] += Psi[iVar]*D[jInst][iInst];

            if (implicit) {
              Psi_old[iVar] = solver_container[ZONE_0][jInst][iMGlevel][ADJFLOW_SOL]->node->GetSolution_Old(iPoint, iVar);
        ...
        for (iVar = 0; iVar < nVar; iVar++) {
          if (!adjoint) {
            solver_container[ZONE_0][iInst][iMGlevel][FLOW_SOL]->node->SetHarmonicBalance_Source(iPoint, iVar, Source[iVar]);
          }
          else {
            solver_container[ZONE_0][iInst][iMGlevel][ADJFLOW_SOL]->node->SetHarmonicBalance_Source(iPoint, iVar, Source[iVar]);
          }
        }
    ...
    for (iPoint = 0; iPoint < geometry_container[ZONE_0][INST_0][MESH_0]->GetnPoint(); iPoint++) {
      ...
      for (jInst = 0; jInst < nInstHB; jInst++) {
        ...
        for (iVar = 0; iVar < nVar_Turb; iVar++) {
          U_Turb[iVar] = solver_container[ZONE_0][jInst][MESH_0][TURB_SOL]->node->GetSolution(iPoint, iVar);
      ...
      for (iVar = 0; iVar < nVar_Turb; iVar++)
        solver_container[ZONE_0][iInst][MESH_0][TURB_SOL]->node->SetHarmonicBalance_Source(iPoint, iVar, Source_Turb[iVar]);
    }
...
void CHBDriver::StabilizeHarmonicBalance() {
  ...
  for (iMGlevel = 0; iMGlevel <= config_container[ZONE_0]->GetnMGLevels(); iMGlevel++) {
    ...
    for (iPoint = 0; iPoint < geometry_container[ZONE_0][INST_0][iMGlevel]->GetnPoint(); iPoint++) {
      ...
      Delta = solver_container[ZONE_0][INST_0][iMGlevel][FLOW_SOL]->node->GetDelta_Time(iPoint);
      ...
      for (iVar = 0; iVar < nVar; iVar++) {
        ...
        for (iInst = 0; iInst < nInstHB; iInst++) {
          Source_old[iInst] = solver_container[ZONE_0][iInst][iMGlevel][FLOW_SOL]->node->GetHarmonicBalance_Source(iPoint, iVar);
          Source[iInst] = 0;
        }
        ...
        for (iInst = 0; iInst < nInstHB; iInst++) {
          ...
          if (!adjoint) {
            solver_container[ZONE_0][iInst][iMGlevel][FLOW_SOL]->node->SetHarmonicBalance_Source(iPoint, iVar, Source[iInst]);
          }
          else {
            solver_container[ZONE_0][iInst][iMGlevel][ADJFLOW_SOL]->node->SetHarmonicBalance_Source(iPoint, iVar, Source[iInst]);
          }
        }
...
void CDiscAdjFSIDriver::Preprocess(unsigned short ZONE_FLOW,
    ...
    if ((ExtIter > 0) && dual_time){
      ...
      for (iMesh=0; iMesh<=config_container[ZONE_FLOW]->GetnMGLevels();iMesh++) {
        for(iPoint=0; iPoint<geometry_container[ZONE_FLOW][INST_0][iMesh]->GetnPoint();iPoint++) {
          solver_container[ZONE_FLOW][INST_0][iMesh][FLOW_SOL]->node->SetSolution(iPoint, solver_container[ZONE_FLOW][INST_0][iMesh][FLOW_SOL]->node->GetSolution_time_n(iPoint));
          if (turbulent) {
            solver_container[ZONE_FLOW][INST_0][iMesh][TURB_SOL]->node->SetSolution(iPoint, solver_container[ZONE_FLOW][INST_0][iMesh][TURB_SOL]->node->GetSolution_time_n(iPoint));
          }
        }
      }
      if (dual_time_1st){
      /*--- Set Solution at timestep n-1 to the previously loaded solution ---*/
        for (iMesh=0; iMesh<=config_container[ZONE_FLOW]->GetnMGLevels();iMesh++) {
          for(iPoint=0; iPoint<geometry_container[ZONE_FLOW][INST_0][iMesh]->GetnPoint();iPoint++) {
            solver_container[ZONE_FLOW][INST_0][iMesh][FLOW_SOL]->node->Set_Solution_time_n(iPoint, solver_container[ZONE_FLOW][INST_0][iMesh][FLOW_SOL]->node->GetSolution_time_n1(iPoint));
            if (turbulent) {
              solver_container[ZONE_FLOW][INST_0][iMesh][TURB_SOL]->node->Set_Solution_time_n(iPoint, solver_container[ZONE_FLOW][INST_0][iMesh][TURB_SOL]->node->GetSolution_time_n1(iPoint));
            }
          }
        }
      }
      if (dual_time_2nd){
        ...
        for (iMesh=0; iMesh<=config_container[ZONE_FLOW]->GetnMGLevels();iMesh++) {
          for(iPoint=0; iPoint<geometry_container[ZONE_FLOW][INST_0][iMesh]->GetnPoint();iPoint++) {
            solver_container[ZONE_FLOW][INST_0][iMesh][FLOW_SOL]->node->Set_Solution_time_n1(iPoint, solver_container[ZONE_FLOW][INST_0][iMesh][FLOW_SOL]->node->GetSolution_Old(iPoint));
            if (turbulent) {
              solver_container[ZONE_FLOW][INST_0][iMesh][TURB_SOL]->node->Set_Solution_time_n1(iPoint, solver_container[ZONE_FLOW][INST_0][iMesh][TURB_SOL]->node->GetSolution_Old(iPoint));
            }
          }
        }
      }
  ...
  if (ExtIter == 0 || dual_time) {
    ...
    for (iMesh=0; iMesh<=config_container[ZONE_FLOW]->GetnMGLevels();iMesh++) {
      for (iPoint = 0; iPoint < geometry_container[ZONE_FLOW][INST_0][iMesh]->GetnPoint(); iPoint++) {
        solver_container[ZONE_FLOW][INST_0][iMesh][ADJFLOW_SOL]->node->SetSolution_Direct(iPoint, solver_container[ZONE_FLOW][INST_0][iMesh][FLOW_SOL]->node->GetSolution(iPoint));
      }
    }
    if (turbulent && !config_container[ZONE_FLOW]->GetFrozen_Visc_Disc()) {
      for (iPoint = 0; iPoint < geometry_container[ZONE_FLOW][INST_0][MESH_0]->GetnPoint(); iPoint++) {
        solver_container[ZONE_FLOW][INST_0][MESH_0][ADJTURB_SOL]->node->SetSolution_Direct(iPoint, solver_container[ZONE_FLOW][INST_0][MESH_0][TURB_SOL]->node->GetSolution(iPoint));
      }
    }
  }
    ...
    for (iPoint = 0; iPoint < geometry_container[ZONE_FLOW][INST_0][MESH_0]->GetnPoint(); iPoint++){
      solver_container[ZONE_FLOW][INST_0][MESH_0][ADJFLOW_SOL]->node->SetGeometry_Direct(iPoint, geometry_container[ZONE_FLOW][INST_0][MESH_0]->node[iPoint]->GetCoord());
    }
  ...
  if (dynamic) {
    ...
    for (iPoint = 0; iPoint < geometry_container[ZONE_STRUCT][INST_0][MESH_0]->GetnPoint(); iPoint++){
      solver_container[ZONE_STRUCT][INST_0][MESH_0][ADJFEA_SOL]->node->SetSolution_Direct(iPoint, solver_container[ZONE_STRUCT][INST_0][MESH_0][FEA_SOL]->node->GetSolution(iPoint));
    }

    for (iPoint = 0; iPoint < geometry_container[ZONE_STRUCT][INST_0][MESH_0]->GetnPoint(); iPoint++){
      solver_container[ZONE_STRUCT][INST_0][MESH_0][ADJFEA_SOL]->node->SetSolution_Accel_Direct(iPoint, solver_container[ZONE_STRUCT][INST_0][MESH_0][FEA_SOL]->node->GetSolution_Accel(iPoint));
    }

    for (iPoint = 0; iPoint < geometry_container[ZONE_STRUCT][INST_0][MESH_0]->GetnPoint(); iPoint++){
      solver_container[ZONE_STRUCT][INST_0][MESH_0][ADJFEA_SOL]->node->SetSolution_Vel_Direct(iPoint, solver_container[ZONE_STRUCT][INST_0][MESH_0][FEA_SOL]->node->GetSolution_Vel(iPoint));
    }
