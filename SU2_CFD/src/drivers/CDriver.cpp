...
void CDriver::Solver_Preprocessing(CConfig* config, CGeometry** geometry, CSolver ***&solver) {
  ...
  switch (config->GetKind_Solver()) {
    ...
    case NAVIER_STOKES: ns = true; compressible = true; scalar = (config->GetKind_Scalar_Model() != NONE); heat_fvm = config->GetWeakly_Coupled_Heat(); break;
    case RANS : ns = true; turbulent = true; compressible = true; if (config->GetKind_Trans_Model() == LM) transition = true; scalar = (config->GetKind_Scalar_Model() != NONE); heat_fvm = config->GetWeakly_Coupled_Heat(); break;
    case INC_EULER : euler = true; incompressible = true; break;
    case INC_NAVIER_STOKES: ns = true; incompressible = true; scalar = (config->GetKind_Scalar_Model() != NONE); heat_fvm = config->GetWeakly_Coupled_Heat(); break;
    case INC_RANS : ns = true; turbulent = true; incompressible = true; if (config->GetKind_Trans_Model() == LM) transition = true; scalar = (config->GetKind_Scalar_Model() != NONE); heat_fvm = config->GetWeakly_Coupled_Heat(); break;
    ...
    case DISC_ADJ_NAVIER_STOKES: ns = true; disc_adj = true; compressible = true; scalar = (config->GetKind_Scalar_Model() != NONE); heat_fvm = config->GetWeakly_Coupled_Heat(); break;
    case DISC_ADJ_RANS: ns = true; turbulent = true; disc_adj = true; compressible = true; disc_adj_turb = (!config->GetFrozen_Visc_Disc()); scalar = (config->GetKind_Scalar_Model() != NONE); heat_fvm = config->GetWeakly_Coupled_Heat(); break;
    case DISC_ADJ_INC_EULER: euler = true; disc_adj = true; incompressible = true; break;
    case DISC_ADJ_INC_NAVIER_STOKES: ns = true; disc_adj = true; incompressible = true; scalar = (config->GetKind_Scalar_Model() != NONE); heat_fvm = config->GetWeakly_Coupled_Heat(); break;
    case DISC_ADJ_INC_RANS: ns = true; turbulent = true; disc_adj = true; incompressible = true; disc_adj_turb = (!config->GetFrozen_Visc_Disc()); scalar = (config->GetKind_Scalar_Model() != NONE); heat_fvm = config->GetWeakly_Coupled_Heat(); break;
...
void CDriver::Solver_Restart(CSolver ***solver, CGeometry **geometry,
  ...
  switch (config->GetKind_Solver()) {
    case TEMPLATE_SOLVER: template_solver = true; break;
    case EULER : case INC_EULER: euler = true; break;
    case NAVIER_STOKES: case INC_NAVIER_STOKES: ns = true; scalar = (config->GetKind_Scalar_Model() != NONE); heat_fvm = config->GetWeakly_Coupled_Heat(); break;
    case RANS : case INC_RANS: ns = true; turbulent = true; scalar = (config->GetKind_Scalar_Model() != NONE); heat_fvm = config->GetWeakly_Coupled_Heat(); break;
    ...
    case ADJ_RANS : ns = true; turbulent = true; adj_ns = true; adj_turb = (!config->GetFrozen_Visc_Cont()); break;
    case DISC_ADJ_EULER: case DISC_ADJ_INC_EULER: euler = true; disc_adj = true; break;
    case DISC_ADJ_NAVIER_STOKES: case DISC_ADJ_INC_NAVIER_STOKES: ns = true; disc_adj = true; scalar = (config->GetKind_Scalar_Model() != NONE); heat_fvm = config->GetWeakly_Coupled_Heat(); break;
    case DISC_ADJ_RANS: case DISC_ADJ_INC_RANS: ns = true; turbulent = true; disc_adj = true; disc_adj_turb = (!config->GetFrozen_Visc_Disc()); scalar = (config->GetKind_Scalar_Model() != NONE); heat_fvm = config->GetWeakly_Coupled_Heat(); break;
...
void CDriver::Solver_Postprocessing(CSolver ****solver, CGeometry **geometry,
  ...
  switch (config->GetKind_Solver()) {
    ...
    case EULER : case INC_EULER: euler = true; break;
    case NAVIER_STOKES: case INC_NAVIER_STOKES: ns = true; scalar = (config->GetKind_Scalar_Model() != NONE); heat_fvm = config->GetWeakly_Coupled_Heat(); break;
    case RANS : case INC_RANS: ns = true; turbulent = true; if (config->GetKind_Trans_Model() == LM) transition = true; scalar = (config->GetKind_Scalar_Model() != NONE); heat_fvm = config->GetWeakly_Coupled_Heat(); break;
  ...
  for (iMGlevel = 0; iMGlevel <= config->GetnMGLevels(); iMGlevel++) {
    ...
    if (scalar) {
      if (passive_scalar || progress_variable) {
        delete solver[val_iInst][iMGlevel][SCALAR_SOL];
      }
    }
    if (heat_fvm) {
...
void CDriver::Integration_Preprocessing(CConfig *config, CIntegration **&integration) {
  ...
  switch (config->GetKind_Solver()) {
    ...
    case DISC_ADJ_NAVIER_STOKES: case DISC_ADJ_INC_NAVIER_STOKES: ns = true; disc_adj = true; scalar = (config->GetKind_Scalar_Model() != NONE); heat_fvm = config->GetWeakly_Coupled_Heat(); break;
    case DISC_ADJ_RANS : case DISC_ADJ_INC_RANS: ns = true; turbulent = true; disc_adj = true; scalar = (config->GetKind_Scalar_Model() != NONE); heat_fvm = config->GetWeakly_Coupled_Heat(); break;
    case DISC_ADJ_FEM: fem = true; disc_adj_fem = true; break;
    case DISC_ADJ_HEAT: heat_fvm = true; disc_adj_heat = true; break;
  }
  ...
  if (scalar) integration[SCALAR_SOL] = new CMultiGridIntegration(config);
...
void CDriver::Integration_Postprocessing(CIntegration ***integration, CGeometry **geometry, CConfig *config, unsigned short val_iInst) {
  ...
  if (scalar) delete integration[val_iInst][SCALAR_SOL];
...
void CDriver::Numerics_Preprocessing(CConfig *config, CGeometry **geometry, CSolver ***solver, CNumerics ****&numerics) {
  ...
  switch (config->GetKind_Solver()) {
    ...
    case NAVIER_STOKES: case DISC_ADJ_NAVIER_STOKES:compressible = true; ns = true; scalar = (config->GetKind_Scalar_Model() != NONE); break;
    case RANS : case DISC_ADJ_RANS:  ns = true; compressible = true; turbulent = true; if (config->GetKind_Trans_Model() == LM) transition = true; scalar = (config->GetKind_Scalar_Model() != NONE); break;
    case INC_EULER : case DISC_ADJ_INC_EULER: incompressible =true; euler = true; break;
    case INC_NAVIER_STOKES: case DISC_ADJ_INC_NAVIER_STOKES:incompressible =true; ns = true; scalar = (config->GetKind_Scalar_Model() != NONE); heat_fvm = config->GetWeakly_Coupled_Heat(); break;
    case INC_RANS : case DISC_ADJ_INC_RANS: incompressible =true; ns = true; turbulent = true; scalar = (config->GetKind_Scalar_Model() != NONE); heat_fvm = config->GetWeakly_Coupled_Heat(); if (config->GetKind_Trans_Model() == LM) transition = true; break;
...
void CDriver::Numerics_Postprocessing(CNumerics *****numerics,
  ...
  switch (config->GetKind_Solver()) {
    ...
    case NAVIER_STOKES: case DISC_ADJ_NAVIER_STOKES:compressible = true; ns = true; scalar = (config->GetKind_Scalar_Model() != NONE); heat_fvm = config->GetWeakly_Coupled_Heat(); break;
    case RANS : case DISC_ADJ_RANS:  ns = true; compressible = true; turbulent = true; if (config->GetKind_Trans_Model() == LM) transition = true;scalar = (config->GetKind_Scalar_Model() != NONE);  break;
    case INC_EULER : case DISC_ADJ_INC_EULER: incompressible =true; euler = true;  heat_fvm = config->GetWeakly_Coupled_Heat(); break;
    case INC_NAVIER_STOKES: case DISC_ADJ_INC_NAVIER_STOKES:incompressible =true; ns = true; scalar = (config->GetKind_Scalar_Model() != NONE); heat_fvm = config->GetWeakly_Coupled_Heat(); break;
    case INC_RANS : case DISC_ADJ_INC_RANS: incompressible =true; ns = true; turbulent = true; if (config->GetKind_Trans_Model() == LM) transition = true; scalar = (config->GetKind_Scalar_Model() != NONE); break;
  ...
  if (scalar) {
    ...
    switch (config->GetKind_ConvNumScheme_Scalar()) {
      case SPACE_UPWIND :
        for (iMGlevel = 0; iMGlevel <= config->GetnMGLevels(); iMGlevel++) {
          if (passive_scalar || progress_variable)
            delete numerics[val_iInst][iMGlevel][SCALAR_SOL][CONV_TERM];
        }
        break;
    }
    ...
    if (passive_scalar || progress_variable) {
      for (iMGlevel = 0; iMGlevel <= config->GetnMGLevels(); iMGlevel++) {
        delete numerics[val_iInst][iMGlevel][SCALAR_SOL][VISC_TERM];
        delete numerics[val_iInst][iMGlevel][SCALAR_SOL][SOURCE_FIRST_TERM];
        delete numerics[val_iInst][iMGlevel][SCALAR_SOL][SOURCE_SECOND_TERM];
        /*--- Definition of the boundary condition method ---*/
        delete numerics[val_iInst][iMGlevel][SCALAR_SOL][CONV_BOUND_TERM];
        delete numerics[val_iInst][iMGlevel][SCALAR_SOL][VISC_BOUND_TERM];
