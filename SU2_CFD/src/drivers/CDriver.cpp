...
void CDriver::Inlet_Preprocessing(CSolver ***solver, CGeometry **geometry,
  ...
  if (config->GetInlet_Profile_From_File()) {
    ...
    if (euler || ns || adj_euler || adj_ns || disc_adj) {
      ...
      solver[MESH_0][FLOW_SOL]->LoadInletProfile(geometry, solver, config, val_iter, FLOW_SOL, SUPERSONIC_FLOW);
    }
    if (turbulent || adj_turb || disc_adj_turb) {
      ...
      solver[MESH_0][TURB_SOL]->LoadInletProfile(geometry, solver, config, val_iter, TURB_SOL, SUPERSONIC_FLOW);
    }
