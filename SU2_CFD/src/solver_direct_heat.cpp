      ...
      if (config->GetMarker_All_KindBC(iMarker) == CHT_WALL_INTERFACE) {
        ...
        for (iVertex = 0; iVertex < geometry->nVertex[iMarker]; iVertex++) {
          ...
          if (geometry->node[iPoint]->GetDomain()) {
            ...
            Tinterface = GetInterfaceVariable(iMarker, iVertex, 0);
      ...
      if (config->GetMarker_All_KindBC(iMarker) == CHT_WALL_INTERFACE) {
        ...
        for (iVertex = 0; iVertex < geometry->nVertex[iMarker]; iVertex++) {
          ...
          if (geometry->node[iPoint]->GetDomain()) {
            ...
            HeatFluxDensity = HeatFluxDensity/rho_cp_solid;
  ...
  for (iPoint = 0; iPoint < nPointDomain; iPoint++) {
    ...
    if (Vol != 0.0) {
      ...
      if(flow) {
        ...
        Local_Delta_Time_Visc = config->GetCFL_Solid()*K_v*Vol*Vol/ node[iPoint]->GetMax_Lambda_Visc();
      }
