...
void CFluidIteration::Update(COutput *output,
  ...
  if ((config[val_iZone]->GetTime_Marching() == DT_STEPPING_1ST) ||
    ...
    if (config[val_iZone]->GetKind_Scalar_Model() != NO_SCALAR_MODEL) {
      for (iMesh = 0; iMesh <= config[val_iZone]->GetnMGLevels(); iMesh++) {
        integration[val_iZone][val_iInst][SCALAR_SOL]->SetDualTime_Solver(geometry[val_iZone][val_iInst][iMesh], solver[val_iZone][val_iInst][iMesh][SCALAR_SOL], config[val_iZone], iMesh);
