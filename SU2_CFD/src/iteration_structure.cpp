...
bool CFluidIteration::Monitor(COutput *output,
  ...
  if((StopCalc) &&
    ...
    StopCalc = false;
    config[val_iZone]->SetWallFunction_Start_Iter(config[val_iZone]->GetInnerIter()+1);
  }
...
void CDiscAdjFluidIteration::SetDependencies(CSolver *****solver,
  ...
  if (turbulent && !frozen_visc) {
    ...
    if (sst) solver[iZone][iInst][MESH_0][TURB_SOL]->SetPrimitive_Variables(solver[iZone][iInst][MESH_0]);
  }
