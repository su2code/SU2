...
void CSingleGridIntegration::SingleGrid_Iteration(CGeometry ****geometry, CSolver *****solver_container,
  ...
  switch (RunTime_EqSystem) {
    ...
    case RUNTIME_POISSON_SYS: monitor = log10(solver_container[iZone][iInst][FinestMesh][POISSON_SOL]->GetRes_RMS(0)); 
       if ((Iteration%100)==0) cout<<"Res : "<<monitor<<endl;
    break;
    case RUNTIME_FLOW_SYS:if ((Iteration%50)==0)  cout<<"Momentum Res: "<<log10(solver_container[iZone][iInst][FinestMesh][FLOW_SOL]->GetRes_RMS(0))<<", "<<log10(solver_container[iZone][iInst][FinestMesh][POISSON_SOL]->GetRes_RMS(1))<<endl;
  }
