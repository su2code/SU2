...
//  for(int i=0; i<geometry->GetnElem(); ++i)
//    element_properties[i]->SetPhysicalDensity(element_properties[i]->GetDesignDensity());
...
void CFEASolver::FilterElementDensities(CGeometry *geometry, CConfig *config)
{
  ...
  The filter is an averaging process over the neighbourhood of each element,
  which is a circle in 2D and a sphere in 3D. The filter is characterized by
  its kernel, i.e. how the weights are computed. ---*/
  ...
  for(iElem=0; iElem<nElemDomain; ++iElem) nFaces_elem[iElem] = 0;
...
#define SWAP_AND_DEL_OLD(p1,p2) tmp=p1; p1=p2; delete[] tmp
  ...
  /*--- The matrix will be in compressed format so first we create the vector
  with the addresses of the start of the neighbours of a given element. This
  is generated by a cumulative sum of the neighbour count. ---*/
  vector<unsigned long> neighbour_start(nElemDomain+1);
  long *neighbour_idx = NULL; // <-- the "coefficients" of the adjacency matrix
...
#undef SWAP_AND_DEL_OLD
  ...
  for (iElem=0; iElem<nElem; ++iElem)
  {
    ...
    passivedouble filterRadius = 4e-3;
    ...
    while (cursor_it != neighbours.end())
    {
      ...
      for (aux_it=new_neighbours.begin(); aux_it!=new_neighbours.end(); ++aux_it)
      {
        ...
        if(sqrt(distance) < filterRadius) neighbours.push_back(*aux_it);
      }
    ...
    for (aux_it=neighbours.begin(); aux_it!=neighbours.end(); ++aux_it)
    {
      su2double distance = 0.0;
      for (unsigned short iDim=0; iDim<nDim; ++iDim)
        distance += pow(cg_elem[nDim*iElem_global+iDim]-cg_elem[nDim*(*aux_it)+iDim],2.0);
      distance = sqrt(distance);
      
      su2double weight = (filterRadius-distance);//*vol_elem[*aux_it];
      
      weighted_sum += weight*rho_elem[*aux_it];
