...
//  if (de_effects){
//
//      unsigned short iEField;
//      // Read the number of electric field regions available
//      unsigned short nEField = config->GetnElectric_Field();
//      unsigned long iElem;
//
//      /*--- Allocate the vector that stores the design variables ---*/
//      DV_Val = new su2double[nEField];
//
//      for (iEField = 0; iEField < nEField; iEField++)
//        DV_Val[iEField] = config->Get_Electric_Field_Mod(iEField);
//
//      cout << "NDV:" << config->GetnDV() << endl;
//      for (unsigned short iDV = 0; iDV < config->GetnDV(); iDV++){
//          cout << "DV_VALUE("<< iDV << "): " << config->GetDV_Value(iDV,0) << endl;
//      }
//
//
//  }
      ...
      for (iDim = 0; iDim < nDim; iDim++) {
        ...
        for (iTerm = 0; iTerm < nFEA_Terms; iTerm++) {
          ...
          if (prestretch_fem) {
            ...
            element_container[iTerm][EL_KIND]->SetRef_Coord(val_Ref, iNode, iDim);
          }
          else{
