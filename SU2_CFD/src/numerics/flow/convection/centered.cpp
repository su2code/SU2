...
CNumerics::ResidualType<> CCentLaxInc_Flow::ComputeResidual(const CConfig* config) {
  ...
  if (implicit) {
    GetInviscidIncProjJac(&MeanDensity, MeanVelocity, &MeanBetaInc2, &MeanEnthalpy, &MeandRhodh, Normal, 0.5, Jacobian_i);
    for (iVar = 0; iVar < nVar; iVar++) {
...
// TODO: Move this class to another file as it is not a centered scheme
// TODO: Current version is copied from the old pressure based solver it is currently not known what limiter it uses
