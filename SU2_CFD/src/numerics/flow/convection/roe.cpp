...
CNumerics::ResidualType<> CUpwRoeBase_Flow::ComputeResidual(const CConfig* config) {
  ...
  for (auto iDim = 0; iDim < nDim; iDim++)
    Epsilon[iDim] = 4.0*max(fabs(Lambda[iDim]-ProjVelocity_i),
                            fabs(ProjVelocity_j-Lambda[iDim]));

  Epsilon[nVar-2] = 4.0*max(fabs(Lambda[nVar-2]-(ProjVelocity_i+SoundSpeed_i)),
                            fabs((ProjVelocity_j+SoundSpeed_j)-Lambda[nVar-2]));
  Epsilon[nVar-1] = 4.0*max(fabs(Lambda[nVar-1]-(ProjVelocity_i-SoundSpeed_i)),
                            fabs((ProjVelocity_j-SoundSpeed_j)-Lambda[nVar-1]));
