...
CNumerics::ResidualType<> CCentLinearPB_Flow::ComputeResidual(const CConfig *config) {
  ...
  su2double MeanDensity, ProjGridMassFlux = 0.0;
...
CNumerics::ResidualType<> CUpwPB_Flow::ComputeResidual(const CConfig *config) {
  ...
  /*--- Find flux. ---*/
  for (iVar = 0; iVar < nVar; iVar++) {
    Flux[iVar] = MassFlux * ((Upw_i) ? V_i[iVar+1] : V_j[iVar+1]);
    Velocity_upw[iVar] = (Upw_i) ? V_i[iVar+1] : V_j[iVar+1];
    if (dynamic_grid) Velocity_upw[iVar] -= (Upw_i) ? GridVel_i[iVar] : GridVel_j[iVar];
  }
