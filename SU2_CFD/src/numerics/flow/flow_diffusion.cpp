...
void CAvgGrad_Base::CorrectGradient(su2double** GradPrimVar,
  ...
  for (auto iVar = 0u; iVar < val_nPrimVar; iVar++) {
  ...
  if (tkeNeeded) {
    const su2double GradTurbVar_Edge = GeometryToolbox::DotProduct(nDim,Mean_GradTurbVar,Edge_Vector);
...
void CAvgGrad_Base::SetTauJacobian() {
  ...
  for (auto iDim = 0u; iDim < nDim; iDim++) {
    ...
    tau_jacobian_i[iDim][0] = -GeometryToolbox::DotProduct(nDim,tau_jacobian_i[iDim]+1,V_i+1);
    tau_jacobian_j[iDim][0] = -GeometryToolbox::DotProduct(nDim,tau_jacobian_j[iDim]+1,V_j+1);
...
void CAvgGrad_Base::GetViscousProjJacs(const su2double *val_Mean_PrimVar,
  ...
  for (auto iDim = 0u; iDim < nDim; iDim++) {
    for (auto iVar = 0u; iVar < (unsigned long)nVar-1; iVar++) {
  ...
  const su2double proj_flux_vel_i = GeometryToolbox::DotProduct(nDim,val_Proj_Visc_Flux+1,V_i+1);
  const su2double proj_flux_vel_j = GeometryToolbox::DotProduct(nDim,val_Proj_Visc_Flux+1,V_j+1);

  const su2double contraction_i = -GeometryToolbox::DotProduct(nDim,Jacobian_i[nVar-1]+1,V_i+1);
  const su2double contraction_j = -GeometryToolbox::DotProduct(nDim,Jacobian_j[nVar-1]+1,V_j+1);
...
CNumerics::ResidualType<> CAvgGrad_Flow::ComputeResidual(const CConfig* config) {
  ...
  proj_vector_ij = correct_gradient? GeometryToolbox::DotProduct(nDim,Normal,Edge_Vector) : su2double(1.0);
...
void CAvgGrad_Flow::SetHeatFluxJacobian(const su2double *val_Mean_PrimVar,
  ...
  if (tkeNeeded) {
    const su2double tke_turb_visc = 0.5*(Eddy_Viscosity_i*sigma_k_i+Eddy_Viscosity_j*sigma_k_j);
...
void CAvgGrad_Flow::SetLaminarViscosityJacobian(const su2double *val_Mean_PrimVar,
  ...
  for (auto iDim = 0u; iDim < nDim; iDim++) {
    ...
    Jacobian_i[nDim+1][iDim+1] -= factor_i*V_i[iDim+1]*(proj_stress_dot_v + proj_heat_flux + proj_tke_flux);
    Jacobian_j[nDim+1][iDim+1] -= factor_j*V_j[iDim+1]*(proj_stress_dot_v + proj_heat_flux + proj_tke_flux);
  }

  Jacobian_i[nDim+1][0] += factor_i*(v2_i/2. - Cv*T_i)*(proj_heat_flux + proj_tke_flux);
  Jacobian_j[nDim+1][0] += factor_j*(v2_j/2. - Cv*T_j)*(proj_heat_flux + proj_tke_flux);

  Jacobian_i[nDim+1][nDim+1] += factor_i*(proj_stress_dot_v + proj_heat_flux + proj_tke_flux);  
  Jacobian_j[nDim+1][nDim+1] += factor_j*(proj_stress_dot_v + proj_heat_flux + proj_tke_flux);
}
