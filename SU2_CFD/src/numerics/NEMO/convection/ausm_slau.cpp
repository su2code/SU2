...
void CUpwAUSM_SLAU_Base_NEMO::ComputeJacobian(su2double **val_Jacobian_i, su2double **val_Jacobian_j) {
  ...
  if (M_F >= 0.0) Fc_LR = Fc_L;
  else            Fc_LR = Fc_R;
  
  su2double A_LR = 0;
  if (M_F >= 0.0) A_LR = A_F[0];
  else            A_LR = A_F[1];
  ...
  for (unsigned short iSpecies = 0; iSpecies < nEl; iSpecies++) {
    da_L[iSpecies] = 1.0/(2.0*SoundSpeed_i*Density_i) * (1+dPdU_i[nSpecies+nDim])*(dPdU_i[iSpecies] - Pressure_i/Density_i);
    da_R[iSpecies] = 1.0/(2.0*SoundSpeed_j*Density_j) * (1+dPdU_j[nSpecies+nDim])*(dPdU_j[iSpecies] - Pressure_j/Density_j);
  }

  // Heavy species
  for (unsigned short iSpecies = nEl; iSpecies < nSpecies; iSpecies++) {
    da_L[iSpecies] = 1.0/(2.0*SoundSpeed_i) * (1/rhoCvtr_i*(Ru/Ms[iSpecies] - Cvtr[iSpecies]*dPdU_i[nSpecies+nDim])*Pressure_i/Density_i
                   + 1.0/Density_i*(1.0+dPdU_i[nSpecies+nDim])*(dPdU_i[iSpecies] - Pressure_i/Density_i));
    da_R[iSpecies] = 1.0/(2.0*SoundSpeed_j) * (1/rhoCvtr_j*(Ru/Ms[iSpecies] - Cvtr[iSpecies]*dPdU_j[nSpecies+nDim])*Pressure_j/Density_j
                   + 1.0/Density_j*(1.0+dPdU_j[nSpecies+nDim])*(dPdU_j[iSpecies] - Pressure_j/Density_j));
  }

  /*--- Sound speed derivatives: Momentum ---*/
  for (unsigned short iDim = 0; iDim < nDim; iDim++) {
    da_L[nSpecies+iDim] = -1.0/(2.0*Density_i*SoundSpeed_i) * ((1.0+dPdU_i[nSpecies+nDim])*dPdU_i[nSpecies+nDim])*Velocity_i[iDim];
    da_R[nSpecies+iDim] = -1.0/(2.0*Density_j*SoundSpeed_j) * ((1.0+dPdU_j[nSpecies+nDim])*dPdU_j[nSpecies+nDim])*Velocity_j[iDim];
  }
  ...
  da_R[nSpecies+nDim+1] = 1.0/(2.0*Density_j*SoundSpeed_j) * ((1.0+dPdU_j[nSpecies+nDim])*dPdU_j[nSpecies+nDim+1]);
  ...
  if (M_F >= 0) {
    ...
    for (unsigned short iSpecies = 0; iSpecies < nSpecies; iSpecies++) {
      Jacobian_i[nSpecies+nDim][iSpecies] += M_F * (dPdU_i[iSpecies]*SoundSpeed_i + Density_i*Enthalpy_i*da_L[iSpecies]);
    }
    for (unsigned short iDim = 0; iDim < nDim; iDim++) {
      Jacobian_i[nSpecies+nDim][nSpecies+iDim] += M_F * (-dPdU_i[nSpecies+nDim]*Velocity_i[iDim]*SoundSpeed_i + Density_i*Enthalpy_i*da_L[nSpecies+iDim]);
    }
    Jacobian_i[nSpecies+nDim][nSpecies+nDim]   += M_F * ((1.0+dPdU_i[nSpecies+nDim])*SoundSpeed_i + Density_i*Enthalpy_i*da_L[nSpecies+nDim]);
    Jacobian_i[nSpecies+nDim][nSpecies+nDim+1] += M_F * (dPdU_i[nSpecies+nDim+1]*SoundSpeed_i + Density_i*Enthalpy_i*da_L[nSpecies+nDim+1]);
    for (unsigned short jVar = 0; jVar < nVar; jVar++) {
  ...
  if ((M_F >= 0) || ((M_F < 0) && (fabs(M_F) <= 1.0))) {
    ...
    if (fabs(M_L) <= 1.0) {
      ...
      /*--- Pressure ---*/
      for (unsigned short iSpecies = 0; iSpecies < nSpecies; iSpecies++)
        dP_LP[iSpecies] = dPdU_i[iSpecies];
      for (unsigned short iDim = 0; iDim < nDim; iDim++)
  ...
  if (M_F < 0) {
    ...
    for (unsigned short iSpecies = 0; iSpecies < nSpecies; iSpecies++) {
      Jacobian_j[nSpecies+nDim][iSpecies] += M_F * (dPdU_j[iSpecies]*SoundSpeed_j + Density_j*Enthalpy_j*da_R[iSpecies]);
    }
    for (unsigned short iDim = 0; iDim < nDim; iDim++) {
      Jacobian_j[nSpecies+nDim][nSpecies+iDim] += M_F * (-dPdU_j[nSpecies+nDim]*Velocity_j[iDim]*SoundSpeed_j + Density_j*Enthalpy_j*da_R[nSpecies+iDim]);
    }
    Jacobian_j[nSpecies+nDim][nSpecies+nDim]   += M_F * ((1.0+dPdU_j[nSpecies+nDim])*SoundSpeed_j + Density_j*Enthalpy_j*da_R[nSpecies+nDim]);
    Jacobian_j[nSpecies+nDim][nSpecies+nDim+1] += M_F * (dPdU_j[nSpecies+nDim+1]*SoundSpeed_j + Density_j*Enthalpy_j*da_R[nSpecies+nDim+1]);
    for (unsigned short jVar = 0; jVar < nVar; jVar++) {
  ...
  if ((M_F < 0) || ((M_F >= 0) && (fabs(M_F) <= 1.0))) {
    if (fabs(M_R) <= 1.0) {
      ...
      /*--- Pressure ---*/
      for (unsigned short iSpecies = 0; iSpecies < nSpecies; iSpecies++)
        dP_RM[iSpecies] = dPdU_j[iSpecies];
      for (unsigned short iDim = 0; iDim < nDim; iDim++)
...
CNumerics::ResidualType<> CUpwAUSM_SLAU_Base_NEMO::ComputeResidual(const CConfig* config) {
  ...
  for (unsigned short iVar = 0; iVar < nVar; iVar++)
    Flux[iVar] = 0.5*((MassFlux_i+DissFlux_i)*Fc_L[iVar] +
		                  (MassFlux_j-DissFlux_j)*Fc_R[iVar]) * Area;

  for (unsigned short iDim = 0; iDim < nDim; iDim++)
    Flux[nSpecies+iDim] += PressureFlux[iDim] * Area;
...
CUpwAUSM_NEMO::CUpwAUSM_NEMO(unsigned short val_nDim, unsigned short val_nVar,
                             unsigned short val_nPrimVar, unsigned short val_nPrimVarGrad,
                             const CConfig* config) : 
                             CUpwAUSM_SLAU_Base_NEMO(val_nDim, val_nVar, val_nPrimVar,
                                                     val_nPrimVarGrad, config) {

}

void CUpwAUSM_NEMO::ComputeInterfaceQuantities(const CConfig* config, su2double *pressure,
                                               su2double &interface_mach,
                                               su2double *interface_soundspeed) {
  ...
  for (auto iDim = 0ul; iDim < nDim; iDim++) 
    pressure[iDim] = (P_LP + P_RM) * UnitNormal[iDim];
...
CUpwAUSMPLUSUP2_NEMO::CUpwAUSMPLUSUP2_NEMO(unsigned short val_nDim, unsigned short val_nVar,
  ...
  if (Minf < EPS) SU2_MPI::Error("AUSM+-Up2 requires a reference Mach number (\"MACH_NUMBER\") greater than 0.", CURRENT_FUNCTION);
...
void CUpwAUSMPLUSUP2_NEMO::ComputeInterfaceQuantities(const CConfig* config, su2double *pressure,
  ...
  for (auto iDim = 0ul; iDim < nDim; iDim++) {
    pressure[iDim] = (0.5*(Pressure_j+Pressure_i)+0.5*(P_LP-P_RM)*
    (Pressure_i-Pressure_j)+pFi)*UnitNormal[iDim];
  }
}
...
CUpwAUSMPLUSM_NEMO::CUpwAUSMPLUSM_NEMO(unsigned short val_nDim, unsigned short val_nVar,
  ...
  if (Minf < EPS) SU2_MPI::Error("AUSM+M requires a reference Mach number (\"MACH_NUMBER\") greater than 0.", CURRENT_FUNCTION);
...
void CUpwAUSMPLUSM_NEMO::ComputeInterfaceQuantities(const CConfig* config, su2double *pressure,
  ...
  su2double A_F;
  if (0.5*(ProjVelocity_i+ProjVelocity_j) >= 0.0) A_F = A_s*A_s/max(fabs(ProjVelocity_i), A_s);
  else                                            A_F = A_s*A_s/max(fabs(ProjVelocity_j), A_s);
  ...
  for (auto iDim =0ul; iDim < nDim; iDim++){
    ...
    P_un[iDim] = -g*Gamma_ij*0.5*(Pressure_i+Pressure_j)/A_F*
                  P_LP*P_RM*(Vel_R-Vel_L);
  }
