...
CUpwSLAU2_NEMO::~CUpwSLAU2_NEMO(void) {
...
CNumerics::ResidualType<> CUpwSLAU2_NEMO::ComputeResidual(const CConfig *config) {
  ...
  pF = 0.5 * (P_i + P_j) + 0.5 * (BetaL - BetaR) * (P_i - P_j) +
      Dissipation_ij * sqrt(aux_slau/2.0) * (BetaL + BetaR - 1.0) *
      aF * 0.5 * (rho_i +rho_j);
