...
void CNumerics::GetPMatrix(const su2double *r, const su2double *v, const su2double *k,
  ...
  su2double c2, alpha, kappa, phi2, theta;
  ...
  if (nDim == 2) {
    ...
    p[3][0] = (phi2-kappa)/Gamma_Minus_One;
    p[3][1] = (*r)*(v[0]*n[1]-v[1]*n[0]);
    p[3][2] = alpha*((phi2+c2+kappa)/Gamma_Minus_One+(*c)*theta);
    p[3][3] = alpha*((phi2+c2+kappa)/Gamma_Minus_One-(*c)*theta);
  }
  else {
    ...
    p[4][0] = (phi2-kappa)/Gamma_Minus_One*n[0]+(*r)*(v[1]*n[2]-v[2]*n[1]);
    p[4][1] = (phi2-kappa)/Gamma_Minus_One*n[1]+(*r)*(v[2]*n[0]-v[0]*n[2]);
    p[4][2] = (phi2-kappa)/Gamma_Minus_One*n[2]+(*r)*(v[0]*n[1]-v[1]*n[0]);
    p[4][3] = alpha*((phi2+c2+kappa)/Gamma_Minus_One+(*c)*theta);
    p[4][4] = alpha*((phi2+c2+kappa)/Gamma_Minus_One-(*c)*theta);
  }
