...
void CNumerics::GetPMatrix(const su2double *r, const su2double *v, const su2double *k,
  ...
  if (nDim == 2) {
    ...
    p[3][2] = alpha*((phi2+c2+(*k))/Gamma_Minus_One+kappa+(*c)*theta);
    p[3][3] = alpha*((phi2+c2+(*k))/Gamma_Minus_One+kappa-(*c)*theta);
  }
  else {
    ...
    p[4][3] = alpha*((phi2+c2+(*k))/Gamma_Minus_One+kappa+(*c)*theta);
    p[4][4] = alpha*((phi2+c2+(*k))/Gamma_Minus_One+kappa-(*c)*theta);
  }
