...
void CUpwSca_TurbSST::FinishResidualCalc(const CConfig* config) {
  ...
  const su2double Diss_rk = Lambda[0]+RoeTke*(Gamma_Minus_One-TWO3)*(0.5*Lambda[1]+0.5*Lambda[2]-Lambda[0])/RoeSoundSpeed2;
  const su2double Diss_ro = Lambda[0];
  const su2double Diss_ro_rk = RoeOmega*(Gamma_Minus_One-TWO3)*(0.5*Lambda[1]+0.5*Lambda[2]-Lambda[0])/RoeSoundSpeed2;
