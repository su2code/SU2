...
void CUpwSca_TurbSST::FinishResidualCalc(const CConfig* config) {
  ...
  if (Flux[0] >  1000*Density_i*TurbVar_i[0]) cout << "I, Lambda[0]= " << Lambda[0] << ", kLambdaG= " << RoeTke*Lambda_G << endl;
  if (Flux[0] < -1000*Density_j*TurbVar_j[0]) cout << "J, Lambda[0]= " << Lambda[0] << ", kLambdaG= " << RoeTke*Lambda_G << endl;
