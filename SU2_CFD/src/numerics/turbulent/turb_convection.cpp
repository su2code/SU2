...
void CUpwSca_TurbSST::FinishResidualCalc(const CConfig* config) {
  ...
  if (RoeSoundSpeed2 <= 0.0) {
    ...
    const su2double Diss_rk = Lambda[0]+RoeTke*(Gamma_Minus_One)*(Lambda[0]-0.5*Lambda[1]-0.5*Lambda[2])/RoeSoundSpeed2;
    const su2double Diss_ro = Lambda[0];
    const su2double Diss_ro_rk = RoeOmega*(Gamma_Minus_One)*(Lambda[0]-0.5*Lambda[1]-0.5*Lambda[2])/RoeSoundSpeed2;
