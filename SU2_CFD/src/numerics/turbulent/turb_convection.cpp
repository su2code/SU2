...
void CUpwSca_TurbSST::FinishResidualCalc(const CConfig* config) {
  ...
  Epsilon[0] = 4.0*max(fabs(Lambda[0]-ProjVel_i), 
                       fabs(ProjVel_j-Lambda[0]));
  Epsilon[1] = 4.0*max(fabs(Lambda[1]-(ProjVel_i+SoundSpeed_i)),
                       fabs((ProjVel_j+SoundSpeed_j)-Lambda[1]));
  Epsilon[2] = 4.0*max(fabs(Lambda[2]-(ProjVel_i-SoundSpeed_i)),
                       fabs((ProjVel_j-SoundSpeed_j)-Lambda[2]));
