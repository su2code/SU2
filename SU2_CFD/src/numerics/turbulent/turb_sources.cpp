...
CNumerics::ResidualType<> CSourcePieceWise_TurbSST::ComputeResidual(const CConfig* config) {
  ...
  Jacobian_i[0][0] -= TWO3*Density_i*TurbVar_i[0]*max(diverg,0.0);
  pk = max(min(pk,pkmax),0.0) - TWO3*Density_i*TurbVar_i[0]*min(diverg,0.0);

  if (!stress_limited)
    Jacobian_i[1][1] -= TWO3*Density_i*alfa_blended*zeta*max(diverg,0.0);
  pw = max(pw,0.0) - TWO3*Density_i*alfa_blended*zeta*min(diverg,0.0);
