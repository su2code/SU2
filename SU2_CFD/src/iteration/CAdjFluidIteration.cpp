...
void CAdjFluidIteration::Preprocess(COutput* output, CIntegration**** integration, CGeometry**** geometry,
  ...
  if ((InnerIter == 0) || (config[val_iZone]->GetTime_Marching() != TIME_MARCHING::STEADY)) {
    if (config[val_iZone]->GetKind_Solver() == ENUM_MAIN_SOLVER::ADJ_EULER)
      config[val_iZone]->SetGlobalParam(ENUM_MAIN_SOLVER::ADJ_EULER, RUNTIME_FLOW_SYS);
    if (config[val_iZone]->GetKind_Solver() == ENUM_MAIN_SOLVER::ADJ_NAVIER_STOKES)
      config[val_iZone]->SetGlobalParam(ENUM_MAIN_SOLVER::ADJ_NAVIER_STOKES, RUNTIME_FLOW_SYS);
    if (config[val_iZone]->GetKind_Solver() == ENUM_MAIN_SOLVER::ADJ_RANS) config[val_iZone]->SetGlobalParam(ENUM_MAIN_SOLVER::ADJ_RANS, RUNTIME_FLOW_SYS);
    ...
    if (config[val_iZone]->GetKind_Solver() == ENUM_MAIN_SOLVER::ADJ_RANS) {
      /*--- Solve the turbulence model ---*/

      config[val_iZone]->SetGlobalParam(ENUM_MAIN_SOLVER::ADJ_RANS, RUNTIME_TURB_SYS);
...
void CAdjFluidIteration::Iterate(COutput* output, CIntegration**** integration, CGeometry**** geometry,
  ...
  switch (config[val_iZone]->GetKind_Solver()) {
    case ENUM_MAIN_SOLVER::ADJ_EULER:
      config[val_iZone]->SetGlobalParam(ENUM_MAIN_SOLVER::ADJ_EULER, RUNTIME_ADJFLOW_SYS);
      break;

    case ENUM_MAIN_SOLVER::ADJ_NAVIER_STOKES:
      config[val_iZone]->SetGlobalParam(ENUM_MAIN_SOLVER::ADJ_NAVIER_STOKES, RUNTIME_ADJFLOW_SYS);
      break;
  ...
  if ((config[val_iZone]->GetKind_Solver() == ENUM_MAIN_SOLVER::ADJ_RANS) && (!config[val_iZone]->GetFrozen_Visc_Cont())) {
    /*--- Adjoint turbulence model solution ---*/

    config[val_iZone]->SetGlobalParam(ENUM_MAIN_SOLVER::ADJ_RANS, RUNTIME_ADJTURB_SYS);
