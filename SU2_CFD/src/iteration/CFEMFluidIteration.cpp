...
void CFEMFluidIteration::Iterate(COutput* output, CIntegration**** integration, CGeometry**** geometry,
  ...
  if (config[val_iZone]->GetKind_Solver() == MAIN_SOLVER::FEM_INC_EULER)
    config[val_iZone]->SetGlobalParam(MAIN_SOLVER::FEM_INC_EULER, RUNTIME_FLOW_SYS);

  if (config[val_iZone]->GetKind_Solver() == MAIN_SOLVER::FEM_INC_NAVIER_STOKES)
    config[val_iZone]->SetGlobalParam(MAIN_SOLVER::FEM_INC_NAVIER_STOKES, RUNTIME_FLOW_SYS);

  if (config[val_iZone]->GetKind_Solver() == MAIN_SOLVER::FEM_INC_RANS)
    config[val_iZone]->SetGlobalParam(MAIN_SOLVER::FEM_INC_RANS, RUNTIME_FLOW_SYS);

  if (config[val_iZone]->GetKind_Solver() == MAIN_SOLVER::FEM_INC_LES)
    config[val_iZone]->SetGlobalParam(MAIN_SOLVER::FEM_INC_LES, RUNTIME_FLOW_SYS);
