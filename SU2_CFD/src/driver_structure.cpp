  ...
  if ((euler) || (ns)) {
    ...
    for (iMGlevel = 0; iMGlevel <= config->GetnMGLevels(); iMGlevel++) {
      ...
      if (config->GetBody_Force() == YES)
        ...
        if (incompressible) {if (rank == MASTER_NODE) cout << "Driver init of CSourceIncPeriodicBodyForce." << endl; numerics_container[val_iInst][iMGlevel][FLOW_SOL][SOURCE_FIRST_TERM] = new CSourceIncPeriodicBodyForce(nDim, nVar_Flow, config);}// TK Currently not implemented for compressible flow
