...
CDriver::CDriver(char* confFile,
 ...
 if (config_container[ZONE_0]->GetBoolTurbomachinery()){
  ...
  if(config_container[ZONE_0]->GetCheckpointing()) {
    ...
    r = new Checkpointing(steps,snaps,snaps_in_ram, config_container[ZONE_0]->GetCheckpointing());
  }
...
void CDriver::StartSolverRevolve() {
    ...
    do
    {
        ...
        if (whatodo == ACTION::adjoint_step)
        {
          AdjointStep();
        }
        if (whatodo == ACTION::restore_full_checkpoint)
        {
          RestoreFullCheckpoint();
        }
        // TODO include ACTION::restore_one
        if (whatodo == ACTION::restore_single_state)
        {
          RestoreSingleState();
        }
        ...
        {
          cout << "ACTION::terminate called." << endl;
        }
    }
...
void CDiscAdjFluidDriver::Run() {
  ...
  for (iZone = 0; iZone < nZone; iZone++) {
      if (config_container[ZONE_0]->GetCheckpointing()) {
          iteration_container[iZone][INST_0]->PreprocessForCheckpointing(output, integration_container, geometry_container,
                                                     solver_container, numerics_container, config_container,
                                                     surface_movement, grid_movement, FFDBox, iZone, INST_0);
      } else {
          iteration_container[iZone][INST_0]->Preprocess(output, integration_container, geometry_container,
