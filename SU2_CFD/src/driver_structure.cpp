...
void CDriver::Solver_Preprocessing(CSolver ****solver_container, CGeometry ***geometry,
  ...
  salsa_spalart_allmaras, rc_spalart_allmaras;
  ...
  salsa_spalart_allmaras = false, rc_spalart_allmaras = false;
  ...
  if (turbulent || fem_turbulent)
    switch (config->GetKind_Turb_Model()) {
      ...
      case SA_SALSA:  salsa_spalart_allmaras = true; break;
      case SA_RC: rc_spalart_allmaras = true; break;
...
void CDriver::Solver_Postprocessing(CSolver ****solver_container, CGeometry **geometry,
  ...
  for (iMGlevel = 0; iMGlevel <= config->GetnMGLevels(); iMGlevel++) {
    ...
    if (turbulent) {
      if (spalart_allmaras || neg_spalart_allmaras || menter_sst || e_spalart_allmaras || comp_spalart_allmaras || e_comp_spalart_allmaras || salsa_spalart_allmaras || rc_spalart_allmaras) {
...
void CDriver::Numerics_Preprocessing(CNumerics *****numerics_container,
  ...
  if (turbulent) {
    ...
    switch (config->GetKind_ConvNumScheme_Turb()) {
      ...
      case SPACE_UPWIND :
        for (iMGlevel = 0; iMGlevel <= config->GetnMGLevels(); iMGlevel++) {
          if (spalart_allmaras || neg_spalart_allmaras || e_spalart_allmaras || comp_spalart_allmaras || e_comp_spalart_allmaras || salsa_spalart_allmaras || rc_spalart_allmaras) {
    ...
    for (iMGlevel = 0; iMGlevel <= config->GetnMGLevels(); iMGlevel++) {
      if (spalart_allmaras || e_spalart_allmaras || comp_spalart_allmaras || e_comp_spalart_allmaras || rc_spalart_allmaras){
      ...
      else if (salsa_spalart_allmaras) numerics_container[val_iInst][iMGlevel][TURB_SOL][VISC_TERM] = new CAvgGrad_TurbSA_SALSA(nDim, nVar_Turb, true, config);
    ...
    for (iMGlevel = 0; iMGlevel <= config->GetnMGLevels(); iMGlevel++) {
      ...
      else if (salsa_spalart_allmaras) numerics_container[val_iInst][iMGlevel][TURB_SOL][SOURCE_FIRST_TERM] = new CSourcePieceWise_TurbSA_SALSA(nDim, nVar_Turb, config);
      else if (rc_spalart_allmaras) numerics_container[val_iInst][iMGlevel][TURB_SOL][SOURCE_FIRST_TERM] = new CSourcePieceWise_TurbSA_RC(nDim, nVar_Turb, config);
    ...
    for (iMGlevel = 0; iMGlevel <= config->GetnMGLevels(); iMGlevel++) {
      ...
      else if (salsa_spalart_allmaras) {
        numerics_container[val_iInst][iMGlevel][TURB_SOL][CONV_BOUND_TERM] = new CUpwSca_TurbSA(nDim, nVar_Turb, config);
        numerics_container[val_iInst][iMGlevel][TURB_SOL][VISC_BOUND_TERM] = new CAvgGrad_TurbSA_SALSA(nDim, nVar_Turb, false, config);
      }
      else if (menter_sst) {
    ...
    switch (config->GetKind_ConvNumScheme_AdjTurb()) {
      ...
      case SPACE_UPWIND :
        for (iMGlevel = 0; iMGlevel <= config->GetnMGLevels(); iMGlevel++)
          ...
          else if (salsa_spalart_allmaras) {SU2_MPI::Error("Adjoint SALSA SA turbulence model not implemented.", CURRENT_FUNCTION);}
        break;
      default :
