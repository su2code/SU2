...
void CDriver::Solver_Preprocessing(CSolver ***solver_container, CGeometry **geometry,
  ...
  for (iMGlevel = 0; iMGlevel <= config->GetnMGLevels(); iMGlevel++) {
    ...
    if (disc_adj) {
      ...
      if(gnorm)
        solver_container[iMGlevel][GRAD_ERR_SOL] = new CGradErrSolver(geometry[iMGlevel], config, solver_container[iMGlevel][FLOW_SOL], solver_container[iMGlevel][ADJFLOW_SOL], RUNTIME_FLOW_SYS, iMGlevel);
      if (adj_turb)
...
CGradErrFluidDriver::CGradErrFluidDriver(char* confFile,
                                                 unsigned short val_nZone,
                                                 unsigned short val_nDim, SU2_Comm MPICommunicator) : CFluidDriver(confFile,
                                                                                    val_nZone,
                                                                                    val_nDim, MPICommunicator) {
...
CGradErrFluidDriver::~CGradErrFluidDriver(){
...
void CGradErrFluidDriver::DiscAdjRun() {
...
void CGradErrFluidDriver::SetRecording(unsigned short kind_recording){
  unsigned short iZone, iMesh;
  int rank = MASTER_NODE;
...
void CGradErrFluidDriver::SetAdj_ObjFunction(){
...
void CGradErrFluidDriver::SetObjFunction(){
...
void CGradErrFluidDriver::DirectRun(){
