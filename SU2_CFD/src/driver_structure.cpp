...
void CDriver::Integration_Preprocessing(CIntegration **integration_container,
  ...
  bool euler, adj_euler, ns, adj_ns, turbulent, adj_turb, poisson, wave, fem, adj_fem,
    ...
    if (rank == MASTER_NODE && ((ExtIter == 0) || unsteady )){
      ...
      if (Kind_Objective_Function == FLOW_OBJECTIVE_FUNCTION){
        ...
        case LIFT_COEFFICIENT:
          ...
          break;
        case AERO_DRAG_COEFFICIENT:
          kind_OFunction = "(Aero-drag coefficient): ";
          val_OFunction = solver_container[ZONE_FLOW][MESH_0][FLOW_SOL]->GetTotal_AeroCD();
          break;
        case RADIAL_DISTORTION:
          kind_OFunction = "(Radial distortion): ";
          val_OFunction =solver_container[ZONE_FLOW][MESH_0][FLOW_SOL]->GetTotal_RadialDistortion();
          break;
        case CIRCUMFERENTIAL_DISTORTION:
          kind_OFunction = "(Circumferential distortion): ";
          val_OFunction = solver_container[ZONE_FLOW][MESH_0][FLOW_SOL]->GetTotal_CircumferentialDistortion();
          break;
        case SIDEFORCE_COEFFICIENT:
...
//  grid_movement[ZONE_FLOW]->UpdateDualGrid(geometry_container[ZONE_FLOW][MESH_0], config_container[ZONE_FLOW]);
