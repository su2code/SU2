...
void CTurbSASolver::SetTurbGradient_L2Proj2(CGeometry *geometry, CConfig *config, CSolver *solver_flow) {
  ...
  for (iElem=0; iElem<nElem; ++iElem) {
    for (unsigned short iNode=0; iNode<3; ++iNode) {
      ...
      density     = solver_flow->node[kNode]->GetDensity();
      velocity[0] = solver_flow->node[kNode]->GetVelocity(0);
      velocity[1] = solver_flow->node[kNode]->GetVelocity(1);

      laminar_viscosity = solver_flow->node[kNode]->GetLaminarViscosity();

      nu_tilde = node[kNode]->GetSolution(0);
      dnu_tilde[0] = node[kNode]->GetGradient(0,0);
      dnu_tilde[1] = node[kNode]->GetGradient(0,1);

      Sens[iNode][0][0] = density*velocity[0]*nu_tilde - 1.5*(laminar_viscosity+density*nu_tilde)*dnu_tilde[0];
      Sens[iNode][0][1] = density*velocity[1]*nu_tilde - 1.5*(laminar_viscosity+density*nu_tilde)*dnu_tilde[1];
    }
...
void CTurbSASolver::SetTurbGradient_L2Proj3(CGeometry *geometry, CConfig *config, CSolver *solver_flow) {
  ...
  for (iElem=0; iElem<nElem; ++iElem) {
    for (unsigned short iNode=0; iNode<4; ++iNode) {
      ...
      velocity[0] = solver_flow->node[kNode]->GetVelocity(0);
      velocity[1] = solver_flow->node[kNode]->GetVelocity(1);
      velocity[2] = solver_flow->node[kNode]->GetVelocity(2);
      ...
      dnu_tilde[0] = node[kNode]->GetGradient(0,0);
      dnu_tilde[1] = node[kNode]->GetGradient(0,1);
      dnu_tilde[2] = node[kNode]->GetGradient(0,2);

      Sens[iNode][0][0] = density*velocity[0]*nu_tilde - 1.5*(laminar_viscosity+density*nu_tilde)*dnu_tilde[0];
      Sens[iNode][0][1] = density*velocity[1]*nu_tilde - 1.5*(laminar_viscosity+density*nu_tilde)*dnu_tilde[1];
      Sens[iNode][0][2] = density*velocity[2]*nu_tilde - 1.5*(laminar_viscosity+density*nu_tilde)*dnu_tilde[2];
    }
