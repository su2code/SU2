  ...
  //exit(0); //Mutation
   ...
   if (config->ShockTube() == false ){
     ...
     for (iPoint = 0; iPoint < nPoint; iPoint++) {
        ...
        pressure_l = config->GetPressure_LeftState();
        pressure_r = config->GetPressure_RightState();
        MassFrac_l = config->GetMassFrac_LeftState();
        MassFrac_r = config->GetMassFrac_RightState();
        Mach_l = config->GetMach_LeftState();
        Mach_r = config->GetMach_RightState();
        Temperature_l = config->GetTemperature_LeftState();
        Temperature_r = config->GetTemperature_RightState();
        Tve_l = config->GetTemperature_ve_LeftState();
        Tve_r = config->GetTemperature_ve_RightState();

        su2double *Mvec_l, *Mvec_r;
        Mvec_l = new su2double[nDim]; Mvec_r = new su2double[nDim];
        Mvec_l[0] = Mach_l;           Mvec_r[0] = Mach_r;

        Mvec_l[1] = Mvec_l[2] = Mvec_r[1] = Mvec_r[2] = 0.0; // 1D problem, only x dimension


        if (geometry->node[iPoint]->GetCoord(0) <= config->Get_DiafragmPosition()) 
        {

          node[iPoint] = new CTNE2EulerVariable(pressure_l, MassFrac_l, 
                                          Mvec_l, Temperature_l, 
                                          Tve_l, nDim, nVar, 
                                          nPrimVar, nPrimVarGrad, config, reactive);
        ...
        else
        {
          ...
          node[iPoint] = new CTNE2EulerVariable(pressure_r, MassFrac_r, 
                                          Mvec_r, Temperature_r, 
                                          Tve_r, nDim, nVar, 
                                          nPrimVar, nPrimVarGrad, config, reactive);
...
void CTNE2EulerSolver::SetNondimensionalization(CGeometry *geometry, CConfig *config, unsigned short iMesh) {
  ...
  //std::cout << setprecision(6)<< "Mutation: soundspeed=" << soundspeed << std::endl << std::endl;
...
//su2double Etr;
  ...
  //}
   ...
   //std::cout <<std::endl<<std::endl<< "Tve = "  << Tve <<std::endl <<std::endl<<std::endl;
   //std::cout <<std::endl<<std::endl<< "cN2 = "  << MassFrac_Inf[0] <<std::endl <<std::endl<<std::endl;
   //std::cout <<std::endl<<std::endl<< "cN = "  << MassFrac_Inf[1] <<std::endl <<std::endl<<std::endl;

   //std::cout <<std::endl<<std::endl<< "Gamma = "  << gamma <<std::endl <<std::endl<<std::endl;
...
void CTNE2EulerSolver::BC_Supersonic_Inlet(CGeometry *geometry, CSolver **solution_container,
  ...
  su2double  *xi, *Tref, *hf, *thetav, **thetae, **g, rhos, Ef, Ee, Ev, density;
  ...
  Mass_Frac[0] = 0.985801;
  Mass_Frac[1] =  0.0141988;
  ...
  su2double Gamma     = reactive->Get_Gamma(Mass_Frac, Density, Temperature, Temperature_ve);


  std::cout << "SUPERSONIC INLET "<< std::endl<< std::endl;
  std::cout << "MassFrac[0] " << Mass_Frac[0] << std::endl;
  std::cout << "MassFrac[1] " << Mass_Frac[1] << std::endl;
  std::cout << "Temperature " << Temperature << std::endl;
  std::cout << "Temperature_ve " << Temperature_ve << std::endl;
