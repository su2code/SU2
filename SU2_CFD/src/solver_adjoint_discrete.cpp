    ...
    for (iVertex = 0; iVertex < geometry->nVertex[iMarker]; iVertex++) {
      ...
      if (geometry->node[iPoint]->GetDomain()) {
        ...
        if (!time_stepping) {
          ...
          geometry->SetSensitivityTranspiration(iPoint, Sensitivity);
        } else {
          node[iPoint]->SetSensitivityTranspiration(node[iPoint]->GetSensitivityTranspiration() + Sensitivity);
          geometry->SetSensitivityTranspiration(iPoint, node[iPoint]->GetSensitivityTranspiration() + Sensitivity);
        }
