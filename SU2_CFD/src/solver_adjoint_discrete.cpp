...
void CDiscAdjSolver::GetInterfaceValues(CGeometry *geometry, CConfig *config, vector<passivedouble> &values) {
  ...
  for (iMarkerInt = 1; iMarkerInt <= nMarkerInt; ++iMarkerInt) {
    ...
    for (iVertex = 0; iVertex < nVertex; ++iVertex) {
      iPoint = geometry->vertex[iMarker][iVertex]->GetNode();
      for (iVar = 0; iVar < nVar; ++iVar)
        interfaceValues.push_back(SU2_TYPE::GetValue(node[iPoint]->GetCross_Term_Derivative(iVar)));
    }
  }
...
void CDiscAdjSolver::SetInterfaceValues(CGeometry *geometry, CConfig *config, vector<passivedouble> &values) {
  ...
  for (iVertex=0; iVertex<nVertex; ++iVertex) {
    ...
    for (iVar = 0; iVar < nVar; ++iVar)
      node[iPoint]->SetCross_Term_Derivative(iVar,interfaceValues[iVertex*nVar+iVar]);
  }
}
