...
void Solver_Preprocessing(CSolver ***solver_container, CGeometry **geometry, CConfig *config, unsigned short iZone) {
	...
	plasma_monatomic, plasma_diatomic, adj_euler, adj_tne2_euler,
	lin_euler, adj_ns, adj_tne2_ns, lin_ns, turbulent, adj_turb, lin_turb, electric, wave, fea,
...
void Integration_Preprocessing(CIntegration **integration_container, CGeometry **geometry, CConfig *config, unsigned short iZone) {
	...
	bool euler, ns, tne2_euler, tne2_ns, plasma_euler, plasma_ns, plasma_monatomic, plasma_diatomic, adj_plasma_euler, adj_plasma_ns,
	adj_euler, adj_tne2_euler, lin_euler, adj_ns, adj_tne2_ns, lin_ns, turbulent, adj_turb, lin_turb, electric, wave, fea, spalart_allmaras, menter_sst, template_solver, transition;
	...
	adj_euler = false;	adj_ns = false; adj_tne2_euler = false;	adj_tne2_ns = false; adj_turb = false;	wave = false; fea = false; spalart_allmaras = false;
...
void Numerics_Preprocessing(CNumerics ****numerics_container, CSolver ***solver_container, CGeometry **geometry,
	...
	unsigned short iMGlevel, iSol, nDim, nVar_Template = 0, nVar_Flow = 0, nVar_TNE2 = 0, nVar_Trans = 0, nVar_Adj_Flow = 0, nVar_Adj_TNE2 = 0, nVar_Plasma = 0,
	...
	adj_plasma_euler, adj_plasma_ns, adj_euler, adj_tne2_euler, lin_euler, adj_ns, adj_tne2_ns, lin_ns, turbulent,
	...
	if (adj_tne2_euler || adj_tne2_ns) {
      ...
      case SPACE_CENTERED :
        switch (config->GetKind_Centered_AdjTNE2()) {
          case NO_CENTERED : cout << "No centered scheme." << endl; break;
          case LAX : numerics_container[MESH_0][ADJTNE2_SOL][CONV_TERM] = new CCentLax_AdjTNE2(nDim, nVar_Adj_Flow, config); break;
          case JST : numerics_container[MESH_0][ADJTNE2_SOL][CONV_TERM] = new CCentJST_AdjTNE2(nDim, nVar_Adj_Flow, config); break;
          default : cout << "Centered scheme not implemented." << endl; cin.get(); break;
        }
        for (iMGlevel = 1; iMGlevel <= config->GetMGLevels(); iMGlevel++)
          numerics_container[iMGlevel][ADJTNE2_SOL][CONV_TERM] = new CCentLax_AdjTNE2(nDim, nVar_Adj_Flow, config);
        
        /*--- Definition of the boundary condition method ---*/
        for (iMGlevel = 0; iMGlevel <= config->GetMGLevels(); iMGlevel++)
          numerics_container[iMGlevel][ADJTNE2_SOL][CONV_BOUND_TERM] = new CUpwRoe_AdjTNE2(nDim, nVar_Adj_Flow, config);
        break;
      case SPACE_UPWIND :
        switch (config->GetKind_Upwind_AdjTNE2()) {
          ...
          case ROE_1ST : case ROE_2ND :
            for (iMGlevel = 0; iMGlevel <= config->GetMGLevels(); iMGlevel++) {
              numerics_container[iMGlevel][ADJTNE2_SOL][CONV_TERM] = new CUpwRoe_AdjTNE2(nDim, nVar_Adj_Flow, config);
              numerics_container[iMGlevel][ADJTNE2_SOL][CONV_BOUND_TERM] = new CUpwRoe_AdjTNE2(nDim, nVar_Adj_Flow, config);
            }
            break;
          default : cout << "Upwind scheme not implemented." << endl; cin.get(); break;
