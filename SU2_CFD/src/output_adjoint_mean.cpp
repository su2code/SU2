...
inline void CAdjFlowOutput::LoadHistoryData(CGeometry ****geometry, CSolver *****solver_container, CConfig **config,
  ...
  SetHistoryOutputField("ADJOINT_DENSITY", log10(adjflow_solver->GetRes_RMS(0)));
  SetHistoryOutputField("ADJOINT_MOMENTUM-X", log10(adjflow_solver->GetRes_RMS(1)));
  SetHistoryOutputField("ADJOINT_MOMENTUM-Y", log10(adjflow_solver->GetRes_RMS(2)));
  if (geometry[val_iZone][val_iInst][MESH_0]->GetnDim() == 3) {
    SetHistoryOutputField("ADJOINT_MOMENTUM-Z", log10(adjflow_solver->GetRes_RMS(3)));
    SetHistoryOutputField("ADJOINT_ENERGY", log10(adjflow_solver->GetRes_RMS(4)));
  } else {
    SetHistoryOutputField("ADJOINT_ENERGY", log10(adjflow_solver->GetRes_RMS(3)));    
  }
  switch(turb_model){
  case SA: case SA_NEG: case SA_E: case SA_COMP: case SA_E_COMP:
    SetHistoryOutputField("ADJOINT_NU_TILDE", log10(adjturb_solver->GetRes_RMS(0)));
    break;  
  case SST:
    SetHistoryOutputField("ADJOINT_KINETIC_ENERGY", log10(adjturb_solver->GetRes_RMS(0)));
    SetHistoryOutputField("ADJOINT_DISSIPATION",    log10(adjturb_solver->GetRes_RMS(1)));
    break;
  default: break;
  }
  SetHistoryOutputField("SENS_GEO", adjflow_solver->GetTotal_Sens_Geo());
  SetHistoryOutputField("SENS_AOA", adjflow_solver->GetTotal_Sens_AoA());
  SetHistoryOutputField("SENS_MACH", adjflow_solver->GetTotal_Sens_Mach());
  SetHistoryOutputField("SENS_PRESS", adjflow_solver->GetTotal_Sens_Press());
  SetHistoryOutputField("SENS_TEMP", adjflow_solver->GetTotal_Sens_Temp());
