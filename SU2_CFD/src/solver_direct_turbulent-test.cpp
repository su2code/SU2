...
CTurbSolver::CTurbSolver(CConfig *config) :
    CSolver() {
...
CTurbSolver::~CTurbSolver(void) {
  ...
  if (FlowPrimVar_i != NULL)
    delete[] FlowPrimVar_i;
  if (FlowPrimVar_j != NULL)
...
void CTurbSolver::Set_MPI_Solution(CGeometry *geometry, CConfig *config) {
  ...
  unsigned long iVertex, iPoint, nVertexS, nVertexR, nBufferS_Vector,
      nBufferR_Vector, nBufferS_Scalar, nBufferR_Scalar;
  su2double *Buffer_Receive_U = NULL, *Buffer_Send_U = NULL,
      *Buffer_Receive_muT = NULL, *Buffer_Send_muT = NULL;
  ...
  for (iMarker = 0; iMarker < config->GetnMarker_All(); iMarker++) {
    ...
    if ((config->GetMarker_All_KindBC(iMarker) == SEND_RECEIVE)
      ...
      nBufferS_Scalar = nVertexS;
      nBufferR_Scalar = nVertexR;
      ...
      for (iVertex = 0; iVertex < nVertexS; iVertex++) {
        ...
        for (iVar = 0; iVar < nVar; iVar++)
          Buffer_Send_U[iVar * nVertexS + iVertex] = node[iPoint]->GetSolution(
              iVar);
      }
      ...
      for (iVertex = 0; iVertex < nVertexR; iVertex++) {
        ...
        for (iVar = 0; iVar < nVar; iVar++)
          Buffer_Receive_U[iVar * nVertexR + iVertex] = Buffer_Send_U[iVar
              * nVertexR + iVertex];
      }
      ...
      for (iVertex = 0; iVertex < nVertexR; iVertex++) {
        ...
        for (iVar = 0; iVar < nVar; iVar++)
          node[iPoint]->SetSolution(iVar,
              Buffer_Receive_U[iVar * nVertexR + iVertex]);
  ...
  for (iMarker = 0; iMarker < config->GetnMarker_All(); iMarker++) {
    ...
    if ((config->GetMarker_All_KindBC(iMarker) == SEND_RECEIVE)
      ...
      for (iVertex = 0; iVertex < nVertexS; iVertex++) {
        iPoint = geometry->vertex[MarkerS][iVertex]->GetNode();
        for (iVar = 0; iVar < nVar; iVar++)
          Buffer_Send_U[iVar * nVertexS + iVertex] =
      ...
      for (iVertex = 0; iVertex < nVertexR; iVertex++) {
        ...
        for (iVar = 0; iVar < nVar; iVar++)
          node[iPoint]->SetSolution_Old(iVar,
              Buffer_Receive_U[iVar * nVertexR + iVertex]);
...
void CTurbSolver::Set_MPI_Solution_Gradient(CGeometry *geometry,
    CConfig *config) {
  unsigned short iVar, iDim, iMarker, iPeriodic_Index, MarkerS, MarkerR;
  unsigned long iVertex, iPoint, nVertexS, nVertexR, nBufferS_Vector,
      nBufferR_Vector;
  su2double rotMatrix[3][3], *angles, theta, cosTheta, sinTheta, phi, cosPhi,
      sinPhi, psi, cosPsi, sinPsi, *Buffer_Receive_Gradient = NULL,
      *Buffer_Send_Gradient = NULL;
  ...
  for (iMarker = 0; iMarker < config->GetnMarker_All(); iMarker++) {
    ...
    if ((config->GetMarker_All_KindBC(iMarker) == SEND_RECEIVE)
      ...
      nVertexS = geometry->nVertex[MarkerS];
      nVertexR = geometry->nVertex[MarkerR];
      nBufferS_Vector = nVertexS * nVar * nDim;
      nBufferR_Vector = nVertexR * nVar * nDim;
      ...
      for (iVertex = 0; iVertex < nVertexS; iVertex++) {
        ...
        for (iVar = 0; iVar < nVar; iVar++)
          for (iDim = 0; iDim < nDim; iDim++)
            Buffer_Send_Gradient[iDim * nVar * nVertexS + iVar * nVertexS
                + iVertex] = node[iPoint]->GetGradient(iVar, iDim);
      }
      ...
      for (iVertex = 0; iVertex < nVertexR; iVertex++) {
        for (iVar = 0; iVar < nVar; iVar++)
          for (iDim = 0; iDim < nDim; iDim++)
            Buffer_Receive_Gradient[iDim * nVar * nVertexR + iVar * nVertexR
                + iVertex] = Buffer_Send_Gradient[iDim * nVar * nVertexR
                + iVar * nVertexR + iVertex];
      }
      ...
      for (iVertex = 0; iVertex < nVertexR; iVertex++) {
        ...
        iPoint = geometry->vertex[MarkerR][iVertex]->GetNode();
        iPeriodic_Index =
            geometry->vertex[MarkerR][iVertex]->GetRotation_Type();
        ...
        for (iVar = 0; iVar < nVar; iVar++)
          for (iDim = 0; iDim < nDim; iDim++)
            Gradient[iVar][iDim] = Buffer_Receive_Gradient[iDim * nVar
                * nVertexR + iVar * nVertexR + iVertex];

        /*--- Need to rotate the gradients for all conserved variables. ---*/
        for (iVar = 0; iVar < nVar; iVar++) {
          if (nDim == 2) {
            Gradient[iVar][0] = rotMatrix[0][0]
                * Buffer_Receive_Gradient[0 * nVar * nVertexR + iVar * nVertexR
                    + iVertex]
                + rotMatrix[0][1]
                    * Buffer_Receive_Gradient[1 * nVar * nVertexR
                        + iVar * nVertexR + iVertex];
            Gradient[iVar][1] = rotMatrix[1][0]
                * Buffer_Receive_Gradient[0 * nVar * nVertexR + iVar * nVertexR
                    + iVertex]
                + rotMatrix[1][1]
                    * Buffer_Receive_Gradient[1 * nVar * nVertexR
                ...
                * Buffer_Receive_Gradient[0 * nVar * nVertexR + iVar * nVertexR
                ...
                + rotMatrix[0][1]
                    * Buffer_Receive_Gradient[1 * nVar * nVertexR
                        + iVar * nVertexR + iVertex]
                + rotMatrix[0][2]
                    * Buffer_Receive_Gradient[2 * nVar * nVertexR
                        + iVar * nVertexR + iVertex];
            Gradient[iVar][1] = rotMatrix[1][0]
                * Buffer_Receive_Gradient[0 * nVar * nVertexR + iVar * nVertexR
                    + iVertex]
...
void CTurbSolver::Set_MPI_Solution_Limiter(CGeometry *geometry,
    CConfig *config) {
  unsigned short iVar, iMarker, MarkerS, MarkerR;
  unsigned long iVertex, iPoint, nVertexS, nVertexR, nBufferS_Vector,
      nBufferR_Vector;
  ...
  for (iMarker = 0; iMarker < config->GetnMarker_All(); iMarker++) {
    ...
    if ((config->GetMarker_All_KindBC(iMarker) == SEND_RECEIVE)
      ...
      for (iVertex = 0; iVertex < nVertexS; iVertex++) {
        iPoint = geometry->vertex[MarkerS][iVertex]->GetNode();
        for (iVar = 0; iVar < nVar; iVar++)
          Buffer_Send_Limit[iVar * nVertexS + iVertex] =
      ...
      for (iVertex = 0; iVertex < nVertexR; iVertex++) {
        for (iVar = 0; iVar < nVar; iVar++)
          Buffer_Receive_Limit[iVar * nVertexR + iVertex] =
              Buffer_Send_Limit[iVar * nVertexR + iVertex];
      }
...
void CTurbSolver::Upwind_Residual(CGeometry *geometry,
    CSolver **solver_container, CNumerics *numerics, CConfig *config,
    unsigned short iMesh) {

  su2double *Turb_i, *Turb_j, *Limiter_i = NULL, *Limiter_j = NULL, *V_i, *V_j,
      **Gradient_i, **Gradient_j, Project_Grad_i, Project_Grad_j;
  ...
  for (iEdge = 0; iEdge < geometry->GetnEdge(); iEdge++) {
    ...
    if (second_order) {
      ...
      if (limiter) {
        Limiter_i =
            solver_container[FLOW_SOL]->node[iPoint]->GetLimiter_Primitive();
        Limiter_j =
            solver_container[FLOW_SOL]->node[jPoint]->GetLimiter_Primitive();
      }

      for (iVar = 0; iVar < solver_container[FLOW_SOL]->GetnPrimVarGrad();
          iVar++) {
        Project_Grad_i = 0.0;
        Project_Grad_j = 0.0;
        for (iDim = 0; iDim < nDim; iDim++) {
...
void CTurbSolver::Viscous_Residual(CGeometry *geometry,
    CSolver **solver_container, CNumerics *numerics, CConfig *config,
  ...
  for (iEdge = 0; iEdge < geometry->GetnEdge(); iEdge++) {
    ...
    numerics->SetTurbVar(node[iPoint]->GetSolution(),
    ...
    if (config->GetKind_Turb_Model() == SST)
      numerics->SetF1blending(node[iPoint]->GetF1blending(),
          node[jPoint]->GetF1blending());
...
void CTurbSolver::BC_Sym_Plane(CGeometry *geometry, CSolver **solver_container,
...
void CTurbSolver::ImplicitEuler_Iteration(CGeometry *geometry,
  ...
  if (!adjoint) {
    ...
    case SA:
      ...
      for (iPoint = 0; iPoint < nPointDomain; iPoint++) {
        node[iPoint]->AddClippedSolution(0,
            config->GetRelaxation_Factor_Turb() * LinSysSol[iPoint],
    ...
    case SST:
      ...
      for (iPoint = 0; iPoint < nPointDomain; iPoint++) {
        ...
        if (compressible) {
          density_old =
              solver_container[FLOW_SOL]->node[iPoint]->GetSolution_Old(0);
        ...
        for (iVar = 0; iVar < nVar; iVar++) {
          node[iPoint]->AddConservativeSolution(iVar,
              config->GetRelaxation_Factor_Turb()
                  * LinSysSol[iPoint * nVar + iVar], density, density_old,
...
void CTurbSolver::SetResidual_DualTime(CGeometry *geometry,
  ...
  if (!grid_movement) {
    ...
    for (iPoint = 0; iPoint < nPointDomain; iPoint++) {
      ...
      if (config->GetKind_Turb_Model() == SST) {
        ...
        Density_nM1 =
            solver_container[FLOW_SOL]->node[iPoint]->GetSolution_time_n1()[0];
        Density_n =
            solver_container[FLOW_SOL]->node[iPoint]->GetSolution_time_n()[0];
        Density_nP1 =
            solver_container[FLOW_SOL]->node[iPoint]->GetSolution()[0];

        for (iVar = 0; iVar < nVar; iVar++) {
          if (config->GetUnsteady_Simulation() == DT_STEPPING_1ST)
            Residual[iVar] = (Density_nP1 * U_time_nP1[iVar]
                - Density_n * U_time_n[iVar]) * Volume_nP1 / TimeStep;
          if (config->GetUnsteady_Simulation() == DT_STEPPING_2ND)
            Residual[iVar] = (3.0 * Density_nP1 * U_time_nP1[iVar]
                - 4.0 * Density_n * U_time_n[iVar]
                + 1.0 * Density_nM1 * U_time_nM1[iVar]) * Volume_nP1
                / (2.0 * TimeStep);
        }
        ...
        for (iVar = 0; iVar < nVar; iVar++) {
          if (config->GetUnsteady_Simulation() == DT_STEPPING_1ST)
            Residual[iVar] = (U_time_nP1[iVar] - U_time_n[iVar]) * Volume_nP1
                / TimeStep;
          if (config->GetUnsteady_Simulation() == DT_STEPPING_2ND)
    ...
    for (iEdge = 0; iEdge < geometry->GetnEdge(); iEdge++) {
      ...
      Residual_GCL = 0.0;
      for (iDim = 0; iDim < nDim; iDim++)
        Residual_GCL += 0.5 * (GridVel_i[iDim] + GridVel_j[iDim])
            * Normal[iDim];
      ...
      if (config->GetKind_Turb_Model() == SST) {
        Density_n =
            solver_container[FLOW_SOL]->node[jPoint]->GetSolution_time_n()[0];
        for (iVar = 0; iVar < nVar; iVar++)
          Residual[iVar] = Density_n * U_time_n[iVar] * Residual_GCL;
      } else {
        for (iVar = 0; iVar < nVar; iVar++)
    ...
    for (iMarker = 0; iMarker < geometry->GetnMarker(); iMarker++) {
      for (iVertex = 0; iVertex < geometry->GetnVertex(iMarker); iVertex++) {
        ...
        Residual_GCL = 0.0;
        for (iDim = 0; iDim < nDim; iDim++)
          Residual_GCL -= 0.5 * (GridVel_i[iDim] + GridVel_i[iDim])
              * Normal[iDim];
    ...
    for (iPoint = 0; iPoint < nPointDomain; iPoint++) {
      ...
      if (config->GetKind_Turb_Model() == SST) {
        ...
        for (iVar = 0; iVar < nVar; iVar++) {
          ...
          if (config->GetUnsteady_Simulation() == DT_STEPPING_2ND)
            Residual[iVar] = (Density_nP1 * U_time_nP1[iVar]
                - Density_n * U_time_n[iVar])
                * (3.0 * Volume_nP1 / (2.0 * TimeStep))
                + (Density_nM1 * U_time_nM1[iVar] - Density_n * U_time_n[iVar])
                    * (Volume_nM1 / (2.0 * TimeStep));
        }
        ...
        for (iVar = 0; iVar < nVar; iVar++) {
          if (config->GetUnsteady_Simulation() == DT_STEPPING_1ST)
            Residual[iVar] = (U_time_nP1[iVar] - U_time_n[iVar])
                * (Volume_nP1 / TimeStep);
          if (config->GetUnsteady_Simulation() == DT_STEPPING_2ND)
            Residual[iVar] = (U_time_nP1[iVar] - U_time_n[iVar])
                * (3.0 * Volume_nP1 / (2.0 * TimeStep))
                + (U_time_nM1[iVar] - U_time_n[iVar])
                    * (Volume_nM1 / (2.0 * TimeStep));
        }
      }
...
CTurbSASolver::CTurbSASolver(CGeometry *geometry, CConfig *config,
  ...
  su2double Density_Inf, Viscosity_Inf, Factor_nu_Inf, Factor_nu_Engine,
      dull_val;
  ...
  if (!restart || (iMesh != MESH_0)) {
    for (iPoint = 0; iPoint < nPoint; iPoint++)
      node[iPoint] = new CTurbSAVariable(nu_tilde_Inf, muT_Inf, nDim, nVar,
          config);
  } else {
    ...
    su2double Density, StaticEnergy, Laminar_Viscosity, nu, nu_hat, muT = 0.0,
        U[5];
    ...
    while (getline(restart_file, text_line)) {
      ...
      if (iPoint_Local >= 0) {
        if (compressible) {
          if (nDim == 2)
            point_line >> index >> dull_val >> dull_val >> U[0] >> U[1] >> U[2]
                >> U[3] >> Solution[0];
          if (nDim == 3)
            point_line >> index >> dull_val >> dull_val >> dull_val >> U[0]
                >> U[1] >> U[2] >> U[3] >> U[4] >> Solution[0];

          Density = U[0];
          if (nDim == 2)
            StaticEnergy = U[3] / U[0]
                - (U[1] * U[1] + U[2] * U[2]) / (2.0 * U[0] * U[0]);
//            Pressure = Gamma_Minus_One*(U[3] - (U[1]*U[1] + U[2]*U[2])/(2.0*U[0]));
          else
            StaticEnergy = U[4] / U[0]
...
//          Laminar_Viscosity = Laminar_Viscosity/Viscosity_Ref;
        ...
        if (incompressible) {
          if (nDim == 2)
            point_line >> index >> dull_val >> dull_val >> dull_val >> dull_val
                >> dull_val >> Solution[0];
          if (nDim == 3)
            point_line >> index >> dull_val >> dull_val >> dull_val >> dull_val
                >> dull_val >> dull_val >> dull_val >> Solution[0];
          muT = muT_Inf;
        }
        ...
        if (freesurface) {
          if (nDim == 2)
            point_line >> index >> dull_val >> dull_val >> dull_val >> dull_val
                >> dull_val >> dull_val >> Solution[0];
          if (nDim == 3)
            point_line >> index >> dull_val >> dull_val >> dull_val >> dull_val
                >> dull_val >> dull_val >> dull_val >> dull_val >> Solution[0];
          muT = muT_Inf;
        }

        /*--- Instantiate the solution at this node, note that the eddy viscosity should be recomputed ---*/
        node[iPoint_Local] = new CTurbSAVariable(Solution[0], muT, nDim, nVar,
            config);
      }
      iPoint_Global++;
    }
    ...
    for (iPoint = nPointDomain; iPoint < nPoint; iPoint++) {
      node[iPoint] = new CTurbSAVariable(Solution[0], muT_Inf, nDim, nVar,
          config);
    }
...
void CTurbSASolver::Preprocessing(CGeometry *geometry,
...
void CTurbSASolver::Postprocessing(CGeometry *geometry,
  ...
  for (iPoint = 0; iPoint < nPoint; iPoint++) {
    ...
    if (neg_spalart_allmaras && (muT < 0.0))
      muT = 0.0;
...
void CTurbSASolver::Source_Residual(CGeometry *geometry,
    CSolver **solver_container, CNumerics *numerics, CNumerics *second_numerics,
  ...
  for (iPoint = 0; iPoint < nPointDomain; iPoint++) {
    ...
    if (transition) {
      numerics->SetIntermittency(
          solver_container[TRANS_SOL]->node[iPoint]->GetIntermittency());
    }
    ...
    if (freesurface) {
      LevelSet = solver_container[FLOW_SOL]->node[iPoint]->GetSolution(
          nDim + 1);
      if (LevelSet > -epsilon)
        for (iVar = 0; iVar < nVar; iVar++)
          Residual[iVar] = 0.0;
...
void CTurbSASolver::Source_Template(CGeometry *geometry,
    CSolver **solver_container, CNumerics *numerics, CConfig *config,
    unsigned short iMesh) {
...
void CTurbSASolver::BC_Isothermal_Wall(CGeometry *geometry,
    CSolver **solver_container, CNumerics *conv_numerics,
    CNumerics *visc_numerics, CConfig *config, unsigned short val_marker) {
  unsigned long iPoint, iVertex;
  unsigned short iVar;
...
void CTurbSASolver::BC_Far_Field(CGeometry *geometry,
    CSolver **solver_container, CNumerics *conv_numerics,
    CNumerics *visc_numerics, CConfig *config, unsigned short val_marker) {
  ...
  for (iVertex = 0; iVertex < geometry->nVertex[val_marker]; iVertex++) {
    ...
    if (geometry->node[iPoint]->GetDomain()) {
      ...
      V_infty = solver_container[FLOW_SOL]->GetCharacPrimVar(val_marker,
          iVertex);
...
void CTurbSASolver::BC_Inlet(CGeometry *geometry, CSolver **solver_container,
    CNumerics *conv_numerics, CNumerics *visc_numerics, CConfig *config,
    unsigned short val_marker) {
  ...
  for (iVertex = 0; iVertex < geometry->nVertex[val_marker]; iVertex++) {
    ...
    if (geometry->node[iPoint]->GetDomain()) {
      ...
      V_inlet = solver_container[FLOW_SOL]->GetCharacPrimVar(val_marker,
          iVertex);
...
void CTurbSASolver::BC_Outlet(CGeometry *geometry, CSolver **solver_container,
    CNumerics *conv_numerics, CNumerics *visc_numerics, CConfig *config,
    unsigned short val_marker) {
  ...
  for (iVertex = 0; iVertex < geometry->nVertex[val_marker]; iVertex++) {
    ...
    if (geometry->node[iPoint]->GetDomain()) {
      ...
      V_outlet = solver_container[FLOW_SOL]->GetCharacPrimVar(val_marker,
          iVertex);
...
void CTurbSASolver::BC_Engine_Inflow(CGeometry *geometry,
    CSolver **solver_container, CNumerics *conv_numerics,
    CNumerics *visc_numerics, CConfig *config, unsigned short val_marker) {
  ...
  for (iVertex = 0; iVertex < geometry->nVertex[val_marker]; iVertex++) {
    ...
    if (geometry->node[iPoint]->GetDomain()) {
      ...
      V_inflow = solver_container[FLOW_SOL]->GetCharacPrimVar(val_marker,
          iVertex);
      ...
      conv_numerics->SetTurbVar(node[iPoint]->GetSolution(),
      ...
      visc_numerics->SetTurbVar(node[iPoint]->GetSolution(),
          node[iPoint]->GetSolution());
      visc_numerics->SetTurbVarGradient(node[iPoint]->GetGradient(),
          node[iPoint]->GetGradient());
...
void CTurbSASolver::BC_Engine_Exhaust(CGeometry *geometry,
    CSolver **solver_container, CNumerics *conv_numerics,
    CNumerics *visc_numerics, CConfig *config, unsigned short val_marker) {
  ...
  for (iVertex = 0; iVertex < geometry->nVertex[val_marker]; iVertex++) {
    ...
    if (geometry->node[iPoint]->GetDomain()) {
      ...
      V_exhaust = solver_container[FLOW_SOL]->GetCharacPrimVar(val_marker,
          iVertex);
...
void CTurbSASolver::BC_Engine_Bleed(CGeometry *geometry,
    CSolver **solver_container, CNumerics *conv_numerics,
    CNumerics *visc_numerics, CConfig *config, unsigned short val_marker) {
  ...
  for (iVertex = 0; iVertex < geometry->nVertex[val_marker]; iVertex++) {
    ...
    if (geometry->node[iPoint]->GetDomain()) {
      ...
      V_bleed = solver_container[FLOW_SOL]->GetCharacPrimVar(val_marker,
          iVertex);
...
void CTurbSASolver::BC_Interface_Boundary(CGeometry *geometry,
    CSolver **solver_container, CNumerics *numerics, CConfig *config) {
...
void CTurbSASolver::BC_NearField_Boundary(CGeometry *geometry,
    CSolver **solver_container, CNumerics *numerics, CConfig *config) {
  //  unsigned long iVertex, iPoint, jPoint;
  //  unsigned short iVar, iDim;
  //
...
void CTurbSASolver::LoadRestart(CGeometry **geometry, CSolver ***solver,
  ...
  if (restart_file.fail()) {
    if (rank == MASTER_NODE)
      cout << "There is no flow restart file!! " << restart_filename.data()
          << "." << endl;
    exit(EXIT_FAILURE);
  }
  ...
  /*--- First, set all indices to a negative value by default ---*/
  for (iPoint = 0; iPoint < geometry[MESH_0]->GetGlobal_nPointDomain();
      iPoint++) {
    Global2Local[iPoint] = -1;
  }
  ...
  solver[MESH_0][TURB_SOL]->Postprocessing(geometry[MESH_0], solver[MESH_0],
      config, MESH_0);
  ...
  for (iMesh = 1; iMesh <= config->GetnMGLevels(); iMesh++) {
    for (iPoint = 0; iPoint < geometry[iMesh]->GetnPoint(); iPoint++) {
      ...
      for (iVar = 0; iVar < nVar; iVar++)
        Solution[iVar] = 0.0;
      for (iChildren = 0;
          iChildren < geometry[iMesh]->node[iPoint]->GetnChildren_CV();
          iChildren++) {
        Point_Fine = geometry[iMesh]->node[iPoint]->GetChildren_CV(iChildren);
        Area_Children = geometry[iMesh - 1]->node[Point_Fine]->GetVolume();
        Solution_Fine =
            solver[iMesh - 1][TURB_SOL]->node[Point_Fine]->GetSolution();
        for (iVar = 0; iVar < nVar; iVar++) {
    ...
    solver[iMesh][TURB_SOL]->Postprocessing(geometry[iMesh], solver[iMesh],
        config, iMesh);
  }
...
CTurbSSTSolver::CTurbSSTSolver(CGeometry *geometry, CConfig *config,
  ...
  constants[7] = 0.31;   //a1
  constants[8] = constants[4] / constants[6]
      - constants[2] * 0.41 * 0.41 / sqrt(constants[6]);  //alfa_1
  constants[9] = constants[5] / constants[6]
      - constants[3] * 0.41 * 0.41 / sqrt(constants[6]);  //alfa_2
  ...
  if (!restart || (iMesh != MESH_0)) {
    for (iPoint = 0; iPoint < nPoint; iPoint++)
      node[iPoint] = new CTurbSSTVariable(kine_Inf, omega_Inf, muT_Inf, nDim,
          nVar, constants, config);
  } else {
    ...
    while (getline(restart_file, text_line)) {
      ...
      if (iPoint_Local >= 0) {
        ...
        if (compressible) {
          if (nDim == 2)
            point_line >> index >> dull_val >> dull_val >> dull_val >> dull_val
                >> dull_val >> dull_val >> Solution[0] >> Solution[1];
          if (nDim == 3)
            point_line >> index >> dull_val >> dull_val >> dull_val >> dull_val
                >> dull_val >> dull_val >> dull_val >> dull_val >> Solution[0]
                >> Solution[1];
        ...
        if (incompressible) {
          if (nDim == 2)
            point_line >> index >> dull_val >> dull_val >> dull_val >> dull_val
                >> dull_val >> Solution[0] >> Solution[1];
          if (nDim == 3)
            point_line >> index >> dull_val >> dull_val >> dull_val >> dull_val
                >> dull_val >> dull_val >> dull_val >> Solution[0]
                >> Solution[1];
        ...
        node[iPoint_Local] = new CTurbSSTVariable(Solution[0], Solution[1],
            muT_Inf, nDim, nVar, constants, config);
      }
      iPoint_Global++;
    }
    ...
    /*--- Instantiate the variable class with an arbitrary solution
     ...
     because a send/recv is performed immediately in the solver. ---*/
    for (iPoint = nPointDomain; iPoint < nPoint; iPoint++) {
      node[iPoint] = new CTurbSSTVariable(Solution[0], Solution[1], muT_Inf,
...
void CTurbSSTSolver::Preprocessing(CGeometry *geometry,
    CSolver **solver_container, CConfig *config, unsigned short iMesh,
    unsigned short iRKStep, unsigned short RunTime_EqSystem, bool Output) {
...
void CTurbSSTSolver::Postprocessing(CGeometry *geometry,
    CSolver **solver_container, CConfig *config, unsigned short iMesh) {
...
void CTurbSSTSolver::Source_Residual(CGeometry *geometry,
    CSolver **solver_container, CNumerics *numerics, CNumerics *second_numerics,
    CConfig *config, unsigned short iMesh) {
...
void CTurbSSTSolver::Source_Template(CGeometry *geometry,
    CSolver **solver_container, CNumerics *numerics, CConfig *config,
    unsigned short iMesh) {

}

void CTurbSSTSolver::BC_HeatFlux_Wall(CGeometry *geometry,
    CSolver **solver_container, CNumerics *conv_numerics,
    CNumerics *visc_numerics, CConfig *config, unsigned short val_marker) {
  ...
  for (iVertex = 0; iVertex < geometry->nVertex[val_marker]; iVertex++) {
    ...
    if (geometry->node[iPoint]->GetDomain()) {
      ...
      distance = 0.0;
      for (iDim = 0; iDim < nDim; iDim++) {
        distance += (geometry->node[iPoint]->GetCoord(iDim)
            - geometry->node[jPoint]->GetCoord(iDim))
            * (geometry->node[iPoint]->GetCoord(iDim)
                - geometry->node[jPoint]->GetCoord(iDim));
      }
      ...
      Solution[0] = 0.0;
      Solution[1] = 60.0 * laminar_viscosity
          / (density * beta_1 * distance * distance);
...
void CTurbSSTSolver::BC_Isothermal_Wall(CGeometry *geometry,
    CSolver **solver_container, CNumerics *conv_numerics,
    CNumerics *visc_numerics, CConfig *config, unsigned short val_marker) {
...
void CTurbSSTSolver::BC_Far_Field(CGeometry *geometry,
    CSolver **solver_container, CNumerics *conv_numerics,
    CNumerics *visc_numerics, CConfig *config, unsigned short val_marker) {
...
void CTurbSSTSolver::BC_Inlet(CGeometry *geometry, CSolver **solver_container,
    CNumerics *conv_numerics, CNumerics *visc_numerics, CConfig *config,
    unsigned short val_marker) {
  ...
  su2double xCoord, yCoord, y_0, y_perio, Physical_dt, Physical_t, Total_t,
      t_perio, Period, Boundary_Vel, kine_boundary, omega_boundary;
  su2double cf_kine[12] = { 3.89730829e+18, -4.55651664e+18, 1.77708922e+18,
      -3.48765006e+17, 3.95506554e+16, -2.72309170e+15, 1.13880635e+14,
      -2.76896802e+12, 3.51712578e+10, -1.84279953e+08, 2.72687598e+05,
      2.55438016e+02 };
  su2double cf_omega[12] = { 1.21948682e+20, -2.12517413e+20, 9.24840370e+19,
      -1.90757174e+19, 2.22023659e+18, -1.54813496e+17, 6.48753320e+15,
      -1.56063222e+14, 1.91633360e+12, -9.13246014e+09, 1.19810669e+07,
      3.25130911e+04 };
  ...
  for (iVertex = 0; iVertex < geometry->nVertex[val_marker]; iVertex++) {
    ...
    if (geometry->node[iPoint]->GetDomain()) {
      ...
      if (HARMONIC_BALANCE) {
        ...
        Physical_dt = (su2double) config->GetHarmonicBalance_Period()
            / (su2double) (config->GetnTimeInstances());
      } else {
      ...
      su2double rho_dim, *Vel_dim, muLam_dim, Intensity, viscRatio, kine_dim,
          omega_dim, omega_dim_fator, kine_dim_factor;

      rho_dim = config->GetDensity_FreeStream();
      Vel_dim = config->GetVelocity_FreeStream();
      muLam_dim = config->GetViscosity_FreeStream();
      Intensity = config->GetTurbulenceIntensity_FreeStream();
      viscRatio = config->GetTurb2LamViscRatio_FreeStream();

      su2double VelMag = 0;
      for (iDim = 0; iDim < nDim; iDim++)
        VelMag += Vel_dim[iDim] * Vel_dim[iDim];
      VelMag = sqrt(VelMag);

      kine_dim = 3.0 / 2.0 * (VelMag * VelMag * Intensity * Intensity);
      omega_dim = rho_dim * kine_dim / (muLam_dim * viscRatio);

      kine_dim_factor = kine_Inf / kine_dim;
      omega_dim_fator = kine_Inf / kine_dim;

      Solution_j[0] = fabs(kine_boundary) * kine_dim_factor;  //kine_Inf;
      Solution_j[1] = fabs(omega_boundary) * omega_dim_fator;  ////omega_Inf;
...
//      };
      ...
      /*--- Menter's first blending function ---*/
      visc_numerics->SetF1blending(node[iPoint]->GetF1blending(),
          node[iPoint]->GetF1blending());
...
void CTurbSSTSolver::BC_Outlet(CGeometry *geometry, CSolver **solver_container,
    CNumerics *conv_numerics, CNumerics *visc_numerics, CConfig *config,
    unsigned short val_marker) {
