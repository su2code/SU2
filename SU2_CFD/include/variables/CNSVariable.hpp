...
//  CNSVariable(su2double *val_solution, unsigned short val_nDim, unsigned short val_nvar, CConfig *config);
   ...
   * \param[in] config - Definition of the particular problem.
   */
  CNSVariable(Idx_t npoint, Idx_t ndim, Idx_t nvar, CConfig *config);
   ...
   */
  inline void SetSpecificHeatCp(Idx_t iPoint, su2double val_Cp) { Primitive(iPoint,nDim+8) = val_Cp; }
   ...
   */
  inline void SetEddyViscosity(Idx_t iPoint, su2double eddy_visc) { Primitive(iPoint,nDim+6) = eddy_visc; }

  /*!
   * \brief Get the laminar viscosity of the flow.
   * \return Value of the laminar viscosity of the flow.
   */
  inline su2double GetLaminarViscosity(Idx_t iPoint) const { return Primitive(iPoint,nDim+5); }

  /*!
   * \brief Get the thermal conductivity of the flow.
   * \return Value of the laminar viscosity of the flow.
   */
  inline su2double GetThermalConductivity(Idx_t iPoint) const { return Primitive(iPoint,nDim+7); }

  /*!
   * \brief Get the eddy viscosity of the flow.
   * \return The eddy viscosity of the flow.
   */
  inline su2double GetEddyViscosity(Idx_t iPoint) const { return Primitive(iPoint,nDim+6); }

  /*!
   * \brief Get the specific heat at constant P of the flow.
   * \return Value of the specific heat at constant P  of the flow.
   */
  inline su2double GetSpecificHeatCp(Idx_t iPoint) const { return Primitive(iPoint,nDim+8); }

  /*!
   * \brief Set the temperature at the wall
   */
  inline void SetWallTemperature(Idx_t iPoint, su2double temperature_wall) {
   ...
   */
  inline void SetdTdrho_e(Idx_t iPoint, su2double dTdrho_e) { Secondary(iPoint,2) = dTdrho_e;}

  /*!
   * \brief Set the derivative of temperature with respect to internal energy (at constant density).
   */
  inline void SetdTde_rho(Idx_t iPoint, su2double dTde_rho) { Secondary(iPoint,3) = dTde_rho;}

  /*!
   * \brief Set the derivative of laminar viscosity with respect to density (at constant temperature).
   */
  inline void Setdmudrho_T(Idx_t iPoint, su2double dmudrho_T) { Secondary(iPoint,4) = dmudrho_T;}

  /*!
   * \brief Set the derivative of laminar viscosity with respect to temperature (at constant density).
   */
  inline void SetdmudT_rho(Idx_t iPoint, su2double dmudT_rho) { Secondary(iPoint,5) = dmudT_rho;}

  /*!
   * \brief Set the derivative of thermal conductivity with respect to density (at constant temperature).
   */
  inline void Setdktdrho_T(Idx_t iPoint, su2double dktdrho_T) { Secondary(iPoint,6) = dktdrho_T;}

  /*!
   * \brief Set the derivative of thermal conductivity with respect to temperature (at constant density).
   */
  inline void SetdktdT_rho(Idx_t iPoint, su2double dktdT_rho) { Secondary(iPoint,7) = dktdT_rho;}
   ...
   */
  inline void SetTauWall(Idx_t iPoint, su2double val_tau_wall) { Tau_Wall(iPoint) = val_tau_wall; }
   ...
   */
  inline su2double GetTauWall(Idx_t iPoint) const { return Tau_Wall(iPoint); }
   ...
   */
  void SetRoe_Dissipation_NTS(Idx_t iPoint, su2double val_delta, su2double val_const_DES);

  /*!
   * \brief Set the new solution for Roe Dissipation.
   */
  void SetRoe_Dissipation_FD(Idx_t iPoint, su2double wall_distance);

  /*!
   * \brief Get the Roe Dissipation Coefficient.
   * \return Value of the Roe Dissipation.
   */
  inline su2double GetRoe_Dissipation(Idx_t iPoint) const { return Roe_Dissipation(iPoint); }

  /*!
   * \brief Set the Roe Dissipation Coefficient.
   * \param[in] val_dissipation - Value of the Roe dissipation factor.
   */
  inline void SetRoe_Dissipation(Idx_t iPoint, su2double val_dissipation) {
