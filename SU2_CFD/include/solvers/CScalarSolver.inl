...
void CScalarSolver<VariableType>::UpdateCompactFlowPrimitives(CSolver** solver_container,
  ...
  /*--- The driver only enables this for the bounded-scalar scheme on a sufficiently unstructured
   *    3D mesh (see CDriver::InstantiateTurbulentNumerics/InstantiateSpeciesNumerics), in which
   *    case velocity is never read and is dropped from the compact container entirely. Otherwise
   *    Upwind_Residual/Viscous_Residual read straight from the full flow primitives, as before. ---*/
  if (!numerics_container[CONV_TERM]->GetCompactPrimitives()) return;

  auto* flowNodes = su2staticcast_p<CFlowVariable*>(solver_container[FLOW_SOL]->GetNodes());
  const auto& primitives = flowNodes->GetPrimitive();

  CompactFlowPrimitives.resize(nPoint, 3);

  SU2_OMP_FOR_STAT(omp_chunk_size)
  for (auto iPoint = 0ul; iPoint < nPoint; ++iPoint) {
    CompactFlowPrimitives(iPoint, 0) = primitives(iPoint, prim_idx.Density());
    CompactFlowPrimitives(iPoint, 1) = primitives(iPoint, prim_idx.LaminarViscosity());
    CompactFlowPrimitives(iPoint, 2) = primitives(iPoint, prim_idx.EddyViscosity());
  }
  END_SU2_OMP_FOR
}
...
void CScalarSolver<VariableType>::Upwind_Residual(CGeometry* geometry, CSolver** solver_container,
  ...
  UpdateCompactFlowPrimitives(solver_container, numerics_container);
  ...
  /*--- Whether this solver reads flow primitives from the compact container (see
   *    UpdateCompactFlowPrimitives) instead of the full flow primitives directly. ---*/
  const bool useCompact = numerics->GetCompactPrimitives();
  ...
  for (auto color : EdgeColoring) {
    ...
    for (auto k = 0ul; k < color.size; ++k) {
      ...
      const auto V_i = useCompact ? CompactFlowPrimitives[iPoint] : flowNodes->GetPrimitive(iPoint);
      const auto V_j = useCompact ? CompactFlowPrimitives[jPoint] : flowNodes->GetPrimitive(jPoint);
      ...
      if (muscl || musclFlow) {
        ...
        if (musclFlow && !bounded_scalar) {
           ...
           * of being computed from the primitive flow variables. This branch never runs when
           * useCompact is set (that requires bounded_scalar), so V_i/V_j here are always the full,
           * unreconstructed flow primitives. ---*/
