...
void CFVMFlowSolverBase<V, R>::BC_Sym_Plane(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
  ...
  su2double Area, ProjVelocity_i, *V_reflected, *V_domain, Normal[MAXNDIM] = {0.0}, UnitNormal[MAXNDIM] = {0.0}, Gamma_domain;
  ...
  for (iVertex = 0; iVertex < geometry->nVertex[val_marker]; iVertex++) {
    ...
    if (geometry->nodes->GetDomain(iPoint)) {
      ...
      conv_numerics->SetGamma(Gamma_domain, Gamma_domain);
        ...
        if (geometry->nodes->GetDomain(iPoint)) {
          ...
          for (jVertex = 0; jVertex < nDonorVertex; jVertex++) {
            ...
            if (FlowRegime == COMPRESSIBLE) {
              if (!(config->GetKind_FluidModel() == STANDARD_AIR || config->GetKind_FluidModel() == IDEAL_GAS || config->GetKind_FluidModel() == THERMALLY_PERFECT)) {
