...
su2double CFVMFlowSolverBase<V,R>::EvaluateCommonConstrFunc(const CConfig& config, unsigned short iConstr) const {
  ...
  for (auto iMarker = 0u; iMarker < config.GetnMarker_Monitoring(); iMarker++) {
    ...
    switch (config.GetKind_ConstrFunc(iConstr)) {
      case DRAG_COEFFICIENT:
        constFun += SurfaceCoeff.CD[iMarker];
        break;
      case LIFT_COEFFICIENT:
        constFun += SurfaceCoeff.CL[iMarker];
        break;
      case SIDEFORCE_COEFFICIENT:
        constFun += SurfaceCoeff.CSF[iMarker];
        break;
      case MOMENT_X_COEFFICIENT:
        constFun += SurfaceCoeff.CMx[iMarker];
        break;
      case MOMENT_Y_COEFFICIENT:
        constFun += SurfaceCoeff.CMy[iMarker];
        break;
      case MOMENT_Z_COEFFICIENT:
        constFun += SurfaceCoeff.CMz[iMarker];
        break;
      case FORCE_X_COEFFICIENT:
        constFun += SurfaceCoeff.CFx[iMarker];
        break;
      case FORCE_Y_COEFFICIENT:
        constFun += SurfaceCoeff.CFy[iMarker];
        break;
      case FORCE_Z_COEFFICIENT:
        constFun += SurfaceCoeff.CFz[iMarker];
        break;
      case TOTAL_HEATFLUX:
        constFun += Surface_HF_Visc[iMarker];
        break;
      case MAXIMUM_HEATFLUX:
        constFun += Surface_MaxHF_Visc[iMarker];
        break;
      default:
        break;
    }
  ...
  switch (config.GetKind_ConstrFunc(iConstr)) {
    case EFFICIENCY:
      constFun += TotalCoeff.CEff;
      break;
    case INVERSE_DESIGN_PRESSURE:
      constFun += Total_CpDiff;
      break;
    case INVERSE_DESIGN_HEATFLUX:
      constFun += Total_HeatFluxDiff;
      break;
    case THRUST_COEFFICIENT:
      constFun += TotalCoeff.CT;
      break;
    case TORQUE_COEFFICIENT:
      constFun += TotalCoeff.CQ;
      break;
    case FIGURE_OF_MERIT:
      constFun += TotalCoeff.CMerit;
      break;
    case SURFACE_TOTAL_PRESSURE:
      constFun += config.GetSurface_TotalPressure(0);
      break;
    case SURFACE_STATIC_PRESSURE:
      constFun += config.GetSurface_Pressure(0);
      break;
    case SURFACE_MASSFLOW:
      constFun += config.GetSurface_MassFlow(0);
      break;
    case SURFACE_UNIFORMITY:
      constFun += config.GetSurface_Uniformity(0);
      break;
    case SURFACE_SECONDARY:
      constFun += config.GetSurface_SecondaryStrength(0);
      break;
    case SURFACE_MOM_DISTORTION:
      constFun += config.GetSurface_MomentumDistortion(0);
      break;
    case SURFACE_SECOND_OVER_UNIFORM:
      constFun += config.GetSurface_SecondOverUniform(0);
      break;
    case SURFACE_PRESSURE_DROP:
      constFun += config.GetSurface_PressureDrop(0);
      break;
    case CUSTOM_OBJFUNC:
      constFun += Total_Custom_ObjFunc;
      break;
    default:
      break;
  }
