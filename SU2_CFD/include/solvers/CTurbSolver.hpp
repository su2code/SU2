...
protected:
  ...
  enum : size_t {MAXNVARFLOW = 6};     /*!< \brief Max number of flow variables, used in some static arrays. */
...
private:
   ...
   * \param[in] turbvar_l - Turbulent vector extrapolated from node i to face.
   * \param[in] turbvar_r - Turbulent vector extrapolated from node j to face.
