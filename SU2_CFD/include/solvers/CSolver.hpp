...
class CSolver {
 protected:
  ...
  unsigned short MGLevel;             /*!< \brief Multi-grid level of this solver object. */
  ...
  su2activematrix
      Point_Max_Coord; /*!< \brief Vector with pointers to the coords of the maximal residual for each variable. */
  su2activematrix
      Point_Max_Coord_BGS; /*!< \brief Vector with pointers to the coords of the maximal residual for each variable. */
  ...
  bool rotate_periodic; /*!< \brief Flag that controls whether the periodic solution needs to be rotated for solver. */
  bool implicit_periodic; /*!< \brief Flag that controls whether the implicit system should be treated by the periodic
                             BC comms. */
  bool dynamic_grid;      /*!< \brief Flag that determines whether the grid is dynamic (moving or deforming + grid
                             velocities). */

  vector<su2activematrix>
      VertexTraction; /*- Temporary, this will be moved to a new postprocessing structure once in place -*/
 ...
 public:
  ...
  inline CVariable* GetNodes() {
    assert(base_nodes != nullptr &&
           "CSolver::base_nodes was not set properly, see brief for CSolver::SetBaseClassPointerToNodes()");
    return base_nodes;
  }
  ...
  /*!
   * \brief Routine to load a solver quantity into the data structures for MPI point-to-point communication and to
   * launch non-blocking sends and recvs.
   * \param[in] geometry - Geometrical definition of the problem.
   * \param[in] config - Definition of the particular problem.
   * \param[in] commType - Enumerated type for the quantity to be communicated.
   */
  void InitiateComms(CGeometry* geometry, const CConfig* config, unsigned short commType);

  /*!
   * \brief Routine to complete the set of non-blocking communications launched by InitiateComms() and unpacking of the
   * data in the solver class.
   * \param[in] geometry - Geometrical definition of the problem.
   * \param[in] config - Definition of the particular problem.
   ...
   */
  void GetPeriodicCommCountAndType(const CConfig* config, unsigned short commType, unsigned short& COUNT_PER_POINT,
                                   unsigned short& MPI_TYPE, unsigned short& ICOUNT, unsigned short& JCOUNT) const;

  /*!
   * \brief Routine to load a solver quantity into the data structures for MPI periodic communication and to launch
   ...
   * \param[in] commType - Enumerated type for the quantity to be communicated.
   */
  void InitiatePeriodicComms(CGeometry* geometry, const CConfig* config, unsigned short val_periodic_index,
                             unsigned short commType);

  /*!
   * \brief Routine to complete the set of non-blocking periodic communications launched by InitiatePeriodicComms() and
   * unpacking of the data in the solver class.
   * \param[in] geometry - Geometrical definition of the problem.
   * \param[in] config - Definition of the particular problem.
   * \param[in] val_periodic_index - Index for the periodic marker to be treated (first in a pair).
   * \param[in] commType - Enumerated type for the quantity to be unpacked.
   */
  void CompletePeriodicComms(CGeometry* geometry, const CConfig* config, unsigned short val_periodic_index,
                             unsigned short commType);
   ...
   */
  inline virtual void SetResidual_DualTime(CGeometry* geometry, CSolver** solver_container, CConfig* config,
                                           unsigned short iRKStep, unsigned short iMesh,
                                           unsigned short RunTime_EqSystem) {}

  /*!
   * \brief Get the residual, this is useful for the convergence history.
  ...
  /*!
     ...
     */
  inline virtual void CheckTimeSynchronization(CConfig* config, const su2double TimeSync, su2double& timeEvolved,
   ...
   * \param[in] iMesh - Index of the mesh in multi-grid computations.
   */
  inline virtual void ProcessTaskList_DG(CGeometry* geometry, CSolver** solver_container, CNumerics** numerics,
   ...
   * \param[in] iMesh - Index of the mesh in multi-grid computations.
   */
  inline virtual void ADER_SpaceTimeIntegration(CGeometry* geometry, CSolver** solver_container, CNumerics** numerics,
                                                CConfig* config, unsigned short iMesh,
                                                unsigned short RunTime_EqSystem) {}
   ...
   * \param[in] iMesh - Index of the mesh in multi-grid computations.
   */
  inline virtual void ComputeSpatialJacobian(CGeometry* geometry, CSolver** solver_container, CNumerics** numerics,
   ...
   */
  inline virtual void Postprocessing(CGeometry* geometry, CConfig* config, CNumerics** numerics,
   ...
   * \param[in] iRKStep - Current step of the Runge-Kutta iteration.
   */
  inline virtual void Centered_Residual(CGeometry* geometry, CSolver** solver_container, CNumerics** numerics_container,
   ...
   * \param[in] iMesh - Index of the mesh in multi-grid computations.
   */
  inline virtual void Upwind_Residual(CGeometry* geometry, CSolver** solver_container, CNumerics** numerics_container,
                                      CConfig* config, unsigned short iMesh) {}
   ...
   */
  inline virtual void Preprocessing(CGeometry* geometry, CSolver** solver_container, CConfig* config,
   ...
   */
  inline virtual void Preprocessing(CGeometry* geometry, CSolver** solver_container, CConfig* config,
                                    CNumerics** numerics, unsigned short iMesh, unsigned long Iteration,
   ...
   */
  inline virtual void BC_Euler_Wall(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_Clamped(CGeometry* geometry, const CConfig* config, unsigned short val_marker) {}
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_Clamped_Post(CGeometry* geometry, const CConfig* config, unsigned short val_marker) {}
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_Sym_Plane(CGeometry* geometry, const CConfig* config, unsigned short val_marker) {}
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_DispDir(CGeometry* geometry, const CConfig* config, unsigned short val_marker) {}
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_Normal_Displacement(CGeometry* geometry, CNumerics* numerics, const CConfig* config,
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_Normal_Load(CGeometry* geometry, const CConfig* config, unsigned short val_marker) {}
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_Dir_Load(CGeometry* geometry, const CConfig* config, unsigned short val_marker) {}
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_Sine_Load(CGeometry* geometry, const CConfig* config, unsigned short val_marker) {}
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_Damper(CGeometry* geometry, const CConfig* config, unsigned short val_marker) {}
   ...
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual void BC_Periodic(CGeometry* geometry, CSolver** solver_container, CNumerics* numerics,
                                  CConfig* config) {}
   ...
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual void BC_Fluid_Interface(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_ActDisk_Inlet(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
                                       CNumerics* visc_numerics, CConfig* config, unsigned short val_marker) {}
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_ActDisk_Outlet(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
                                        CNumerics* visc_numerics, CConfig* config, unsigned short val_marker) {}
   ...
   */
  inline virtual void BC_ActDisk(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
                                 CNumerics* visc_numerics, CConfig* config, unsigned short val_marker,
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_Isothermal_Wall(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
                                         CNumerics* visc_numerics, CConfig* config, unsigned short val_marker) {}
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_HeatFlux_Wall(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
                                       CNumerics* visc_numerics, CConfig* config, unsigned short val_marker) {}
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_HeatTransfer_Wall(const CGeometry* geometry, const CConfig* config,
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_Far_Field(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
                                   CNumerics* visc_numerics, CConfig* config, unsigned short val_marker) {}
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_Sym_Plane(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
                                   CNumerics* visc_numerics, CConfig* config, unsigned short val_marker) {}
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_Riemann(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
                                 CNumerics* visc_numerics, CConfig* config, unsigned short val_marker) {}
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_TurboRiemann(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
                                      CNumerics* visc_numerics, CConfig* config, unsigned short val_marker) {}

  /*!
   * \brief It computes Fourier transformation for the needed quantities along the pitch for each span in turbomachinery
   * analysis. \param[in] geometry - Geometrical definition of the problem. \param[in] solver_container - Container
   * vector with all the solutions. \param[in] config - Definition of the particular problem. \param[in] marker_flag -
   * Surface marker flag where the function is applied.
   */
  inline virtual void PreprocessBC_Giles(CGeometry* geometry, CConfig* config, CNumerics* conv_numerics,
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_Giles(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
                               CNumerics* visc_numerics, CConfig* config, unsigned short val_marker) {}
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_Inlet(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
                               CNumerics* visc_numerics, CConfig* config, unsigned short val_marker) {}
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_Inlet_Turbo(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
                                     CNumerics* visc_numerics, CConfig* config, unsigned short val_marker) {}
  /*!
   * \brief A virtual member.
   * \param[in] geometry - Geometrical definition of the problem.
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_Inlet_MixingPlane(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
                                           CNumerics* visc_numerics, CConfig* config, unsigned short val_marker) {}
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_Supersonic_Inlet(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
                                          CNumerics* visc_numerics, CConfig* config, unsigned short val_marker) {}
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_Supersonic_Outlet(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
                                           CNumerics* visc_numerics, CConfig* config, unsigned short val_marker) {}
   ...
   * \param[in] val_marker       - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_Custom(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
                                CNumerics* visc_numerics, CConfig* config, unsigned short val_marker) {}
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_Outlet(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
                                CNumerics* visc_numerics, CConfig* config, unsigned short val_marker) {}
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_Engine_Inflow(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
                                       CNumerics* visc_numerics, CConfig* config, unsigned short val_marker) {}
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_Engine_Exhaust(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
                                        CNumerics* visc_numerics, CConfig* config, unsigned short val_marker) {}
   ...
   * \param[in] val_marker - Surface marker where the boundary condition is applied.
   */
  inline virtual void BC_Smoluchowski_Maxwell(CGeometry* geometry, CSolver** solver_container, CNumerics* conv_numerics,
                                              CNumerics* visc_numerics, CConfig* config, unsigned short val_marker) {}
  /*!
   ...
   */
  inline virtual su2double GetSlidingState(unsigned short val_marker, unsigned long val_vertex,
                                           unsigned short val_state, unsigned long donor_index) const {
    return 0;
  }

  /*!
   * \brief Allocates the final pointer of SlidingState depending on how many donor vertex donate to it. That number is
   * stored in SlidingStateNodes[val_marker][val_vertex].
   * \param[in] val_marker - marker index.
   * \param[in] val_vertex - vertex index.
   */
   ...
   */
  inline virtual void SetSlidingState(unsigned short val_marker, unsigned long val_vertex, unsigned short val_state,
                                      unsigned long donor_index, su2double component) {}
   ...
   */
  inline virtual void SetConjugateHeatVariable(unsigned short val_marker, unsigned long val_vertex,
                                               unsigned short pos_var, su2double relaxation_factor, su2double val_var) {}
   ...
   * \param[in] pos_var - variable position (in vector of all conjugate heat variables).
   */
  inline virtual su2double GetConjugateHeatVariable(unsigned short val_marker, unsigned long val_vertex,
   ...
   * \param[in] iRKStep - Current step of the Runge-Kutta iteration.
   */
  inline virtual void ExplicitRK_Iteration(CGeometry* geometry, CSolver** solver_container, CConfig* config,
   ...
   * \param[in] iRKStep - Current step of the Runge-Kutta iteration.
   */
  inline virtual void ClassicalRK4_Iteration(CGeometry* geometry, CSolver** solver_container, CConfig* config,
                                             unsigned short iRKStep) {}
   ...
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual void PrepareImplicitIteration(CGeometry* geometry, CSolver** solver_container, CConfig* config) {}
   ...
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual void CompleteImplicitIteration(CGeometry* geometry, CSolver** solver_container, CConfig* config) {}
  ...
  /*!
   * \brief Adapt the CFL number based on the local under-relaxation parameters computed for each nonlinear iteration.
   * \param[in] geometry - Geometrical definition of the problem.
   * \param[in] config - Definition of the particular problem.
   * \param[in] solver_container - Container vector with all the solutions.
   */
   ...
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual void ImplicitNewmark_Iteration(const CGeometry* geometry, CNumerics** numerics,
   ...
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual void ImplicitNewmark_Update(const CGeometry* geometry, const CConfig* config) {}
   ...
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual void ImplicitNewmark_Relaxation(const CGeometry* geometry, const CConfig* config) {}
   ...
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual void GeneralizedAlpha_Iteration(const CGeometry* geometry, CNumerics** numerics,
                                                 const CConfig* config) {}
   ...
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual void GeneralizedAlpha_UpdateDisp(const CGeometry* geometry, const CConfig* config) {}
   ...
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual void GeneralizedAlpha_UpdateSolution(const CGeometry* geometry, const CConfig* config) {}
   ...
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual void GeneralizedAlpha_UpdateLoads(const CGeometry* geometry, const CConfig* config) {}
   ...
   * \param[in] reconstruction - indicator that the gradient being computed is for upwind reconstruction.
   */
  inline virtual void SetPrimitive_Gradient_GG(CGeometry* geometry, const CConfig* config,
   ...
   * \param[in] reconstruction - indicator that the gradient being computed is for upwind reconstruction.
   */
  inline virtual void SetPrimitive_Gradient_LS(CGeometry* geometry, const CConfig* config,
                                               bool reconstruction = false) {}
   ...
   * \param[in] iRKStep - Current step of the Runge-Kutta iteration.
   */
  inline virtual void Viscous_Residual(CGeometry* geometry, CSolver** solver_container, CNumerics** numerics_container,
                                       CConfig* config, unsigned short iMesh, unsigned short iRKStep) {}
   ...
   * \param[in] iMesh - Index of the mesh in multi-grid computations.
   */
  inline virtual void Source_Residual(CGeometry* geometry, CSolver** solver_container, CNumerics** numerics_container,
                                      CConfig* config, unsigned short iMesh) {}
   ...
   * \param[in] iMesh - Index of the mesh in multi-grid computations.
   */
  inline virtual void Source_Template(CGeometry* geometry, CSolver** solver_container, CNumerics* numerics,
                                      CConfig* config, unsigned short iMesh) {}
   ...
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual void Inviscid_Sensitivity(CGeometry* geometry, CSolver** solver_container, CNumerics* numerics,
                                           CConfig* config) {}
   ...
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual void Smooth_Sensitivity(CGeometry* geometry, CSolver** solver_container, CNumerics* numerics,
                                         CConfig* config) {}
   ...
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual void Viscous_Sensitivity(CGeometry* geometry, CSolver** solver_container, CNumerics* numerics,
                                          CConfig* config) {}
   ...
   * \return Sensitivity of aero funcs wrt design variables (Mach and AoA for now).
   */
   ...
   * \param[in] iTrim - Trim variable index.
   * \return Sensitivity of aero residuals wrt design variables (Mach and AoA for now).
   */
   ...
   * \return Sensitivity of aero funcs wrt flow states.
   */
   ...
   * \param[in] iPoint - Vertex in fluid domain where the sensitivity is computed.
   * \param[in] iVar - Variable index.
   * \return Sensitivity of aero resids wrt flow states.
   */
   ...
   * \param[in] iPoint - Vertex in fluid domain where the sensitivity is computed.
   * \param[in] iVar - Variable index.
   * \return Sensitivity of aero tractions wrt wrt flow states.
   */
   ...
   * \return Sensitivity of aero funcs wrt FSI boundary displacements.
   */
   ...
   * \param[in] iVertex - Marker point index.
   * \param[in] iDim - Dimension index.
   * \return Sensitivity of aero residuals wrt FSI boundary displacements.
   */
   ...
   * \param[in] iVertex - Marker point index.
   * \param[in] iDim - Dimension index.
   * \return Sensitivity of aero tractions wrt FSI boundary displacements.
   */
  ...
  inline virtual void SetCharacPrimVar(unsigned short val_marker, unsigned long val_vertex, unsigned short val_var,
                                       su2double val_value) {}
   ...
   * \return Value of the pressure coefficient.
   */
  inline virtual unsigned long GetDonorGlobalIndex(unsigned short val_marker, unsigned long val_vertex) const {
    return 0;
  }
   ...
   * \return Value of the pressure coefficient.
   */
  inline virtual void SetDonorGlobalIndex(unsigned short val_marker, unsigned long val_vertex,
   ...
   * \return Value of the pressure coefficient.
   */
  inline virtual su2double* GetCharacPrimVar(unsigned short val_marker, unsigned long val_vertex) { return nullptr; }
   ...
   */
  inline virtual su2double GetInlet_FlowDir(unsigned short val_marker, unsigned long val_vertex,
                                            unsigned short val_dim) const {
    return 0;
  }
   ...
   */
  inline virtual void SetInlet_FlowDir(unsigned short val_marker, unsigned long val_vertex, unsigned short val_dim,
   ...
   */
  inline virtual void SetInlet_TurbVar(unsigned short val_marker, unsigned long val_vertex, unsigned short val_dim,
   ...
   */
  inline virtual void SetInletAtVertex(const su2double* val_inlet, unsigned short iMarker, unsigned long iVertex){};
   ...
   */
  inline virtual su2double GetInletAtVertex(su2double* val_inlet, unsigned long val_inlet_point,
   ...
   */
  inline virtual su2double GetCSkinFriction(unsigned short val_marker, unsigned long val_vertex,
                                            unsigned short val_dim) const {
    return 0;
  }
   ...
   */
  inline virtual su2double GetWallShearStress(unsigned short val_marker, unsigned long val_vertex) const { return 0; }
   ...
   * \return Value of the back pressure sensitivity coefficient (inviscid + viscous contribution).
   */
   ...
   */
  inline virtual su2double GetCSensitivity(unsigned short val_marker, unsigned long val_vertex) const { return 0; }
   ...
   */
  inline virtual void SetInitialCondition(CGeometry** geometry, CSolver*** solver_container, CConfig* config,
   ...
   */
  inline virtual void PredictStruct_Displacement(CGeometry* geometry, const CConfig* config) {}
   ...
   */
  inline virtual void ComputeAitken_Coefficient(CGeometry* geometry, const CConfig* config, unsigned long iOuterIter) {}
   ...
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual void SetAitken_Relaxation(CGeometry* geometry, const CConfig* config) {}
   ...
   */
  void BasicLoadRestart(CGeometry* geometry, const CConfig* config, const string& filename, unsigned long skipVars);
   ...
   */
  inline virtual void LoadRestart(CGeometry** geometry, CSolver*** solver, CConfig* config, int val_iter,
                                  bool val_update_geo) {}
   ...
   */
  void Read_SU2_Restart_ASCII(CGeometry* geometry, const CConfig* config, string val_filename);
   ...
   * \param[in] val_filename - String name of the restart file.
   */
  void Read_SU2_Restart_Binary(CGeometry* geometry, const CConfig* config, string val_filename);
   ...
   * \param[in] val_filename - String name of the restart file.
   */
  void Read_SU2_Restart_Metadata(CGeometry* geometry, CConfig* config, bool adjoint_run, string val_filename) const;
   ...
   */
  void LoadInletProfile(CGeometry** geometry, CSolver*** solver, CConfig* config, int val_iter,
   ...
   */
  void SolveTypicalSectionWingModel(CGeometry* geometry, su2double Cl, su2double Cm, CConfig* config,
   ...
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual su2double Compute_LoadCoefficient(su2double CurrentTime, su2double RampTime, const CConfig* config) {
    return 0.0;
  }
   ...
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual void Compute_StiffMatrix(CGeometry* geometry, CNumerics** numerics, const CConfig* config) {}
   ...
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual void Compute_StiffMatrix_NodalStressRes(CGeometry* geometry, CNumerics** numerics,
                                                         const CConfig* config) {}
   ...
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual void Compute_MassMatrix(const CGeometry* geometry, CNumerics** numerics, const CConfig* config) {}
   ...
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual void Compute_MassRes(const CGeometry* geometry, CNumerics** numerics, const CConfig* config) {}
   ...
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual void Compute_NodalStressRes(CGeometry* geometry, CNumerics** numerics, const CConfig* config) {}
   ...
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual void Compute_DeadLoad(CGeometry* geometry, CNumerics** numerics, const CConfig* config) {}
   ...
   * \param[in] config - Definition of the particular problem.
   */
  inline virtual void SetVolumetricHeatSource(CGeometry* geometry, CConfig* config) {}
   ...
   */
  inline virtual void PreprocessAverage(CSolver** solver, CGeometry* geometry, CConfig* config,
                                        unsigned short marker_flag) {}
   ...
   * \param[in] val_marker - Surface marker where the average is evaluated.
   */
  inline virtual void TurboAverageProcess(CSolver** solver, CGeometry* geometry, CConfig* config,
                                          unsigned short marker_flag) {}
   ...
   */
  inline virtual const su2double* GetAverageTurboVelocity(unsigned short valMarker, unsigned short valSpan) const {
    return nullptr;
  }
   ...
   * \return Value of the Average Total Pressure on the surface <i>val_marker</i>.
   */
  inline virtual void SetExtAverageTurboVelocity(unsigned short valMarker, unsigned short valSpan,
                                                 unsigned short valIndex, su2double valTurboVelocity) {}
   ...
   */
  inline virtual const su2double* GetTurboVelocityIn(unsigned short inMarkerTP, unsigned short valSpan) const {
    return nullptr;
  }
   ...
   */
  inline virtual const su2double* GetTurboVelocityOut(unsigned short inMarkerTP, unsigned short valSpan) const {
    return nullptr;
  }
   ...
   * \param[in] inMarkerTP - turboperformance marker.
   */
  inline virtual void SetTurboVelocityIn(const su2double* value, unsigned short inMarkerTP, unsigned short valSpan) {}
   ...
   * \param[in] inMarkerTP - turboperformance marker.
   */
  inline virtual void SetTurboVelocityOut(const su2double* value, unsigned short inMarkerTP, unsigned short valSpan) {}
   ...
   * \param[in] referenceCoord - Determine if the mesh is deformed from the reference or from the current coordinates.
   */
  inline virtual void SetMesh_Stiffness(CNumerics** numerics, CConfig* config) {}
   ...
   */
  virtual void ApplyGradientSmoothingDV(CGeometry* geometry, CNumerics* numerics, CSurfaceMovement* surface_movement,
                                        CVolumetricMovement* grid_movement, CConfig* config, su2double** Gradient) {}
   ...
   * \param[in] config - Definition of the particular problem.
   */
  virtual void RecordTapeAndCalculateOriginalGradient(CGeometry* geometry, CSurfaceMovement* surface_movement,
   ...
   */
  inline void StoreVertexTractionsAdjoint(unsigned short iMarker, unsigned long iVertex, unsigned short iDim,
  ...
  /*!
   * \brief Get minimum volume in the mesh.
   * \return
   */
  ...
  /*!
   * \brief Whether the methods of the solver called by multi/single-grid iteration can be executed by multiple threads.
 ...
 protected:
   ...
   * \param[in] config - Definition of the particular problem.
   */
  void SetVerificationSolution(unsigned short nDim, unsigned short nVar, CConfig* config);
   ...
   */
  static inline void ResidualReductions_PerThread(unsigned long iPoint, unsigned short iVar, su2double res,
                                                  su2double* resRMS, su2double* resMax, unsigned long* idxMax) {
    res = fabs(res);
    resRMS[iVar] += res * res;
    if (res > resMax[iVar]) {
      resMax[iVar] = res;
   ...
   */
  inline void ResidualReductions_FromAllThreads(const CGeometry* geometry, const CConfig* config,
                                                const su2double* resRMS, const su2double* resMax,
    ...
    for (unsigned short iVar = 0; iVar < nVar; iVar++) {
      ...
      AddRes_Max(iVar, resMax[iVar], geometry->nodes->GetGlobalIndex(idxMax[iVar]),
                 geometry->nodes->GetCoord(idxMax[iVar]));
    }
   ...
   */
  inline void AddRes_Max(unsigned short val_var, su2double val_residual, unsigned long val_point,
    ...
    if (val_residual > Residual_Max[val_var]) {
      ...
      for (unsigned short iDim = 0; iDim < nDim; iDim++) Point_Max_Coord[val_var][iDim] = val_coord[iDim];
    }
  }
   ...
   * \param[in] val_coord - Location (x, y, z) of the max residual point.
   */
  inline void AddRes_Max_BGS(unsigned short val_var, su2double val_residual, unsigned long val_point,
                             const su2double* val_coord) {
    if (val_residual > Residual_Max_BGS[val_var]) {
      Residual_Max_BGS[val_var] = val_residual;
      Point_Max_BGS[val_var] = val_point;
      for (unsigned short iDim = 0; iDim < nDim; iDim++) Point_Max_Coord_BGS[val_var][iDim] = val_coord[iDim];
    }
  }
};
