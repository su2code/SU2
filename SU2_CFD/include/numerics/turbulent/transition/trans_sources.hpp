...
/*!
 * \class CSourcePieceWise_TranLMROUGH
 * \brief Class for integrating the source terms of the LMROUGH transition model equations.
 * \ingroup SourceDiscr
 * \author M. Schifone.
 */
template <class FlowIndices>
class CSourcePieceWise_TransLMROUGH final : public CNumerics {
 private:
  const FlowIndices idx; /*!< \brief Object to manage the access to the flow primitives. */
  ...
  const su2double c_CF = 0.6;
  const su2double sigmat = 2.0;
  const su2double sigma_A_r = 10.0;
  const su2double c_Ar1 = 8.0;
  const su2double c_Ar2 = 0.0005;
  const su2double c_Ar3 = 2.0;
  ...
  su2double Jacobian_Buffer[9];  // Static storage for the Jacobian (which needs to be pointer for return type). nb contains the partial derivatives of residual wrt trans variables
 ...
 public:
   ...
   * \param[in] config - Definition of the particular problem.
   */
  CSourcePieceWise_TransLMROUGH(unsigned short val_nDim, unsigned short val_nVar, const CConfig* config)
      : CNumerics(val_nDim, 3, config), idx(val_nDim, config->GetnSpecies()), options(config->GetLMParsedOptions()){
    /*--- "Allocate" the Jacobian using the static buffer. ---*/
    Jacobian_i[0] = Jacobian_Buffer;
   ...
   */
  ResidualType<> ComputeResidual(const CConfig* config) override {
    /*--- ScalarVar[0] = k, ScalarVar[0] = w, TransVar[0] = gamma, TransVar[1] = ReThetaT and TransVar[2] = A_r ---*/
    /*--- dU/dx = PrimVar_Grad[1][0] ---*/
    AD::StartPreacc();
    AD::SetPreaccIn(StrainMag_i);
    AD::SetPreaccIn(ScalarVar_i, nVar);
    ...
    if (dist_i > 1e-10) {
      su2double Tu = 1.0;
      if (TurbFamily == TURB_FAMILY::KW) Tu = max(100.0 * sqrt(2.0 * ScalarVar_i[0] / 3.0) / Velocity_Mag, 0.027); //computed as in LM class
      if (TurbFamily == TURB_FAMILY::SA) Tu = config->GetTurbulenceIntensity_FreeStream() * 100;

      /*--- Roughness influence function for LMROUGH transition model ---*/
      su2double F_Ar1 = 0.0;
      const su2double C_Ar = sqrt(c_Ar3 / (3.0 * c_Ar2)); 
      if (TransVar_i[2] < C_Ar) {
          // Cubic growth for A_r < C_Ar
          F_Ar1 = c_Ar2 * pow(TransVar_i[2], 3);
      } else {
          // Linear growth for A_r >= C_Ar
          F_Ar1 = c_Ar3 * (TransVar_i[2] - C_Ar) + (c_Ar2 * pow(C_Ar, 3));
      }
      ...
      /*-- production term of ReThetaT modified for LMROUGHm model--*/
      su2double PRethetat = 0.0; 
      PRethetat = c_theta *( Density_i / time_scale) * ((Corr_Ret - TransVar_i[1]) * (1.0 - f_theta) - ((TransVar_i[1]-30.0)/TransVar_i[1])*F_Ar1);
      ...
      /* To enable the production term defined in LM solver class*/
      su2double PAr = 0.0;
      su2double DAr = 0.0;
      ...
      Residual[0] += (Pg - Dg) * Volume;
      Residual[1] += (PRethetat - DRethetat) * Volume;
      Residual[2] += (PAr - DAr)* Volume;
      // No production term for A_r in this model
      ...
      if (TransVar_i[2] < C_Ar) {
        Jacobian_i[1][2] += -(c_theta / time_scale) * (((TransVar_i[1]-30.0)/TransVar_i[1])*3 * c_Ar2 * pow(TransVar_i[2], 2)) * Volume;
      }  else {
        Jacobian_i[1][2] += -(c_theta / time_scale) * (((TransVar_i[1]-30.0)/TransVar_i[1])*c_Ar3) * Volume;
      }
    
      /*--- Jacobian for A_r (Residual[2]) ---*/
      //Since the source term for A_r (Residual[2]) is zero in this model, derivatives are also zero.
      Jacobian_i[2][0] = 0.0;  
      Jacobian_i[2][1] = 0.0;  
      Jacobian_i[2][2] = 0.0;  
