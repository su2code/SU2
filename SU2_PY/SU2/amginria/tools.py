...
def set_flow_config_ini(config, cur_solfil, sensor_tags, mesh_size):
    ...
    config.HISTORY_OUTPUT   = ['ITER', 'RMS_RES', 'AERO_COEFF', 'FLOW_COEFF', 'CFL_NUMBER', 'WALL_TIME']
    config.MATH_PROBLEM     = 'DIRECT'
    if 'GOAL' in sensor_tags:
        config.VOLUME_OUTPUT  = 'PRIMITIVE, RESIDUAL, MESH_QUALITY'
        config.COMPUTE_METRIC = 'NO'
    else:
        config.VOLUME_OUTPUT   = 'PRIMITIVE, RESIDUAL, METRIC, MESH_QUALITY'
...
def set_adj_config_ini(config, cur_solfil, cur_solfil_adj, mesh_size):
    ...
    config.VOLUME_OUTPUT        = 'PRIMITIVE, RESIDUAL, SENSITIVITY, METRIC'
    config.HISTORY_OUTPUT       = ['ITER', 'RMS_RES', 'SENSITIVITY', 'WALL_TIME']
    config.COMPUTE_METRIC       = 'YES'
    config.ADAP_COMPLEXITY      = int(mesh_size)
