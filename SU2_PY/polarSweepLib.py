...
#
    ...
    keyWord='mach'
    MachNum,iprMach=readParameter(b,n,keyWord,iparMcoeff,verbose)  # look for Mach, but avoid Mach for coefficients
...
#
    ...
    if refParNo == 3 :
        ...
        if polarSweepType == 4 :
            raise SystemExit('ERROR in control file: in Mach ramp, base file should include (Pr,rho_r,Tr) or none of them')
...
#
    ...
    nd=0;
    for i in range(0,n):
        sline=b[i].replace('   ',' ').replace('  ',' ').replace('  ',' ').replace('  ',' ').strip().split(' ')
        sv=size(sline)
        if sv == nCol:
            try:
                dd=map(float,sline)
...
#-avoid NULL  error
        ...
        reader = csv.reader((line.replace('\0','').replace('   ',' ').replace('  ',' ').strip() for line in f),delimiter=delim)
