...
def main():
   ...
   node = 2312
   Coord = 2
   file = open("Sensitivity_FD_node" + str(node) + "_centered.txt", "w")
   file.write("node = {}\n".format(node))
   file.flush()
   for i in range(len(delta)):
      ...
      file.flush()
      FD_sens = [node,Coord,delta_used]
      drag_plus = Sens(options, FD_sens)
      file.write("drag_plus = {:16.12f}\n".format(drag_plus))
      file.flush()
      FD_sens = [node,Coord,-delta_used]
      drag_minus = Sens(options, FD_sens)   
      file.write("drag_minus = {:16.12f}\n".format(drag_minus))
      file.flush()
      Sensitivity = (drag_plus - drag_minus)/(2*delta_used)
